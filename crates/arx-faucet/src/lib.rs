...
// README: The arx-faucet is deprecated in favor of the tap. Do not add new code
// to this until you've spoken with the Ecosystem Platform team + dport.

//! This crate provides the Faucet service for creating and funding accounts on the Arx Network.
//!
...
//!
//! ```bash
//! cargo run --bin arx-faucet -- -c TESTNET -m <mint-private-key-path> -s http://localhost:8080 -p 8081
//! ```
//!
//! Check help doc for options details:
//!
//! ```bash
//! cargo run -p arx-faucet -- -h
//! ```

use anyhow::Result;
use arx_config::keys::ConfigKey;
use arx_crypto::ed25519::Ed25519PrivateKey;
use arx_logger::info;
use arx_rest_client::Client;
use arx_sdk::{
    transaction_builder::{arx, TransactionFactory},
    types::{
        account_address::AccountAddress, account_config::arx_test_root_address,
...
/// Arx Testnet utility service for creating test accounts and minting test coins
#[derive(Clone, Debug, Parser)]
#[clap(name = "Arx Faucet", author, version)]
pub struct FaucetArgs {
    ...
    pub port: u16,
    /// Arx fullnode/validator server URL
    #[clap(short = 's', long, default_value = "https://testnet.arx.network/")]
    pub server_url: Url,
    /// Path to the private key for creating test account and minting coins.
    /// To keep Testnet simple, we used one private key for arx root account
    /// To manually generate a keypair, use generate-key:
    /// `cargo run -p generate-keypair -- -o <output_file_path>`
    #[clap(
        short = 'm',
        long,
        default_value = "/opt/arx/etc/mint.key",
        parse(from_os_str)
    )]
...
impl FaucetArgs {
    pub async fn run(self) {
        ...
        let faucet_address: AccountAddress = self
            ...
            .unwrap_or_else(arx_test_root_address);
    ...
    pub fn new(
        ...
        Service {
            ...
            transaction_factory: TransactionFactory::new(chain_id)
                .with_gas_unit_price(std::cmp::max(1, arx_protocol_constants::GAS_UNIT_PRICE))
        ...
        service
            ...
            .submit_and_wait(&faucet_account.sign_with_transaction_builder(
                service.transaction_factory.payload(
                    arx::arx_coin_delegate_mint_capability(delegated_account.address()),
                ),
            ))
            .await
    ...
    service
        ...
        .submit_and_wait(
            &delegated_account.sign_with_transaction_builder(
                service
                    .transaction_factory
                    .payload(arx::arx_coin_claim_mint_capability()),
            ),
        )
        .await
        .unwrap();
