...
#[cfg(test)]
mod tests {
    use arx_crypto::{ed25519::Ed25519PublicKey, hash::HashValue};
    use arx_faucet::{routes, Service};
    ...
    fn setup(maximum_amount: Option<u64>) -> (AccountStates, Arc<Service>) {
        ...
        let accounts = AccountStates::new(arx_infallible::RwLock::new(HashMap::new()));
        accounts
            .write()
    ...
    async fn handle_get_transaction(
        ...
        last_txn.lock().unwrap().as_ref().map_or_else(
            || Err(warp::reject()),
            |txn| {
                let info = arx_rest_client::arx_api_types::TransactionInfo {
                    version: 0.into(),
                ...
                let serializable_txn: arx_rest_client::arx_api_types::Transaction = (
