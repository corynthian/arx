...
use arx_logger::{error, info, warn};
pub use arx_metrics_core::{
...
use arx_metrics_core::{Encoder, TextEncoder};
...
#[deprecated(
    note = "The arx-push-metrics crate is deprecrated. In future prefer to use arx-metrics-core directly (pull) or use arx-telemetry (push)."
)]
impl MetricsPusher {
    fn push(push_metrics_endpoint: &str) {
        let mut buffer = Vec::new();

        if let Err(e) = TextEncoder::new().encode(&arx_metrics_core::gather(), &mut buffer) {
