...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestArxCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestArxCrypto {
    TestArxCrypto(
...
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestArxCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestArxCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestArxCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestArxCrypto>>();
...
//     assert!(sig.verify_arbitrary_msg(b"Hello Arx!", &pk).is_ok());
// }
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    let message = TestArxCrypto("test".to_owned());
    ...
    println!("// signature on TestArxCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    println!(
        "// The signed messages are \"Hello, Arx <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );
