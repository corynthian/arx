...
def run_node(network: Network, image_repo: str):
    ...
    container_name = f"arx-tools-{network}"
    LOG.info(f"Trying to run arx CLI local testnet from image: {image_name}")
    ...
    LOG.info(f"Running arx CLI local testnet from image: {image_name}")
...
def build_image_name(network: Network, image_repo: str):
    return f"{image_repo}arxlabs/tools:{network}"
