...
use crate::{
    ...
    genesis::git::{
        Client, GitOptions, BALANCES_FILE, LAYOUT_FILE, OPERATOR_FILE, OWNER_FILE,
    },
...
use arx_vm_genesis::{default_gas_schedule, AccountBalance};
...
/// Tool for setting up an Arx chain Genesis transaction
///
...
pub enum GenesisTool {
    ...
    GetStakeLockAddresses(tools::StakeLockAddresses),
}
...
impl GenesisTool {
    pub async fn execute(self) -> CliResult {
        match self {
            ...
            GenesisTool::GetStakeLockAddresses(tool) => tool.execute_serialized().await,
        }
    }
}
...
pub fn fetch_mainnet_genesis_info(git_options: GitOptions) -> CliTypedResult<MainnetGenesisInfo> {
    ...
    // Check that the user has a reasonable amount of ARX, since below the minimum gas amount is
    // not useful 1 ARX minimally
...
fn get_config(
    ...
    let vault_account_address: AccountAddress = parse_required_option(
        &owner_config.vault_account_address,
        owner_file,
        "vault_account_address",
        AccountAddressWithChecks::from_str,
    )?
    .into();
    let vault_account_public_key = parse_required_option(
        &owner_config.vault_account_public_key,
        owner_file,
    ...
    if is_mainnet && !join_during_genesis {
        return Ok(ValidatorConfiguration {
            ...
            operator_account_public_key,
            vault_account_address: vault_account_address.into(),
            vault_account_public_key,
...
fn validate_validators(
    ...
    for (i, validator) in validators.iter().enumerate() {
        let name = layout.users.get(i).unwrap().to_string();
        ...
        if !initialized_accounts.contains_key(&validator.vault_account_address.into()) {
            errors.push(CliError::UnexpectedError(format!(
                "Vault {} in validator {} is is not in the balances.yaml file",
                validator.vault_account_address, name
            )));
        }
