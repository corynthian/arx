...
use crate::{
    common::{
        types::{OptionalLockAddressArgs, PromptOptions, RngArgs},
        utils::{read_from_file, write_to_file},
    },
    genesis::{
        git::{
            from_yaml, GitOptions, SetupGit, BALANCES_FILE, FRAMEWORK_NAME,
        },
...
use arx_genesis::{
    config::{
        AccountBalanceMap, HostAndPort, Layout, ValidatorConfiguration,
    },
    keys::PublicIdentity,
};
use arx_keygen::KeyGen;
use arx_temppath::TempPath;
use arx_types::{account_address::AccountAddress, chain_id::ChainId};
use arx_vm_genesis::{AccountBalance, TestValidator};
use std::{
    collections::HashMap,
    path::{Path, PathBuf},
    str::FromStr,
...
async fn test_mainnet_genesis_e2e_flow() {
    ...
    let vault_identity1 = load_identity(dir.path(), "vault-0");
    let vault_identity2 = load_identity(dir.path(), "vault-1");
    ...
    create_account_balances_file(PathBuf::from(git_dir), vec![
        ...
        vault_identity1.account_address,
        vault_identity2.account_address,
        account_1,
        account_2,
...
async fn create_layout_file(
    ...
    layout.is_test = true;
    layout.total_supply = Some(INITIAL_BALANCE * 10);
...
async fn generate_keys(dir: &Path, name: &str) -> PathBuf {
    ...
    let command = GenerateKeys {
        lock_address_args: OptionalLockAddressArgs { lock_address: None },
...
async fn set_validator_config(
    ...
    let command = SetValidatorConfiguration {
        ...
        vault_public_identity_file: Some(vault_identity_file.to_path_buf()),
