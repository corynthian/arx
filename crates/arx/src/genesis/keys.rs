...
use crate::{
    common::{
        types::{CliError, CliTypedResult, OptionalLockAddressArgs, PromptOptions, RngArgs},
        utils::{
...
pub struct SetValidatorConfiguration {
    ...
    /// Host and port pair for the validator e.g. 127.0.0.1:6180 or arxlabs.com:6180
    #[clap(long)]
    pub(crate) validator_host: HostAndPort,

    /// Host and port pair for the fullnode e.g. 127.0.0.1:6180 or arxlabs.com:6180
    #[clap(long)]
    ...
    /// Path to vault public identity, defaults to owner identity
    #[clap(long, parse(from_os_str))]
    pub(crate) vault_public_identity_file: Option<PathBuf>,
...
impl CliCommand<()> for SetValidatorConfiguration {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        // Load vault
        let vault_identity = if let Some(vault_keys_file) = self.vault_public_identity_file {
            read_public_identity_file(vault_keys_file.as_path())?
        } else {
        ...
        let owner_config = OwnerConfiguration {
            ...
            vault_account_address: vault_identity.account_address.into(),
            vault_account_public_key: vault_identity.account_public_key,
            operator_account_address: operator_identity.account_address.into(),
