...
use arx_gas::{AbstractValueSizeGasParameters, NativeGasParameters};
use arx_rest_client::arx_api_types::MoveType;
...
pub struct FrameworkPackageArgs {
    /// Git revision or branch for the Arx framework
    ///
    ...
    /// Local framework directory for the Arx framework
    ///
...
impl FrameworkPackageArgs {
    pub fn init_move_dir(
        ...
        const ARX_FRAMEWORK: &str = "ArxFramework";
        const ARX_GIT_PATH: &str = "https://github.com/arx-labs/arx-core.git";
        const SUBDIR_PATH: &str = "arx-move/framework/arx-framework";
        ...
        if let Some(ref path) = self.framework_local_dir {
            dependencies.insert(ARX_FRAMEWORK.to_string(), Dependency {
            ...
            dependencies.insert(ARX_FRAMEWORK.to_string(), Dependency {
                local: None,
                git: Some(ARX_GIT_PATH.to_string()),
...
/// This can then be run with `arx move run-script`
#[derive(Parser)]
...
impl CliCommand<&'static str> for TestPackage {
    ...
    async fn execute(self) -> CliTypedResult<&'static str> {
        ...
        let result = move_cli::base::test::run_move_unit_tests(
            ...
            arx_debug_natives::arx_debug_natives(
...
/// Publishes the modules in a Move package to the Arx blockchain
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for PublishPackage {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        let payload = arx_cached_packages::arx::code_publish_package_txn(
...
/// Publishes the modules in a Move package to the Arx blockchain under a resource account
#[derive(Parser)]
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        .map(|p| p.account)
        {
            ...
            return Err(CliError::CommandArgumentError(
                "Please provide an account using --profile or run arx init".to_string(),
            ));
        };
        ...
        let payload = arx_cached_packages::arx::resource_account_create_resource_account_and_publish_package(
            bcs::to_bytes(&seed)?,
            bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
            compiled_units,
        );
