...
/// Tool for interacting with configuration of the Arx CLI tool
///
...
impl CliCommand<()> for GenerateShellCompletions {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        generate(self.shell, &mut command, "arx".to_string(), &mut file);
        Ok(())
    }
}
...
pub struct SetGlobalConfig {
    ...
    ///
    /// `Workspace` will put the `.arx/` folder in the current directory, where
    /// `Global` will put the `.arx/` folder in your home directory
    #[clap(long)]
...
pub enum ConfigType {
    /// Per system user configuration put in `<HOME>/.arx`
    Global,
    /// Per directory configuration put in `<CURRENT_DIR>/.arx`
    Workspace,
}
