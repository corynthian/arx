...
use arx_logger::{debug, Level};
use arx_rest_client::{arx_api_types::HashValue, Account, Client, State};
use arx_telemetry::service::telemetry_is_disabled;
use arx_types::{chain_id::ChainId, transaction::authenticator::AuthenticationKey};
...
async fn send_telemetry_event(
    ...
    // Send the event
    arx_telemetry::cli_metrics::send_cli_telemetry_event(
        build_information,
        command.into(),
        latency,
        success,
...
pub fn start_logger() {
    let mut logger = arx_logger::Logger::new();
