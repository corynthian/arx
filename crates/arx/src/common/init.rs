...
use arx_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, ValidCryptoMaterialStringExt};
use arx_rest_client::{
    arx_api_types::{ArxError, ArxErrorCode},
    error::{ArxErrorResponse, RestError},
};
...
/// 1 ARX (might not actually get that much, depending on the faucet)
const NUM_DEFAULT_OCTAS: u64 = 100000000;

/// Tool to initialize current directory for the arx tool
///
/// Configuration will be pushed into .arx/config.yaml
#[derive(Debug, Parser)]
pub struct InitTool {
...
impl CliCommand<()> for InitTool {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        let mut profile_config = if let Some(profile_config) = config.remove_profile(profile_name) {
            prompt_yes_with_override(&format!("Arx already initialized for profile {}, do you want to overwrite the existing config?", profile_name), self.prompt_options)?;
            profile_config
        } else {
        ...
        match network {
            Network::Mainnet => {
                profile_config.rest_url =
                    Some("https://fullnode.mainnet.arxlabs.com".to_string());
                profile_config.faucet_url = None;
            },
            Network::Testnet => {
                profile_config.rest_url =
                    Some("https://fullnode.testnet.arxlabs.com".to_string());
                profile_config.faucet_url =
                    Some("https://faucet.testnet.arxlabs.com".to_string());
            },
            Network::Devnet => {
                profile_config.rest_url = Some("https://fullnode.devnet.arxlabs.com".to_string());
                profile_config.faucet_url = Some("https://faucet.devnet.arxlabs.com".to_string());
            },
            Network::Local => {
        ...
        let client = arx_rest_client::Client::new(
        ...
        let account_exists = match client.get_account(address).await {
            Ok(_) => true,
            Err(err) => {
                if let RestError::Api(ArxErrorResponse {
                    error:
                        ArxError {
                            error_code: ArxErrorCode::ResourceNotFound,
                            ..
                        },
                    ..
                })
                | RestError::Api(ArxErrorResponse {
                    error:
                        ArxError {
                            error_code: ArxErrorCode::AccountNotFound,
                            ..
                        },
                    ..
                }) = err
        ...
        config.save()?;
        eprintln!("\n---\nArx CLI is now set up for account {} as profile {}!  Run `arx --help` for more information about commands", address, self.profile_options.profile_name().unwrap_or(DEFAULT_PROFILE));
        Ok(())
    }
}
