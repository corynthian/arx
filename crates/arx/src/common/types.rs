...
use arx_protocol_constants::adjust_gas_headroom;
use arx_keygen::KeyGen;
use arx_rest_client::{arx_api_types::HashValue, error::RestError, Client, Transaction};
use arx_sdk::{transaction_builder::TransactionFactory, types::LocalAccount};
use arx_types::{
    chain_id::ChainId,
...
pub enum CliError {
    ...
    #[error("Unable to find config {0}, have you run `arx init`?")]
...
impl From<arx_config::config::Error> for CliError {
    fn from(e: arx_config::config::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}

impl From<arx_github_client::Error> for CliError {
    fn from(e: arx_github_client::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<arx_crypto::CryptoMaterialError> for CliError {
    fn from(e: arx_crypto::CryptoMaterialError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
/// Config saved to `.arx/config.yaml`
...
pub const CONFIG_FOLDER: &str = ".arx";
...
impl CliConfig {
    ...
    pub fn config_exists(mode: ConfigSearchMode) -> bool {
        if let Ok(folder) = Self::arx_folder(mode) {
    ...
    pub fn load(mode: ConfigSearchMode) -> CliTypedResult<Self> {
        let folder = Self::arx_folder(mode)?;
    ...
    /// Saves the config to ./.arx/config.yaml
    pub fn save(&self) -> CliTypedResult<()> {
        let arx_folder = Self::arx_folder(ConfigSearchMode::CurrentDir)?;

        // Create if it doesn't exist
        create_dir_if_not_exist(arx_folder.as_path())?;

        // Save over previous config file
        let config_file = arx_folder.join(CONFIG_FILE);
        ...
        let legacy_config_file = arx_folder.join(LEGACY_CONFIG_FILE);
    ...
    /// Finds the current directory's .arx folder
    fn arx_folder(mode: ConfigSearchMode) -> CliTypedResult<PathBuf> {
...
impl RestOptions {
    ...
    pub fn url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|p| p.rest_url)
        {
            ...
            Err(CliError::CommandArgumentError("No rest url given.  Please add --url or add a rest_url to the .arx/config.yaml for the current profile".to_string()))
        }
    }
...
pub fn load_account_arg(str: &str) -> Result<AccountAddress, CliError> {
            ...
            .map(|p| p.private_key)
    {
        ...
        Err(CliError::CommandArgumentError(
            "'--account' or '--profile' after using arx init must be provided".to_string(),
        ))
    }
}
...
pub struct FaucetOptions {
    /// URL for the faucet endpoint e.g. https://faucet.devnet.arxlabs.com
    #[clap(long)]
...
impl FaucetOptions {
    ...
    pub fn faucet_url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|profile| profile.faucet_url)
        {
            ...
            Err(CliError::CommandArgumentError("No faucet given.  Please add --faucet-url or add a faucet URL to the .arx/config.yaml for the current profile".to_string()))
        }
    }
}
...
pub struct GasOptions {
    ...
    ///
    /// The amount of Octas (10^-8 ARX) used for a transaction is equal
...
#[derive(Parser)]
pub struct OptionalLockAddressArgs {
    /// Address of the Staking lock
    ///
    /// Defaults to the profile's `AccountAddress`
    #[clap(long, parse(try_from_str=crate::common::types::load_account_arg))]
    pub(crate) lock_address: Option<AccountAddress>,
}

#[derive(Parser)]
pub struct LockAddressArgs {
    /// Address of the Staking lock
    #[clap(long, parse(try_from_str=crate::common::types::load_account_arg))]
    pub(crate) lock_address: AccountAddress,
}
