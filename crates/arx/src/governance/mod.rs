...
use crate::{
    common::{
        types::{
            CliError, CliTypedResult, MovePackageDir, LockAddressArgs, ProfileOptions,
...
use arx_framework::{BuildOptions, BuiltPackage, ReleasePackage};
use arx_logger::warn;
use arx_rest_client::{
    arx_api_types::{Address, HexEncodedBytes, U128, U64},
    Client, Transaction,
};
use arx_sdk::move_types::language_storage::CORE_CODE_ADDRESS;
use arx_types::{
    ...
    async fn execute(mut self) -> CliTypedResult<Vec<ProposalSummary>> {
        ...
        let events = client
            .get_account_events_bcs(
                AccountAddress::ONE,
                "0x1::governance::GovernanceEvents",
...
impl CliCommand<ProposalSubmissionSummary> for SubmitProposal {
    ...
    async fn execute(mut self) -> CliTypedResult<ProposalSubmissionSummary> {
        ...
        let txn: Transaction = if self.is_multi_step {
            self.txn_options
                .submit_transaction(arx::governance_create_proposal_v2(
                    self.lock_address_args.lock_address,
                    script_hash.to_vec(),
            ...
            self.txn_options
                .submit_transaction(arx::governance_create_proposal(
                    self.lock_address_args.lock_address,
                    script_hash.to_vec(),
                    self.metadata_url.to_string().as_bytes().to_vec(),
                    metadata_hash.to_hex().as_bytes().to_vec(),
        ...
        if let Transaction::UserTransaction(inner) = txn {
            ...
            let proposal_id = if let Some(event) = inner.events.into_iter().find(|event| {
                event.typ.to_string().as_str() == "0x1::governance::CreateProposalEvent"
            }) {
...
impl CliCommand<Vec<TransactionSummary>> for SubmitVote {
    ...
    async fn execute(mut self) -> CliTypedResult<Vec<TransactionSummary>> {
        ...
        for lock_address in self.lock_addresses {
            ...
            if voted {
                println!("Stake {} already voted", lock_address);
                continue;
            }
            ...
            let stake_lock = client
                ...
                .await?
                .into_inner();
            let voting_power = stake_lock.get_governance_voting_power();

            prompt_yes_with_override(
                &format!(
                    "Vote {} with voting power = {} from stake lock {}?",
                    vote_str, voting_power, lock_address
                ),
                self.txn_options.prompt_options,
            )?;
            ...
            summaries.push(
                self.txn_options
                    .submit_transaction(arx::governance_vote(
                        lock_address,
                        proposal_id,
                        vote,
                    ))
...
struct JsonProposal {
    ...
    execution_hash: arx_rest_client::arx_api_types::HashValue,
    expiration_secs: U64,
    is_resolved: bool,
    min_vote_threshold: U128,
    no_votes: U128,
