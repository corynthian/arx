...
use arx_rest_client::{
    arx_api_types::{HashValue, WriteResource, WriteSetChange},
    Transaction,
};
...
// TODO: Add ability to transfer non-ARX coins
// TODO: Add ability to not create account by default
/// Transfer ARX between accounts
///
#[derive(Debug, Parser)]
pub struct TransferCoins {
    /// Address of account to send ARX to
    #[clap(long, parse(try_from_str = crate::common::types::load_account_arg))]
    pub(crate) account: AccountAddress,

    /// Amount of Octas (10^-8 ARX) to transfer
    #[clap(long)]
...
impl CliCommand<TransferSummary> for TransferCoins {
    ...
    async fn execute(self) -> CliTypedResult<TransferSummary> {
        self.txn_options
            .submit_transaction(arx::arx_account_transfer(
                self.account,
                self.amount,
            ))
            .await
...
const SUPPORTED_COINS: [&str; 1] = ["0x1::coin::CoinStore<0x1::arx_coin::ArxCoin>"];
