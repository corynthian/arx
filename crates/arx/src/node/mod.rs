...
use crate::{
    common::{
        types::{
            ...
            OptionalLockAddressArgs, LockAddressArgs, ProfileOptions, PromptOptions, RestOptions,
...
use arx_crypto::{bls12381, bls12381::PublicKey, x25519, ValidCryptoMaterialStringExt};
use arx_faucet::FaucetArgs;
use arx_genesis::config::{HostAndPort, OperatorConfiguration};
use arx_rest_client::{arx_api_types::VersionedEvent, Client, State};
use arx_types::{
    ...
    stake_lock::StakeLock,
    validator_info::ValidatorInfo,
    validator_performance::ValidatorPerformances,
};
...
impl NodeTool {
    pub async fn execute(self) -> CliResult {
        ...
        match self {
            ...
            GetStakeLock(tool) => tool.execute_serialized().await,
...
pub struct StakeLockResult {
    pub state: StakeLockState,
    pub lock_address: AccountAddress,
    pub operator_address: AccountAddress,
    pub vault_address: AccountAddress,
    pub lock_type: StakeLockType,
    pub total_stake: u64,
...
/// Show the stake lock
///
/// Retrieves the associated stake lock from the multiple types for the given owner address
#[derive(Parser)]
pub struct GetStakeLock {
    /// The owner address of the stake lock
...
#[async_trait]
impl CliCommand<Vec<StakeLockResult>> for GetStakeLock {
    fn command_name(&self) -> &'static str {
        "GetStakeLock"
    }

    async fn execute(mut self) -> CliTypedResult<Vec<StakeLockResult>> {
        let owner_address = self.owner_address;
        let client = &self.rest_options.client(&self.profile_options)?;
        get_stake_locks(client, owner_address).await
    }
}

#[derive(Debug, Serialize)]
pub struct StakeLockPerformance {
...
/// Show staking performance of the given staking lock
#[derive(Parser)]
pub struct GetPerformance {
    #[clap(flatten)]
    pub(crate) lock_address_args: LockAddressArgs,
    #[clap(flatten)]
    pub(crate) rest_options: RestOptions,
    #[clap(flatten)]
    pub(crate) profile_options: ProfileOptions,
...
#[async_trait]
impl CliCommand<StakeLockPerformance> for GetPerformance {
    fn command_name(&self) -> &'static str {
        "GetPerformance"
    }

    async fn execute(mut self) -> CliTypedResult<StakeLockPerformance> {
        let client = &self.rest_options.client(&self.profile_options)?;
        let lock_address = self.lock_address_args.lock_address;
        let validator_set = &client
            .get_account_resource_bcs::<ValidatorSet>(CORE_CODE_ADDRESS, "0x1::validator::ValidatorSet")
            .await?
            .into_inner();

        let mut current_epoch_successful_proposals = 0;
        let mut current_epoch_failed_proposals = 0;
        let state = get_stake_lock_state(validator_set, &lock_address);
        if state == StakeLockState::Active || state == StakeLockState::PendingInactive {
            let validator_config = client
                .get_account_resource_bcs::<ValidatorConfig>(
                    lock_address,
                    "0x1::validator::ValidatorConfig",
                )
                .await?
                .into_inner();
            let validator_performances = &client
                .get_account_resource_bcs::<ValidatorPerformances>(
                    CORE_CODE_ADDRESS,
                    "0x1::validator::ValidatorPerformance",
                )
                .await?
                .into_inner();
...
/// Retrieves the stake locks associated with an account
pub async fn get_stake_locks(
    client: &Client,
    owner_address: AccountAddress,
) -> CliTypedResult<Vec<StakeLockResult>> {
    let epoch_info = get_epoch_info(client).await?;
    let validator_set = &client
        .get_account_resource_bcs::<ValidatorSet>(CORE_CODE_ADDRESS, "0x1::validator::ValidatorSet")
        .await?
        .into_inner();
    let mut stake_lock_results: Vec<StakeLockResult> = vec![];

    // Add direct stake locks.
    let direct_stake_lock = get_stake_lock_info(
        client,
        owner_address,
        StakeLockType::Direct,
        0,
        0,
        epoch_info.clone(),
        validator_set,
    ...
    .await;
    if let Ok(direct_stake_lock) = direct_stake_lock {
        stake_lock_results.push(direct_stake_lock);
    };

    Ok(stake_lock_results)
}

pub async fn get_stake_lock_info(
    client: &Client,
    lock_address: AccountAddress,
    lock_type: StakeLockType,
    principal: u64,
    commission_percentage: u64,
    epoch_info: EpochInfo,
    validator_set: &ValidatorSet,
) -> CliTypedResult<StakeLockResult> {
    let stake_lock = client
        .get_account_resource_bcs::<StakeLock>(lock_address, "0x1::validator::StakeLock")
        .await?
        .into_inner();
    let validator_config = client
        .get_account_resource_bcs::<ValidatorConfig>(lock_address, "0x1::validator::ValidatorConfig")
        .await?
        .into_inner();
    let total_stake = stake_lock.get_total_staked_amount();
    ...
    Ok(StakeLockResult {
        state,
        lock_address,
        operator_address: stake_lock.operator_address,
        vault_address: stake_lock.vault_address,
        lock_type,
        total_stake,
        commission_percentage,
        commission_not_yet_unlocked,
        lockup_expiration_utc_time: Time::new_seconds(stake_lock.locked_until_secs).utc_time,
        consensus_public_key,
...
fn get_stake_lock_state(
    validator_set: &ValidatorSet,
    lock_address: &AccountAddress,
) -> StakeLockState {
    if validator_set.active_validators().contains(lock_address) {
        StakeLockState::Active
    } else if validator_set
        .pending_active_validators()
        .contains(lock_address)
    {
        StakeLockState::PendingActive
    } else if validator_set
        .pending_inactive_validators()
        .contains(lock_address)
    {
        StakeLockState::PendingInactive
    } else {
        StakeLockState::Inactive
    }
}

/// Register the current account as a validator node operator
///
/// This will create a new stake lock for the given account.  The voter and operator fields will be
/// defaulted to the stake lock account if not provided.
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for InitializeValidator {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(arx::validator_initialize_validator(
...
/// Arguments used for operator of the staking lock
#[derive(Parser)]
pub struct OperatorArgs {
    #[clap(flatten)]
    pub(crate) lock_address_args: OptionalLockAddressArgs,
}
...
impl OperatorArgs {
    fn address_fallback_to_profile(
        ...
        if let Some(address) = self.lock_address_args.lock_address {
            Ok(address)
        } else {
...
impl CliCommand<TransactionSummary> for JoinValidatorSet {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(arx::validator_join_validator_set(address))
            .await
            .map(|inner| inner.into())
    }
}
...
impl CliCommand<TransactionSummary> for LeaveValidatorSet {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(arx::validator_leave_validator_set(address))
            .await
            .map(|inner| inner.into())
    }
}
...
impl CliCommand<serde_json::Value> for ShowValidatorStake {
    ...
    async fn execute(mut self) -> CliTypedResult<serde_json::Value> {
        ...
        let response = client
            .get_resource(address, "0x1::validator::StakeLock")
            .await?;
...
impl CliCommand<ValidatorConfigSummary> for ShowValidatorConfig {
    ...
    async fn execute(mut self) -> CliTypedResult<ValidatorConfigSummary> {
        ...
        let validator_config: ValidatorConfig = client
            .get_account_resource_bcs(address, "0x1::validator::ValidatorConfig")
            .await?
            .into_inner();
        Ok((&validator_config)
            .try_into()
...
impl CliCommand<ValidatorSetSummary> for ShowValidatorSet {
    ...
    async fn execute(mut self) -> CliTypedResult<ValidatorSetSummary> {
        ...
        let validator_set: ValidatorSet = client
            .get_account_resource_bcs(CORE_CODE_ADDRESS, "0x1::validator::ValidatorSet")
            .await?
            .into_inner();
...
impl From<&ValidatorInfoSummary> for ValidatorInfo {
    fn from(summary: &ValidatorInfoSummary) -> Self {
        ...
        ValidatorInfo::new(
            ...
            arx_types::validator_config::ValidatorConfig::new(
...
/// network locally.  Optionally, a faucet can be added for minting ARX coins.
#[derive(Parser)]
pub struct RunLocalTestnet {
    ...
    ///
    /// This will wipe the arxdb in `test-dir` to remove any incompatible changes, and start
...
impl CliCommand<()> for RunLocalTestnet {
    ...
    async fn execute(mut self) -> CliTypedResult<()> {
        ...
        let node_thread_handle = thread::spawn(move || {
            let result = arx_node::setup_test_environment_and_start_node(
                config_path,
                Some(test_dir_copy),
                false,
                false,
                arx_cached_packages::head_release_bundle(),
                rng,
            );
        ...
        let maybe_faucet_future = if self.with_faucet {
            ...
            let rest_client = arx_rest_client::Client::new(rest_url.clone());
            let start = Instant::now();
...
impl CliCommand<TransactionSummary> for UpdateConsensusKey {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        let consensus_proof_of_possession = self
            ...
            .get_consensus_proof_of_possession(&operator_config)?;
        self.txn_options
            .submit_transaction(arx::validator_rotate_consensus_key(
                address,
                consensus_public_key.to_bytes().to_vec(),
                consensus_proof_of_possession.to_bytes().to_vec(),
            ))
...
impl CliCommand<TransactionSummary> for UpdateValidatorNetworkAddresses {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(arx::validator_update_network_info(
                address,
                // BCS encode, so that we can hide the original type
                bcs::to_bytes(&validator_network_addresses)?,
                bcs::to_bytes(&full_node_network_addresses)?,
...
pub struct AnalyzeValidatorPerformance {
    ...
    /// Filter of stake lock addresses to analyze
    ///
    /// Defaults to all stake lock addresses
    #[clap(long, multiple_values = true, parse(try_from_str=crate::common::types::load_account_arg))]
    pub lock_addresses: Vec<AccountAddress>,
...
impl CliCommand<()> for AnalyzeValidatorPerformance {
    ...
    async fn execute(mut self) -> CliTypedResult<()> {
        ...
        for epoch_info in epochs {
            ...
            if !self.lock_addresses.is_empty() {
                let mut filtered_stats: HashMap<AccountAddress, ValidatorStats> = HashMap::new();
                for lock_address in &self.lock_addresses {
                    filtered_stats.insert(
                        *lock_address,
                        *epoch_stats.validator_stats.get(lock_address).unwrap(),
                    );
                }
...
pub struct BootstrapDbFromBackup {
    ...
    ///
    /// The directory to create the ArxDB with snapshots and transactions from the backup.
    /// The data folder can later be used to start an Arx node. e.g. /opt/arx/data/db
