...
pub struct UpdateTool {
    ...
    #[clap(long, default_value = "arx-labs")]
    repo_owner: String,

    /// The name of the repo to download the binary from.
    #[clap(long, default_value = "arx-core")]
    repo_name: String,
}
    ...
    fn update(&self) -> CliTypedResult<String> {
        ...
        match installation_method {
            ...
            InstallationMethod::Homebrew => {
                return Err(anyhow!(
                    "Detected this CLI comes from homebrew, use `brew upgrade arx` instead"
                )
                .into());
            },
        ...
        let result = config
            .update()
            .map_err(|e| anyhow!("Failed to update Arx CLI: {:#}", e))?;
...
impl CliCommand<String> for UpdateTool {
    ...
    async fn execute(self) -> CliTypedResult<String> {
        tokio::task::spawn_blocking(move || self.update())
            .await
            .context("Failed to self-update Arx CLI")?
    }
}
