...
use crate::{
    ...
    common::{
        ...
        types::{
            ...
            MovePackageDir, OptionalLockAddressArgs, LockAddressArgs, PrivateKeyInputOptions,
    ...
    node::{
        AnalyzeMode, AnalyzeValidatorPerformance, GetStakeLock, InitializeValidator,
        JoinValidatorSet, LeaveValidatorSet, OperatorArgs, OperatorConfigFileArgs,
        ShowValidatorConfig, ShowValidatorSet, ShowValidatorStake, StakeLockResult,
    ...
    stake::{
        LockStake, IncreaseLockup, InitializeStakeOwner, SetVault, SetOperator,
        UnlockStake, WithdrawStake,
    },
    CliCommand,
};
use arx_config::config::Peer;
use arx_crypto::{
    bls12381,
    ed25519::{Ed25519PrivateKey, Ed25519PublicKey},
    x25519, PrivateKey,
};
use arx_genesis::config::HostAndPort;
use arx_keygen::KeyGen;
use arx_logger::warn;
use arx_rest_client::{
    arx_api_types::{IdentifierWrapper, MoveStructTag, MoveType},
    Transaction,
};
use arx_sdk::move_types::{
    account_address::AccountAddress, identifier::Identifier, language_storage::ModuleId,
};
use arx_temppath::TempPath;
use arx_types::on_chain_config::ValidatorSet;
use reqwest::Url;
...
impl CliTestFramework {
    ...
    async fn check_account_exists(&self, index: usize) -> bool {
        ...
        let client = arx_rest_client::Client::new(self.endpoint.clone());
    ...
    pub async fn transfer_invalid_addr(
        ...
        RunFunction {
            ...
            type_args: vec![MoveType::Struct(MoveStructTag::new(
                ...
                IdentifierWrapper::from_str("arx_coin").unwrap(),
                IdentifierWrapper::from_str("ArxCoin").unwrap(),
                vec![],
            ))],
            txn_options: self.transaction_options(sender_index, gas_options),
        }
    ...
    pub async fn show_validator_config(
        ...
        ShowValidatorConfig {
            rest_options: self.rest_options(),
            profile_options: Default::default(),
            operator_args: self.operator_args(Some(lock_index)),
        }
        .execute()
        .await
        .map(|v| (&v).into())
    ...
    pub async fn show_validator_stake(&self, lock_index: usize) -> CliTypedResult<Value> {
        ShowValidatorStake {
            rest_options: self.rest_options(),
            profile_options: Default::default(),
            operator_args: self.operator_args(Some(lock_index)),
    ...
    pub async fn join_validator_set(
        ...
        JoinValidatorSet {
            ...
            operator_args: self.operator_args(lock_index),
        }
        .execute()
        .await
    }
    ...
    pub async fn initialize_stake_owner(
        ...
        InitializeStakeOwner {
            ...
            vault_address: vault_index.map(|idx| self.account_id(idx)),
        }
        .execute()
        .await
    }

    pub async fn create_stake_lock(
        &self,
        owner_index: usize,
        operator_index: usize,
        vault_index: usize,
        ...
        RunFunction {
            ...
            args: vec![
                ...
                ArgWithType::address(self.account_id(vault_index)),
    ...
    pub async fn set_vault(
        ...
        SetVault {
            txn_options: self.transaction_options(owner_index, None),
            vault_address: self.account_id(vault_index),
        }
        .execute()
        .await
    }
    ...
    async fn last_n_transactions_details(&self, count: u16) -> String {
        let result = arx_rest_client::Client::new(self.endpoint.clone())
    ...
    pub fn add_move_files(&self) {
        ...
        let hello_blockchain_contents = include_str!(
            "../../../../arx-move/move-examples/hello_blockchain/sources/hello_blockchain.move"
        );
        ...
        let hello_blockchain_test_contents = include_str!("../../../../arx-move/move-examples/hello_blockchain/sources/hello_blockchain_test.move");
    ...
    pub async fn run_script(
        ...
        RunScript {
            ...
            compile_proposal_args: CompileScriptFunction {
                ...
                framework_package_args: FrameworkPackageArgs {
                    framework_git_rev: None,
                    framework_local_dir: Some(Self::arx_framework_dir()),
                    skip_fetch_latest_git_deps: false,
                },
    ...
    fn operator_args(&self, lock_index: Option<usize>) -> OperatorArgs {
        OperatorArgs {
            lock_address_args: OptionalLockAddressArgs {
                lock_address: lock_index.map(|idx| self.account_id(idx)),
            },
        }
    }
    ...
    pub async fn create_proposal(
        ...
        SubmitProposal {
            ...
            lock_address_args: LockAddressArgs { lock_address },
            txn_options: self.transaction_options(index, None),
            is_multi_step,
            compile_proposal_args: CompileScriptFunction {
    ...
    pub async fn vote(
        ...
        yes: bool,
        no: bool,
        lock_addresses: Vec<AccountAddress>,
    ) {
        SubmitVote {
            proposal_id,
            yes,
