...
use arx_types::{chain_id::ChainId, transaction::Transaction, waypoint::Waypoint};
use arx_vm::ArxVM;
use arx_vm_genesis::{AccountBalance, ValidatorWithCommissionRate};
...
impl MainnetGenesisInfo {
    ...
    fn generate_genesis_txn(&self) -> Transaction {
        arx_vm_genesis::encode_arx_mainnet_genesis_transaction(
            &self.accounts,
            &self.validators,
            &self.framework,
            self.chain_id,
            &arx_vm_genesis::GenesisConfiguration {
    ...
    pub fn generate_waypoint(&mut self) -> anyhow::Result<Waypoint> {
        ...
        let db_rw = DbReaderWriter::new(arxdb);
        arx_executor::db_bootstrapper::generate_waypoint::<ArxVM>(&db_rw, genesis)
    }
}
