...
use arx_vm_genesis::default_gas_schedule;
use rand::Rng;
...
impl TryFrom<&ValidatorNodeConfig> for ValidatorConfiguration {
	...
	let ValidatorIdentity { private_identity, .. } = config.get_key_objects(None)?;
        ...
        Ok(ValidatorConfiguration {
            ...
            vault_account_address: private_identity.account_address.into(),
            vault_account_public_key: private_identity.account_private_key.public_key(),
...
fn set_identity_for_network(network: &mut NetworkConfig) -> anyhow::Result<()> {
    if let Identity::None = network.identity {
        ...
        let peer_id = arx_types::account_address::from_identity_public_key(key.public_key());
        ...
        R: rand::RngCore + rand::CryptoRng,
    {
        ...
        let fullnode_network_listen_address =
            if let Some(template_fullnode_config) = config.full_node_networks.first() {
                ...
                arx_config::utils::get_available_port_in_multiaddr(true)
            };
        ...
        let vfn_network = NetworkConfig {
            listen_address: arx_config::utils::get_available_port_in_multiaddr(true),
