...
use arx_crypto::{bls12381, ed25519::Ed25519PublicKey, x25519};
use arx_types::{
...
pub struct ValidatorConfiguration {
    ...
    pub vault_account_address: AccountAddressWithChecks,
    pub vault_account_public_key: Ed25519PublicKey,
...
impl TryFrom<ValidatorConfiguration> for Validator {
    ...
    fn try_from(config: ValidatorConfiguration) -> Result<Self, Self::Error> {
        ...
        let auth_key = AuthenticationKey::ed25519(&config.vault_account_public_key);
        let derived_address = auth_key.derived_address();
        let vault_address = AccountAddress::from(config.vault_account_address);
        if vault_address != derived_address {
            return Err(anyhow::Error::msg(format!(
                "vault_account_address {} does not match account key derived one {}",
...
pub struct StringOwnerConfiguration {
    ...
    pub vault_account_address: Option<String>,
