...
/// A builder for a `ArxData`, configures what, where, and how to write logs.
pub struct ArxDataBuilder {
    channel_size: usize,
...
impl ArxDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<ArxData> {
    ...
    pub fn build(&mut self) -> Arc<ArxData> {
        let logger = self.build_logger();

        let console_port = if cfg!(feature = "arx-console") {
            self.console_port
        } else {
            None
        };
...
impl ArxData {
    pub fn builder() -> ArxDataBuilder {
        ArxDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> ArxDataBuilder {
        Self::builder()
    }

    pub fn init_for_testing() {
        // Create the Arx Data Builder
...
/// or to a `ArxData` for external logging.
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/arx-logger/src/lib.rs:261 Hello { "world": true }
...
pub struct LoggerFilterUpdater {
    logger: Arc<ArxData>,
    logger_builder: ArxDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<ArxData>, logger_builder: ArxDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
#[cfg(test)]
mod tests {
    use super::{ArxData, LogEntry};
    use crate::{
        arx_logger::{json_format, RUST_LOG_TELEMETRY},
        debug, error, info,
        logger::Logger,
        trace, warn, ArxDataBuilder, Event, Key, KeyValue, Level, LoggerFilterUpdater, Metadata,
    ...
    fn basic() {
        ...
        let expected = format!("{{\"level\":\"INFO\",\"source\":{{\"package\":\"arx_logger\",\"file\":\"crates/arx-logger/src/arx_logger.rs:{line_num}\"}},\"thread_name\":\"{thread_name}\",\"hostname\":\"test-host\",\"timestamp\":\"2022-07-24T23:42:29.540278Z\",\"message\":\"This is a log\",\"data\":{{\"bar\":\"foo_bar\",\"category\":\"name\",\"display\":\"12345\",\"foo\":5,\"test\":true}}}}");
    ...
    fn new_async_logger() -> (ArxDataBuilder, Arc<ArxData>) {
        let mut logger_builder = ArxDataBuilder::new();
