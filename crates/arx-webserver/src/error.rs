...
pub struct Error {
    pub code: u16,
    pub message: String,
    /// Arx blockchain latest onchain ledger version.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub arx_ledger_version: Option<U64>,
}
...
impl Error {
    ...
    pub fn not_found<S: Display>(resource: &str, identifier: S, ledger_version: u64) -> Self {
        ...
        .arx_ledger_version(ledger_version)
    }
    ...
    pub fn arx_ledger_version(mut self, ledger_version: u64) -> Self {
        self.arx_ledger_version = Some(ledger_version.into());
        self
    }
}
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(val) = &self.arx_ledger_version {
            write!(f, "\nArx ledger version: {}", val)?;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_to_string_with_arx_ledger_version() {
        let err = Error::new(StatusCode::BAD_REQUEST, "invalid address".to_owned())
            .arx_ledger_version(123);
        assert_eq!(
            err.to_string(),
            "400 Bad Request: invalid address\nArx ledger version: 123"
        )
    }
