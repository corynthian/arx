...
use arx_api::Context;
use arx_api_types::{AsConverter, LedgerInfo, Transaction, TransactionOnChainData};
use arx_logger::prelude::*;
use arx_storage_interface::state_view::DbStateView;
use arx_vm::data_cache::StorageAdapterOwned;
...
async fn fetch_nexts(
    ...
    let mut epoch_bcs = arx_api_types::U64::from(epoch);
    let mut block_height = block_event.height();
    let mut block_height_bcs = arx_api_types::U64::from(block_height);
    ...
    for (ind, raw_txn) in raw_txns.into_iter().enumerate() {
        ...
        if ind > 0 {
            ...
            if let arx_types::transaction::Transaction::BlockMetadata(ref txn) =
                raw_txn.transaction
            {
                timestamp = txn.timestamp_usecs();
                epoch = txn.epoch();
                epoch_bcs = arx_api_types::U64::from(epoch);
                block_height += 1;
                block_height_bcs = arx_api_types::U64::from(block_height);
            }
        }
        match converter
