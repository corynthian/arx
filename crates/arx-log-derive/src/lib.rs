...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let from_serde = quote! { ::arx_logger::Value::from_serde };
    let from_display = quote! { ::arx_logger::Value::from_display };
    let from_debug = quote! { ::arx_logger::Value::from_debug };
    let key_new = quote! { ::arx_logger::Key::new };
    ...
    let expanded = quote! {
        ...
        impl #impl_generics ::arx_logger::Schema for #name #ty_generics #where_clause {
            fn visit(&self, visitor: &mut dyn ::arx_logger::Visitor) {
                #(#visits)*
            }
        }
    };
