...
use crate::{
    arx::{ArxVersion, Balance},
    error::RestError,
};
use anyhow::{anyhow, Result};
pub use arx_api_types::{
...
use arx_api_types::{
    ...
    ArxError, BcsBlock, Block, GasEstimation, HexEncodedBytes, IndexResponse, MoveModuleId,
    TransactionData, TransactionOnChainData, TransactionsBatchSubmissionResult, UserTransaction,
    VersionedEvent,
};
use arx_crypto::HashValue;
use arx_logger::{debug, info, sample, sample::SampleRate};
use arx_types::{
...
pub const USER_AGENT: &str = concat!("arx-client-sdk-rust / ", env!("CARGO_PKG_VERSION"));
...
type ArxResult<T> = Result<T, RestError>;
...
impl Client {
    ...
    pub fn version_path_base(mut self, version_path_base: String) -> ArxResult<Self> {
    ...
    pub fn build_path(&self, path: &str) -> ArxResult<Url> {
        Ok(self.base_url.join(&self.version_path_base)?.join(path)?)
    }

    pub async fn get_arx_version(&self) -> ArxResult<Response<ArxVersion>> {
        self.get_resource::<ArxVersion>(CORE_CODE_ADDRESS, "0x1::version::Version")
            .await
    }
    ...
    ) -> ArxResult<Response<Balance>> {
        let resp = self
            .get_account_resource(address, "0x1::coin::CoinStore<0x1::arx_coin::ArxCoin>")
            .await?;
    ...
    pub async fn get_account_balance_at_version(
        ...
        let resp = self
            .get_account_resource_at_version(
                address,
                "0x1::coin::CoinStore<0x1::arx_coin::ArxCoin>",
                version,
            )
            .await?;
        resp.and_then(|resource| {
    ...
    pub async fn get_index(&self) -> ArxResult<Response<IndexResponse>> {
        self.get(self.build_path("")?).await
    }

    pub async fn get_index_bcs(&self) -> ArxResult<Response<IndexResponseBcs>> {
        let url = self.build_path("")?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    // TODO: Remove this, just use `get_index`: https://github.com/corynthian/arx/issues/N.
    pub async fn get_ledger_information(&self) -> ArxResult<Response<State>> {
    ...
    ) -> ArxResult<Response<Vec<UserTransaction>>> {
    ...
    pub async fn simulate_bcs(
        &self,
        txn: &SignedTransaction,
    ) -> ArxResult<Response<TransactionOnChainData>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions/simulate")?;
    ...
    pub async fn submit(
        &self,
        txn: &SignedTransaction,
    ) -> ArxResult<Response<PendingTransaction>> {
        let txn_payload = bcs::to_bytes(txn)?;
    ...
    pub async fn submit_bcs(&self, txn: &SignedTransaction) -> ArxResult<Response<()>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions")?;
    ...
    ) -> ArxResult<Response<TransactionsBatchSubmissionResult>> {
    ...
    pub async fn submit_and_wait(
        &self,
        txn: &SignedTransaction,
    ) -> ArxResult<Response<Transaction>> {
    ...
    async fn wait_for_transaction_by_hash_inner<F, Fut, T>(
        ...
        fetch: F,
    ) -> ArxResult<Response<T>>
    where
        F: Fn(HashValue) -> Fut,
        Fut: Future<Output = ArxResult<WaitForTransactionResult<T>>>,
    {
        ...
        loop {
            ...
            match fetch(hash).await {
                ...
                Ok(WaitForTransactionResult::NotFound(error)) => {
                    if let RestError::Api(arx_error_response) = error {
                        if let Some(state) = arx_error_response.state {
                            if expiration_timestamp_secs <= state.timestamp_usecs / 1_000_000 {
                                if reached_mempool {
                                    return Err(anyhow!("Transaction expired. It is guaranteed it will not be committed on chain.").into());
                                } else {
            ...
            if let Some(max_server_lag_wait_duration) = max_server_lag_wait {
                if arx_infallible::duration_since_epoch().as_secs()
                    ...
                    debug!(
                        ...
                        if let Some(timestamp_usecs) = chain_timestamp_usecs {
                            format!(
                                ...
                                arx_infallible::duration_since_epoch()
    ...
    pub async fn get_transactions(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> ArxResult<Response<Vec<Transaction>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transactions_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> ArxResult<Response<Vec<TransactionOnChainData>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transaction_by_hash_bcs(
        &self,
        hash: HashValue,
    ) -> ArxResult<Response<TransactionData>> {
    ...
    pub async fn get_account_resources(
        &self,
        address: AccountAddress,
    ) -> ArxResult<Response<Vec<Resource>>> {
    ...
    pub async fn get_account_resources_bcs(
        &self,
        address: AccountAddress,
    ) -> ArxResult<Response<BTreeMap<StructTag, Vec<u8>>>> {
    ...
    pub async fn get_account_resource(
        ...
        address: AccountAddress,
        resource_type: &str,
    ) -> ArxResult<Response<Option<Resource>>> {
    ...
    pub async fn get_account_modules(
        &self,
        address: AccountAddress,
    ) -> ArxResult<Response<Vec<MoveModuleBytecode>>> {
        self.paginate_with_cursor(
    ...
    pub async fn get_account_modules_bcs(
        &self,
        address: AccountAddress,
    ) -> ArxResult<Response<BTreeMap<MoveModuleId, Vec<u8>>>> {
        self.paginate_with_cursor_bcs(
            &format!("accounts/{}/modules", address),
            MODULES_PER_CALL_PAGINATION,
            None,
    ...
    pub async fn get_account_module(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> ArxResult<Response<MoveModuleBytecode>> {
    ...
    pub async fn get_account_module_bcs(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> ArxResult<Response<bytes::Bytes>> {
        let url = self.build_path(&format!("accounts/{}/module/{}", address, module_name))?;
    ...
    pub async fn get_account_events(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> ArxResult<Response<Vec<VersionedEvent>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
    ...
    pub async fn get_account_events_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> ArxResult<Response<Vec<EventWithVersion>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
            address.to_hex_literal(),
            struct_tag,
    ...
    pub async fn get_account(&self, address: AccountAddress) -> ArxResult<Response<Account>> {
    ...
    pub async fn get_account_bcs(
        &self,
        address: AccountAddress,
    ) -> ArxResult<Response<AccountResource>> {
        let url = self.build_path(&format!("accounts/{}", address))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    pub async fn estimate_gas_price(&self) -> ArxResult<Response<GasEstimation>> {
        let url = self.build_path("estimate_gas_price")?;
        let response = self.inner.get(url).send().await?;
        self.json(response).await
    }

    pub async fn set_failpoint(&self, name: String, actions: String) -> ArxResult<String> {
    ...
    ) -> ArxResult<(reqwest::Response, State)> {
        if !response.status().is_success() {
            Err(parse_error(response).await)
        } else {
    ...
    pub async fn health_check(&self, seconds: u64) -> ArxResult<()> {
    ...
    async fn get<T: DeserializeOwned>(&self, url: Url) -> ArxResult<Response<T>> {
        self.json(self.inner.get(url).send().await?).await
    }

    async fn get_bcs(&self, url: Url) -> ArxResult<Response<bytes::Bytes>> {
        let response = self.inner.get(url).header(ACCEPT, BCS).send().await?;
    ...
    where
        F: Fn() -> Fut,
        RetryFun: Fn(StatusCode, Option<ArxError>) -> bool,
        Fut: Future<Output = ArxResult<T>>,
    {
    ...
    pub async fn paginate_with_cursor_bcs<T: for<'a> Deserialize<'a> + Ord>(
        ...
        limit_per_request: u64,
        ledger_version: Option<u64>,
    ) -> ArxResult<Response<BTreeMap<T, Vec<u8>>>> {
...
pub fn retriable_with_404(status_code: StatusCode, arx_error: Option<ArxError>) -> bool {
    retriable(status_code, arx_error) | matches!(status_code, StatusCode::NOT_FOUND)
}

pub fn retriable(status_code: StatusCode, _arx_error: Option<ArxError>) -> bool {
    matches!(
        status_code,
...
fn parse_state(response: &reqwest::Response) -> ArxResult<State> {
...
async fn parse_error(response: reqwest::Response) -> RestError {
    ...
    match response.json::<ArxError>().await {
