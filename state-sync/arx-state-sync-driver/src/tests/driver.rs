...
use arx_config::config::{NodeConfig, RoleType, StateSyncDriverConfig};
use arx_consensus_notifications::{ConsensusNotificationSender, ConsensusNotifier};
...
async fn create_driver_for_tests(
    ...
    // Create test arx database
    let db_path = arx_temppath::TempPath::new();
    db_path.create_as_dir().unwrap();
    let (_, db_rw) = DbReaderWriter::wrap(ArxDB::new_for_test(db_path.path()));

    // Bootstrap the genesis transaction
    let (genesis, _) = arx_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis));
    bootstrap_genesis::<ArxVM>(&db_rw, &genesis_txn).unwrap();
    ...
    let (consensus_notifier, consensus_listener) =
        arx_consensus_notifications::new_consensus_notifier_listener_pair(5000);
