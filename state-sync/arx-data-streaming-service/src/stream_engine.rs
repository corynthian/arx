...
use arx_data_client::{AdvertisedData, GlobalDataSummary, ResponsePayload};
use arx_id_generator::{IdGenerator, U64IdGenerator};
use arx_logger::prelude::*;
use arx_types::{ledger_info::LedgerInfoWithSignatures, transaction::Version};
...
pub trait DataStreamEngine {
    ...
    /// that can be sent to the arx data client to progress the stream.
...
impl DataStreamEngine for StateStreamEngine {
    fn create_data_client_requests(
        ...
        if let Some(number_of_states) = self.number_of_states {
            ...
            info!(
                (LogSchema::new(LogEntry::ArxDataClient)
    ...
    fn create_notification_for_subscription_data(
        ...
        let (num_versions, target_ledger_info) = match &client_response_payload {
            ...
            response_payload => {
                ...
                return Err(Error::ArxDataClientResponseIsInvalid(format!(
        ...
        if num_versions == 0 {
            // TODO(joshlind): eventually we want to notify the data client of the bad response
            return Err(Error::ArxDataClientResponseIsInvalid(
