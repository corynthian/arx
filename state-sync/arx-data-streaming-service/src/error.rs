...
pub enum Error {
    ...
    #[error("Error returned by the arx data client: {0}")]
    ArxDataClientError(String),
    #[error("The response from the arx data client is invalid! Error: {0}")]
    ArxDataClientResponseIsInvalid(String),
...
impl Error {
    ...
    pub fn get_label(&self) -> &'static str {
        match self {
            ...
            Self::ArxDataClientError(_) => "arx_data_client_error",
            Self::ArxDataClientResponseIsInvalid(_) => "arx_data_client_response_is_invalid",
...
impl From<arx_data_client::Error> for Error {
    fn from(error: arx_data_client::Error) -> Self {
        Error::ArxDataClientError(error.to_string())
    }
}
