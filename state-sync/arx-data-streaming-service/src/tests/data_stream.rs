...
use crate::{
    ...
    tests::utils::{
        ...
        initialize_logger, MockArxDataClient, NoopResponseCallback, MAX_ADVERTISED_EPOCH_END,
...
use arx_id_generator::U64IdGenerator;
use arx_infallible::Mutex;
use arx_storage_service_types::responses::CompleteDataRange;
use arx_types::{
...
async fn test_stream_data_error() {
    ...
    let pending_response = PendingClientResponse {
        client_request: client_request.clone(),
        client_response: Some(Err(arx_data_client::Error::DataIsUnavailable(
            "Missing data!".into(),
        ))),
    };
    insert_response_into_pending_queue(&mut data_stream, pending_response);
...
async fn test_continuous_stream_subscription_timeout() {
    ...
    let data_client_config = ArxDataClientConfig {
...
fn create_state_value_stream(
    ...
    version: Version,
) -> (DataStream<MockArxDataClient>, DataStreamListener) {
...
fn create_data_stream(
    ...
    // Create an arx data client mock and notification generator
    let arx_data_client = MockArxDataClient::new(data_client_config, true, false, true, false);
...
fn set_epoch_ending_response_in_queue(
    data_stream: &mut DataStream<MockArxDataClient>,
    index: usize,
    version: u64,
) {
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
...
/// Sets the client response at the index in the pending queue to contain a
/// timeout response.
fn set_timeout_response_in_queue(data_stream: &mut DataStream<MockArxDataClient>, index: usize) {
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
    let pending_response = sent_requests.as_mut().unwrap().get_mut(index).unwrap();
    let client_response = Some(Err(arx_data_client::Error::TimeoutWaitingForResponse(
        "Timed out!".into(),
    )));
    pending_response.lock().client_response = client_response;
}
...
async fn wait_for_data_client_to_respond(
    ...
    loop {
        if let Some(client_response) = &pending_response.lock().client_response {
            if !matches!(
                client_response,
                Err(arx_data_client::Error::TimeoutWaitingForResponse(_))
            ) {
                return;
            }
        }
...
fn set_transaction_response_at_queue_head(data_stream: &mut DataStream<MockArxDataClient>) {
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
