...
use arx_config::config::ArxDataClientConfig;
use arx_crypto::{ed25519::Ed25519PrivateKey, HashValue, PrivateKey, SigningKey, Uniform};
use arx_data_client::{
    AdvertisedData, ArxDataClient, GlobalDataSummary, OptimalChunkSizes, Response,
    ResponseCallback, ResponseContext, ResponseError,
};
use arx_infallible::Mutex;
use arx_logger::Level;
use arx_storage_service_types::{
    requests::{
...
/// A simple mock of the Arx Data Client
#[derive(Clone, Debug)]
pub struct MockArxDataClient {
    pub arx_data_client_config: ArxDataClientConfig,
...
impl MockArxDataClient {
    pub fn new(
        arx_data_client_config: ArxDataClientConfig,
    ...
    fn emulate_subscription_expiration(&self) -> arx_data_client::Error {
        thread::sleep(Duration::from_secs(MAX_NOTIFICATION_TIMEOUT_SECS));
        arx_data_client::Error::TimeoutWaitingForResponse("RPC timed out!".into())
    }
    ...
    fn verify_request_timeout(
        ...
        let expected_timeout = if is_subscription_request {
            self.arx_data_client_config.subscription_timeout_ms
        } else {
            let min_timeout = self.arx_data_client_config.response_timeout_ms;
            let max_timeout = self.arx_data_client_config.max_response_timeout_ms;
...
#[async_trait]
impl ArxDataClient for MockArxDataClient {
    ...
    async fn get_state_values_with_proof(
        ...
        end_index: u64,
        request_timeout_ms: u64,
    ) -> Result<Response<StateValueChunkWithProof>, arx_data_client::Error> {
    ...
    async fn get_epoch_ending_ledger_infos(
        ...
        end_epoch: Epoch,
        request_timeout_ms: u64,
    ) -> Result<Response<Vec<LedgerInfoWithSignatures>>, arx_data_client::Error> {
        self.verify_request_timeout(
            request_timeout_ms,
            false,
    ...
    > {
        ...
        if self.data_beyond_highest_advertised && known_version < MAX_REAL_TRANSACTION_OUTPUT {
            ...
            let mut arx_data_client = self.clone();
            arx_data_client.skip_timeout_verification = true;
            ...
            let outputs_with_proof = arx_data_client
                .get_transaction_outputs_with_proof(
                    known_version + 1,
                    known_version + 1,
                    known_version + 1,
                    self.arx_data_client_config.response_timeout_ms,
                )
                .await
                .unwrap()
                .payload;
    ...
    > {
        ...
        if self.data_beyond_highest_advertised && known_version < MAX_REAL_TRANSACTION {
            ...
            let transactions_with_proof = arx_data_client
    ...
    > {
        ...
        let response = if return_transactions_instead_of_outputs() {
            let (transactions, ledger_info) = arx_data_client
            ...
            let (outputs, ledger_info) = arx_data_client
    ...
    async fn get_number_of_states(
        ...
        version: Version,
        request_timeout_ms: u64,
    ) -> Result<Response<u64>, arx_data_client::Error> {
        self.verify_request_timeout(
            request_timeout_ms,
            false,
    ...
    async fn get_transaction_outputs_with_proof(
        ...
        end_version: Version,
        request_timeout_ms: u64,
    ) -> Result<Response<TransactionOutputListWithProof>, arx_data_client::Error> {
        self.verify_request_timeout(
            request_timeout_ms,
            false,
    ...
    async fn get_transactions_with_proof(
        ...
        include_events: bool,
        request_timeout_ms: u64,
    ) -> Result<Response<TransactionListWithProof>, arx_data_client::Error> {
        self.verify_request_timeout(
            request_timeout_ms,
            false,
    ...
    async fn get_transactions_or_outputs_with_proof(
        ...
        include_events: bool,
        request_timeout_ms: u64,
    ) -> arx_data_client::Result<Response<TransactionOrOutputListWithProof>> {
        self.verify_request_timeout(
            request_timeout_ms,
            false,
...
/// Initializes the Arx logger for tests
pub fn initialize_logger() {
    arx_logger::Logger::builder()
        .is_async(false)
        .level(Level::Info)
        .build();
}
