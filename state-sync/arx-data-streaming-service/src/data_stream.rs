...
use arx_config::config::{ArxDataClientConfig, DataStreamingServiceConfig};
use arx_data_client::{
    AdvertisedData, ArxDataClient, GlobalDataSummary, Response, ResponseContext, ResponseError,
    ResponsePayload,
};
use arx_id_generator::{IdGenerator, U64IdGenerator};
use arx_infallible::Mutex;
...
/// has already been sent along the stream to the client and the in-flight Arx
...
pub struct DataStream<T> {
    ...
    data_client_config: ArxDataClientConfig,
    ...
    // The data client through which to fetch data from the Arx network
    arx_data_client: T,
...
impl<T: ArxDataClient + Send + Clone + 'static> DataStream<T> {
    pub fn new(
        data_client_config: ArxDataClientConfig,
    ...
    /// Notifies the Arx data client of a bad client response
    ...
    /// Creates and sends a batch of arx data client requests to the network
            ...
            if let Some(pending_response) = self.pop_pending_response_queue()? {
                ...
                match client_response {
                    ...
                    Err(error) => {
                        ...
                        if matches!(
                            error,
                            arx_data_client::Error::TimeoutWaitingForResponse(_)
    ...
    fn handle_data_client_error(
        &mut self,
        data_client_request: &DataClientRequest,
        data_client_error: &arx_data_client::Error,
    ) -> Result<(), Error> {
...
/// can be sent to the Arx data client.
...
fn spawn_request_task<T: ArxDataClient + Send + Clone + 'static>(
    data_client_request: DataClientRequest,
    arx_data_client: T,
    ...
    tokio::spawn(async move {
        ...
        let client_response = match data_client_request {
            DataClientRequest::EpochEndingLedgerInfos(request) => {
                get_epoch_ending_ledger_infos(arx_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::NewTransactionsWithProof(request) => {
                get_new_transactions_with_proof(arx_data_client, request, request_timeout_ms)
                    .await
            },
            ...
            DataClientRequest::NumberOfStates(request) => {
                get_number_of_states(arx_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::StateValuesWithProof(request) => {
                get_states_values_with_proof(arx_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::TransactionOutputsWithProof(request) => {
                get_transaction_outputs_with_proof(arx_data_client, request, request_timeout_ms)
                    .await
            },
            DataClientRequest::TransactionsWithProof(request) => {
                get_transactions_with_proof(arx_data_client, request, request_timeout_ms).await
            },
...
async fn get_states_values_with_proof<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: StateValuesWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response = arx_data_client.get_state_values_with_proof(
        request.version,
        request.start_index,
        request.end_index,
        request_timeout_ms,
...
async fn get_epoch_ending_ledger_infos<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: EpochEndingLedgerInfosRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response = arx_data_client.get_epoch_ending_ledger_infos(
        request.start_epoch,
        request.end_epoch,
        request_timeout_ms,
    );
...
async fn get_new_transaction_outputs_with_proof<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: NewTransactionOutputsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response = arx_data_client.get_new_transaction_outputs_with_proof(
...
async fn get_new_transactions_with_proof<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: NewTransactionsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response = arx_data_client.get_new_transactions_with_proof(
        request.known_version,
        request.known_epoch,
...
async fn get_new_transactions_or_outputs_with_proof<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: NewTransactionsOrOutputsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response = arx_data_client.get_new_transactions_or_outputs_with_proof(
        request.known_version,
        request.known_epoch,
        request.include_events,
        request_timeout_ms,
...
async fn get_number_of_states<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: NumberOfStatesRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response =
        arx_data_client.get_number_of_states(request.version, request_timeout_ms);
    client_response
        .await
        .map(|response| response.map(ResponsePayload::from))
}

async fn get_transaction_outputs_with_proof<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: TransactionOutputsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response = arx_data_client.get_transaction_outputs_with_proof(
...
async fn get_transactions_with_proof<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: TransactionsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response = arx_data_client.get_transactions_with_proof(
        request.proof_version,
        request.start_version,
        request.end_version,
        request.include_events,
...
async fn get_transactions_or_outputs_with_proof<T: ArxDataClient + Send + Clone + 'static>(
    arx_data_client: T,
    request: TransactionsOrOutputsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, arx_data_client::Error> {
    let client_response = arx_data_client.get_transactions_or_outputs_with_proof(
        request.proof_version,
        request.start_version,
        request.end_version,
        request.include_events,
