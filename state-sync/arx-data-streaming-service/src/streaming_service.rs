...
use arx_data_client::{ArxDataClient, GlobalDataSummary, OptimalChunkSizes};
...
impl<T: ArxDataClient + Send + Clone + 'static> DataStreamingService<T> {
    pub fn new(
        data_client_config: ArxDataClientConfig,
        streaming_service_config: DataStreamingServiceConfig,
        arx_data_client: T,
    ...
    /// Refreshes the global data summary by communicating with the Arx data client
    ...
    fn fetch_global_data_summary(&mut self) -> Result<(), Error> {
        let global_data_summary = self.arx_data_client.get_global_data_summary();
...
fn verify_optimal_chunk_sizes(optimal_chunk_sizes: &OptimalChunkSizes) -> Result<(), Error> {
    ...
    {
        Err(Error::ArxDataClientResponseIsInvalid(format!(
