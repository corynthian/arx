...
/// An error returned by the Arx Data Client for failed API calls.
...
impl From<arx_storage_service_client::Error> for Error {
    fn from(error: arx_storage_service_client::Error) -> Self {
        Self::UnexpectedErrorEncountered(error.to_string())
    }
}

impl From<arx_storage_service_types::responses::Error> for Error {
    fn from(error: arx_storage_service_types::responses::Error) -> Self {
        Self::InvalidResponse(error.to_string())
    }
}

/// The API offered by the Arx Data Client.
#[async_trait]
pub trait ArxDataClient {
...
/// A response error that users of the Arx Data Client can use to notify
...
/// A snapshot of the global state of data available in the Arx network.
