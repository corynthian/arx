...
use crate::{
    ...
    ArxDataClient, Error, GlobalDataSummary, Response, ResponseCallback, ResponseContext,
    ResponseError, ResponseId, Result,
};
use arx_config::{
    config::{ArxDataClientConfig, BaseConfig, StorageServiceConfig},
    network_id::PeerNetworkId,
};
use arx_id_generator::{IdGenerator, U64IdGenerator};
use arx_infallible::RwLock;
use arx_logger::prelude::*;
use arx_network::{application::interface::NetworkClient, protocols::rpc::error::RpcError};
...
use arx_time_service::{TimeService, TimeServiceTrait};
use arx_types::{
...
// Useful constants for the Arx Data Client
...
/// An [`ArxDataClient`] that fulfills requests from remote peers' Storage Service
/// over ArxNet.
///
/// The `ArxNetDataClient`:
///
/// 1. Sends requests to connected Arx peers.
...
/// and/or threads.
#[derive(Clone, Debug)]
pub struct ArxNetDataClient {
    /// Config for ArxNet data client.
    data_client_config: ArxDataClientConfig,
    /// The underlying ArxNet storage service client.
    ...
    fn get_all_connected_peers(&self) -> Result<Vec<PeerNetworkId>, Error> {
        ...
        if connected_peers.is_empty() {
            return Err(Error::DataIsUnavailable(
                "No connected ArxNet peers!".to_owned(),
            ));
        }
    ...
    async fn send_request_to_peer(
        ...
        match result {
            Ok(response) => {
                ...
                // with this RPC.
                let response_callback = ArxNetResponseCallback {
            ...
            Err(error) => {
                ...
                let client_error = match error {
                    arx_storage_service_client::Error::RpcError(rpc_error) => match rpc_error {
                    ...
                    arx_storage_service_client::Error::StorageServiceError(err) => {
...
#[async_trait]
impl ArxDataClient for ArxNetDataClient {
...
/// The ArxNet-specific request context needed to update a peer's scoring.
struct ArxNetResponseCallback {
    data_client: ArxNetDataClient,
    id: ResponseId,
    peer: PeerNetworkId,
    request: StorageServiceRequest,
}

impl ResponseCallback for ArxNetResponseCallback {
...
impl fmt::Debug for ArxNetResponseCallback {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ArxNetResponseCallback")
            .field("data_client", &"..")
            .field("id", &self.id)
...
pub struct DataSummaryPoller {
    data_client: ArxNetDataClient, // The data client through which to poll peers
    ...
    pub async fn start_poller(self) {
        info!(
            (LogSchema::new(LogEntry::DataSummaryPoller)
                .message("Starting the Arx data poller!"))
        );
