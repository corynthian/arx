...
use super::{ArxDataClient, ArxNetDataClient, DataSummaryPoller, Error};
use crate::arxnet::{poll_peer, state::calculate_optimal_chunk_sizes};
use arx_channel::{arx_channel, message_queues::QueueStyle};
use arx_config::{
    config::{ArxDataClientConfig, BaseConfig, RoleType, StorageServiceConfig},
...
use arx_storage_service_server::network::{NetworkRequest, ResponseSender};
...
use arx_time_service::{MockTimeService, TimeService};
use arx_types::{
...
struct MockNetwork {
    ...
    peer_mgr_reqs_rx: arx_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
...
impl MockNetwork {
    fn new(
        ...
        data_client_config: Option<ArxDataClientConfig>,
        networks: Option<Vec<NetworkId>>,
    ) -> (Self, MockTimeService, ArxNetDataClient, DataSummaryPoller) {
        // Setup the request managers
        let queue_cfg = arx_channel::Config::new(10).queue_style(QueueStyle::FIFO);
        ...
        let (client, poller) = ArxNetDataClient::new(
            data_client_config,
            base_config,
...
async fn request_works_only_when_data_available() {
    ::arx_logger::Logger::init_for_testing();
...
fn get_num_in_flight_polls(client: ArxNetDataClient, is_priority_peer: bool) -> u64 {
    if is_priority_peer {
