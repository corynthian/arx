...
use arx_storage_interface::{state_view::DbStateViewAtVersion, DbReaderWriter};
use arx_types::{
...
impl EventSubscriptionService {
    ...
    pub fn subscribe_to_events(
        ...
        let (notification_sender, notification_receiver) =
            channel::new(QueueStyle::KLAST, EVENT_NOTIFICATION_CHANNEL_SIZE, None);
    ...
    pub fn subscribe_to_reconfigurations(&mut self) -> Result<ReconfigNotificationListener, Error> {
        let (notification_sender, notification_receiver) =
            channel::new(QueueStyle::KLAST, RECONFIG_NOTIFICATION_CHANNEL_SIZE, None);
    ...
    fn read_on_chain_configs(&self, version: Version) -> Result<OnChainConfigPayload, Error> {
        ...
        let arx_framework_account_view =
            db_state_view.as_account_with_state_view(&CORE_CODE_ADDRESS);

        let epoch = arx_framework_account_view
...
struct EventSubscription {
    ...
    pub notification_sender: channel::Sender<(), EventNotification>,
}
...
struct ReconfigSubscription {
    pub notification_sender: channel::Sender<(), ReconfigNotification>,
}
...
pub struct NotificationListener<T> {
    pub notification_receiver: channel::Receiver<(), T>,
}
