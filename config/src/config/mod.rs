...
use arx_types::{waypoint::Waypoint, PeerId};
...
mod api_config;
pub use api_config::*;
use arx_crypto::{bls12381, ed25519::Ed25519PrivateKey, x25519};
...
impl Default for BaseConfig {
    fn default() -> BaseConfig {
        BaseConfig {
            data_dir: PathBuf::from("/opt/arx/data"),
            working_dir: None,
        ...
        let waypoint = match &self {
            ...
            WaypointConfig::FromStorage(backend) => {
                ...
                let waypoint = storage
                    .get::<Waypoint>(arx_protocol_constants::WAYPOINT)
    ...
    pub fn genesis_waypoint(&self) -> Waypoint {
        match &self {
            WaypointConfig::FromStorage(backend) => {
                let storage: Storage = backend.into();
                storage
                    .get::<Waypoint>(arx_protocol_constants::GENESIS_WAYPOINT)
                    .expect("Unable to read waypoint")
                    .value
            },
            _ => self.waypoint(),
