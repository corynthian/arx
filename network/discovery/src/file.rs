...
use arx_config::config::PeerSet;
#[cfg(test)]
use arx_logger::spawn_named;
use arx_time_service::{Interval, TimeService, TimeServiceTrait};
use futures::Stream;
use std::{
    path::{Path, PathBuf},
    pin::Pin,
...
mod tests {
    ...
    use arx_channels::Receiver;
    use arx_config::{
        config::{Peer, PeerRole},
        network_id::NetworkContext,
    };
    use arx_network::connectivity_manager::{ConnectivityRequest, DiscoverySource};
    ...
    fn create_listener(path: Arc<TempPath>) -> Receiver<ConnectivityRequest> {
        ...
        let (conn_mgr_reqs_tx, conn_mgr_reqs_rx) = arx_channels::new(
            1,
            &arx_network::counters::PENDING_CONNECTIVITY_MANAGER_REQUESTS,
        );
