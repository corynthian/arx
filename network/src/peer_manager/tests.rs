...
fn build_test_peer_manager(
    ...
    PeerManager<
        ...
        MemorySocket,
    >,
    arx_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    arx_channel::Sender<PeerId, ConnectionRequest>,
    arx_channel::Receiver<(PeerId, ProtocolId), PeerManagerNotification>,
    conn_notifs_channel::Receiver,
) {
    let (peer_manager_request_tx, peer_manager_request_rx) =
        arx_channel::new(QueueStyle::FIFO, 1, None);
    let (connection_reqs_tx, connection_reqs_rx) = arx_channel::new(QueueStyle::FIFO, 1, None);
    let (hello_tx, hello_rx) = arx_channel::new(QueueStyle::FIFO, 1, None);
