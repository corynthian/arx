...
use crate::{
    ...
    transport::{self, ArxNetTransport, Connection, ARX_TCP_TRANSPORT},
    ProtocolId,
};
...
use arx_crypto::x25519;
use arx_infallible::RwLock;
use arx_logger::prelude::*;
#[cfg(any(test, feature = "testing", feature = "fuzzing"))]
use arx_netcore::transport::memory::MemoryTransport;
use arx_netcore::transport::{
    tcp::{TCPBufferCfg, TcpSocket, TcpTransport},
    Transport,
};
use arx_rate_limiter::rate_limit::TokenBucketRateLimiter;
use arx_time_service::TimeService;
use arx_types::{chain_id::ChainId, network_address::NetworkAddress, PeerId};
...
struct PeerManagerContext {
    ...
    pm_reqs_tx: channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    pm_reqs_rx: channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    connection_reqs_tx: channel::Sender<PeerId, ConnectionRequest>,
    connection_reqs_rx: channel::Receiver<PeerId, ConnectionRequest>,

    peers_and_metadata: Arc<PeersAndMetadata>,
    trusted_peers: Arc<RwLock<PeerSet>>,
    upstream_handlers:
        HashMap<ProtocolId, channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>>,
...
impl PeerManagerContext {
    ...
    fn new(
        ...
        upstream_handlers: HashMap<
            ProtocolId,
            channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>,
        >,
        connection_event_handlers: Vec<conn_notifs_channel::Sender>,
    ...
    fn add_upstream_handler(
        ...
        channel: channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>,
    ) -> &mut Self {
...
type MemoryPeerManager =
    PeerManager<ArxNetTransport<MemoryTransport>, NoiseStream<arx_memsocket::MemorySocket>>;
type TcpPeerManager = PeerManager<ArxNetTransport<TcpTransport>, NoiseStream<TcpSocket>>;
...
impl PeerManagerBuilder {
    ...
    pub fn create(
        ...
        let (pm_reqs_tx, pm_reqs_rx) = channel::new(
            QueueStyle::FIFO,
            channel_size,
        ...
        let (connection_reqs_tx, connection_reqs_rx) =
            channel::new(QueueStyle::FIFO, channel_size, None);
    ...
    pub fn connection_reqs_tx(&self) -> channel::Sender<PeerId, ConnectionRequest> {
    ...
    pub fn build(&mut self, executor: &Handle) -> &mut Self {
        use arx_types::network_address::Protocol::*;
        ...
        let mut arx_tcp_transport = ARX_TCP_TRANSPORT.clone();
        let tcp_cfg = self.get_tcp_buffers_cfg();
        arx_tcp_transport.set_tcp_buffers(&tcp_cfg);
    ...
    pub fn add_service(
        ...
        channel::Receiver<(PeerId, ProtocolId), PeerManagerNotification>,
