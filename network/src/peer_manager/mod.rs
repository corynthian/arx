...
use arx_config::network_id::{NetworkContext, PeerNetworkId};
use arx_logger::prelude::*;
use arx_netcore::transport::{ConnectionOrigin, Transport};
...
use arx_types::{network_address::NetworkAddress, PeerId};
use futures::{
    channel::oneshot,
...
use arx_config::config::{PeerRole, PeerSet};
    ...
    TSocket: AsyncRead + AsyncWrite,
{
    ...
    active_peers: HashMap<
        ...
        (
            ConnectionMetadata,
            channel::Sender<ProtocolId, PeerRequest>,
        ),
    >,
    ...
    requests_rx: channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    ...
    transport_reqs_tx: arx_channel::Sender<TransportRequest>,
    /// Sender for connection events.
    transport_notifs_tx: arx_channel::Sender<TransportNotification<TSocket>>,
    ...
    transport_notifs_rx: arx_channel::Receiver<TransportNotification<TSocket>>,
    ...
    TSocket: TransportTSocket,
{
    ...
    pub fn new(
        ...
        let (transport_notifs_tx, transport_notifs_rx) = arx_channel::new(
            channel_size,
            &counters::PENDING_CONNECTION_HANDLER_NOTIFICATIONS,
        );
        let (transport_reqs_tx, transport_reqs_rx) =
            arx_channel::new(channel_size, &counters::PENDING_PEER_MANAGER_DIAL_REQUESTS);
    ...
    fn handle_connection_event(&mut self, event: TransportNotification<TSocket>) {
		...
		// Disconnected events should be handled correctly.
                info!(
                    NetworkSchema::new(&self.network_context)
    ...
    fn spawn_peer_network_events_handler(
        &self,
        peer_id: PeerId,
        network_events: channel::Receiver<ProtocolId, PeerNotification>,
    ) {
...
fn handle_inbound_request(
    ...
    if let Some(handler) = upstream_handlers.get_mut(&protocol_id) {
        // Send over arx channel for fairness.
