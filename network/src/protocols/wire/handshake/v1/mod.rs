...
//! These should serialize as per the [ArxNet Handshake v1 Specification].
//!
...
//!
//! [ArxNet Handshake v1 Specification]:
//! https://github.com/diem/diem/blob/main/specifications/network/handshake-v1.md

use anyhow::anyhow;
use arx_compression::metrics::CompressionClient;
use arx_config::{config::MAX_APPLICATION_MESSAGE_SIZE, network_id::NetworkId};
...
impl ProtocolId {
    ...
    pub fn from_bytes<T: DeserializeOwned>(&self, bytes: &[u8]) -> anyhow::Result<T> {
        match self.encoding() {
            ...
            Encoding::CompressedBcs(limit) => {
                let compression_client = self.get_compression_client();
                let raw_bytes = arx_compression::decompress(
                    &bytes.to_vec(),
                    compression_client,
                    MAX_APPLICATION_MESSAGE_SIZE,
                )
    ...
    // ARX-TODO: use limit
    fn bcs_encode<T: Serialize>(&self, value: &T, _limit: usize) -> anyhow::Result<Vec<u8>> {
	
        bcs::to_bytes(value).map_err(|e| anyhow!("{:?}", e))
    }

    // ARX-TODO: use limit
    fn bcs_decode<T: DeserializeOwned>(&self, bytes: &[u8], _limit: usize) -> anyhow::Result<T> {
...
/// ArxNet peers in order to negotiate the set of common supported protocols for
/// use on a new ArxNet connection.
#[derive(Clone, Debug, Default, Deserialize, Eq, PartialEq, Serialize)]
#[cfg_attr(any(test, feature = "fuzzing"), derive(Arbitrary))]
pub struct ProtocolIdSet(arx_bitvec::BitVec);
...
/// Enum representing different versions of the Arx network protocol. These
...
pub enum HandshakeError {
    #[error("arx-handshake: the received message has a different chain id: {0}, expected: {1}")]
    InvalidChainId(ChainId, ChainId),
    #[error(
        "arx-handshake: the received message has an different network id: {0}, expected: {1}"
    )]
    InvalidNetworkId(NetworkId, NetworkId),
    #[error("arx-handshake: could not find an intersection of supported protocol with the peer")]
    NoCommonProtocols,
}
