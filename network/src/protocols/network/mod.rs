...
//! Convenience Network API for Arx
...
/// Configuration needed for the client side of ArxNet applications
#[derive(Clone)]
...
/// Configuration needed for the service side of ArxNet applications
#[derive(Clone)]
pub struct NetworkServiceConfig {
    ...
    pub inbound_queue_config: channel::Config,
}
...
impl NetworkServiceConfig {
    pub fn new(
        direct_send_protocols_and_preferences: Vec<ProtocolId>,
        rpc_protocols_and_preferences: Vec<ProtocolId>,
        inbound_queue_config: channel::Config,
    ) -> Self {
        Self {
            direct_send_protocols_and_preferences,
            rpc_protocols_and_preferences,
...
/// Configuration needed for ArxNet applications to register with the network
...
pub struct NetworkEvents<TMessage> {
    ...
    event_stream: Select<
        ...
        >,
        Map<
            channel::Receiver<PeerId, ConnectionNotification>,
...
pub trait NewNetworkEvents {
    fn new(
        peer_mgr_notifs_rx: channel::Receiver<(PeerId, ProtocolId), PeerManagerNotification>,
        connection_notifs_rx: channel::Receiver<PeerId, ConnectionNotification>,
    ) -> Self;
}
...
/// a thin wrapper on `channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>`,
