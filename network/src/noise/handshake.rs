...
use arx_crypto::{noise, x25519};
use arx_infallible::{duration_since_epoch, RwLock};
use arx_logger::trace;
use arx_netcore::transport::ConnectionOrigin;
use arx_short_hex_str::{AsShortHexStr, ShortHexStr};
...
pub enum HandshakeAuthMode {
    ...
    ///
    /// For example, in the Arx validator network, validator peers will only
                ...
                match trusted_peers.read().get(&remote_peer_id) {
                    ...
                    None => {
                        ...
                        let derived_remote_peer_id =
                            arx_types::account_address::from_identity_public_key(
                                remote_public_key,
                            );
...
mod test {
    ...
    use arx_config::config::{Peer, PeerRole};
    use arx_crypto::{test_utils::TEST_SEED, traits::Uniform as _};
    ...
    fn build_peers(
        ...
        let (client_auth, server_auth, client_peer_id, server_peer_id) = if is_mutual_auth {
            ...
            let client_peer_id =
                arx_types::account_address::from_identity_public_key(client_public_key);
            let server_peer_id =
                arx_types::account_address::from_identity_public_key(server_public_key);
            (
