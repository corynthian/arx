...
use arx_channels::{arx_channel, message_queues::QueueStyle};
...
/// Returns an arx channel for testing
fn create_arx_channel<K: Eq + Hash + Clone, T>(
) -> (arx_channel::Sender<K, T>, arx_channel::Receiver<K, T>) {
    arx_channel::new(QueueStyle::FIFO, 10, None)
}
...
fn create_network_sender_and_events(
    ...
    HashMap<NetworkId, arx_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>,
    HashMap<NetworkId, arx_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>>,
) {
    ...
    for network_id in network_ids {
        ...
        let (inbound_request_sender, inbound_request_receiver) = create_arx_channel();
        let (outbound_request_sender, outbound_request_receiver) = create_arx_channel();
        let (connection_outbound_sender, _connection_outbound_receiver) = create_arx_channel();
        let (_connection_inbound_sender, connection_inbound_receiver) = create_arx_channel();
...
async fn wait_for_network_event(
    ...
    outbound_request_receivers: &mut HashMap<
        NetworkId,
        arx_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    >,
