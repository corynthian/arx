...
use arx_proptest_helpers::ValueGenerator;
...
pub fn fuzz(data: &[u8]) {
    ...
    let (connection_notifs_tx, connection_notifs_rx) = arx_channels::new_test(8);
    let channel_size = 8;

    let (peer_reqs_tx, peer_reqs_rx) = arx_channel::new(QueueStyle::FIFO, channel_size, None);
    let (peer_notifs_tx, peer_notifs_rx) = arx_channel::new(QueueStyle::FIFO, channel_size, None);
