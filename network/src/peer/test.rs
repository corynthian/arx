...
use arx_config::{config::PeerRole, network_id::NetworkContext};
use arx_memsocket::MemorySocket;
...
fn build_test_peer(
    ...
    PeerHandle,
    MemorySocket,
    arx_channels::Receiver<TransportNotification<MemorySocket>>,
    arx_channel::Receiver<ProtocolId, PeerNotification>,
) {
    ...
    let (connection_notifs_tx, connection_notifs_rx) = arx_channels::new_test(1);
    let (peer_reqs_tx, peer_reqs_rx) =
        arx_channel::new(QueueStyle::FIFO, NETWORK_CHANNEL_SIZE, None);
...
async fn assert_disconnected_event(
    ...
    connection_notifs_rx: &mut arx_channels::Receiver<TransportNotification<MemorySocket>>,
) {
...
#[derive(Clone)]
struct PeerHandle(arx_channel::Sender<ProtocolId, PeerRequest>);
