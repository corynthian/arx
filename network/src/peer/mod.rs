...
use arx_channel::channel;
use arx_config::network_id::NetworkContext;
use arx_logger::prelude::*;
use arx_rate_limiter::rate_limit::SharedBucket;
...
pub struct Peer<TSocket> {
    ...
    connection_notifs_tx: arx_channel::Sender<TransportNotification<TSocket>>,
    /// Channel to receive requests from PeerManager to send messages and rpcs.
    peer_reqs_rx: channel::Receiver<ProtocolId, PeerRequest>,
    /// Channel to notifty PeerManager of new inbound messages and rpcs.
    peer_notifs_tx: channel::Sender<ProtocolId, PeerNotification>,
    ...
    TSocket: AsyncRead + AsyncWrite + Send + 'static,
{
    ...
    fn start_writer_task(
        ...
        max_frame_size: usize,
        max_message_size: usize,
    ) -> (arx_channel::Sender<NetworkMessage>, oneshot::Sender<()>) {
        let remote_peer_id = connection_metadata.remote_peer_id;
        let (write_reqs_tx, mut write_reqs_rx): (arx_channel::Sender<NetworkMessage>, _) =
            arx_channel::new(1024, &counters::PENDING_WIRE_MESSAGES);
        let (close_tx, mut close_rx) = oneshot::channel();

        let (mut msg_tx, msg_rx) = arx_channel::new(1024, &counters::PENDING_MULTIPLEX_MESSAGE);
        let (stream_msg_tx, stream_msg_rx) =
            arx_channel::new(1024, &counters::PENDING_MULTIPLEX_STREAM);
    ...
    async fn handle_inbound_message(
        ...
        write_reqs_tx: &mut arx_channel::Sender<NetworkMessage>,
    ) -> Result<(), PeerManagerError> {
        trace!(
            NetworkSchema::new(&self.network_context)
                .connection_metadata(&self.connection_metadata),
