...
use arx_id_generator::{IdGenerator, U32IdGenerator};
use arx_logger::prelude::*;
// Re-exposed for arx-network-checker
pub use arx_netcore::transport::tcp::{resolve_and_connect, TCPBufferCfg, TcpSocket};
use arx_netcore::transport::{proxy_protocol, tcp, ConnectionOrigin, Transport};
use arx_short_hex_str::AsShortHexStr;
use arx_time_service::{timeout, TimeService, TimeServiceTrait};
use arx_types::{
    chain_id::ChainId,
...
/// tcp::Transport with Arx-specific configuration applied.
pub const ARX_TCP_TRANSPORT: tcp::TcpTransport = tcp::TcpTransport {
    // Use default options.
    ttl: None,
    // Use TCP_NODELAY for Arx tcp connections.
    nodelay: Some(true),
...
async fn upgrade_inbound<T: TSocket>(
    ...
    // try to negotiate common arxnet version and supported application protocols
...
// TODO: rework Transport trait, possibly include Upgrade trait.
pub struct ArxNetTransport<TTransport> {
...
impl<TTransport> ArxNetTransport<TTransport>
where
    ...
    TTransport::Listener: Send + 'static,
{
    ...
    fn parse_dial_addr(
        ...
        // parse out the arxnet protocols (noise ik and handshake)
...
// If using `ArxNetTransport` as a `Transport` trait, then all upgrade futures
...
impl<TTransport: Transport> Transport for ArxNetTransport<TTransport>
where
