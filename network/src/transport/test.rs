...
fn setup<TTransport>(
    ...
    Runtime,
    MockTimeService,
    (PeerId, ArxNetTransport<TTransport>),
    (PeerId, ArxNetTransport<TTransport>),
    Arc<RwLock<PeerSet>>,
    ProtocolIdSet,
)
    ...
    TTransport::Listener: Send + 'static,
{
    ...
    let (listener_peer_id, dialer_peer_id, listener_auth_mode, dialer_auth_mode, trusted_peers) =
        match auth {
            ...
            Auth::MaybeMutual => {
                let listener_peer_id = arx_types::account_address::from_identity_public_key(
                    listener_key.public_key(),
                );
                let dialer_peer_id =
                    arx_types::account_address::from_identity_public_key(dialer_key.public_key());
                let trusted_peers = build_trusted_peers(
                    dialer_peer_id,
                    &dialer_key,
                    PeerRole::Validator,
    ...
    let listener_transport = ArxNetTransport::new(
    ...
    let dialer_transport = ArxNetTransport::new(
        base_transport,
...
////////////////////////////////////////
// ArxNetTransport<MemoryTransport> //
////////////////////////////////////////
...
/////////////////////////////////////
// ArxNetTransport<TcpTransport> //
/////////////////////////////////////

#[test]
fn test_tcp_transport_mutual_auth() {
    test_transport_success(
        ARX_TCP_TRANSPORT.clone(),
        Auth::Mutual,
        "/ip4/127.0.0.1/tcp/0",
