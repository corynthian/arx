...
use arx_config::config::{Peer, PeerRole, PeerSet, HANDSHAKE_VERSION};
use arx_crypto::{test_utils::TEST_SEED, x25519, Uniform};
use arx_logger::info;
use arx_time_service::{MockTimeService, TimeService};
use arx_types::{account_address::AccountAddress, network_address::NetworkAddress};
...
struct TestHarness {
    ...
    connection_reqs_rx: arx_channel::Receiver<PeerId, ConnectionRequest>,
    connection_notifs_tx: conn_notifs_channel::Sender,
    conn_mgr_reqs_tx: arx_channels::Sender<ConnectivityRequest>,
}
...
impl TestHarness {
    fn new(seeds: PeerSet) -> (Self, ConnectivityManager<FixedInterval>) {
        ...
        let (conn_mgr_reqs_tx, conn_mgr_reqs_rx) = arx_channels::new_test(0);
