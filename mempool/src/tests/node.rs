...
use arx_crypto::{x25519::PrivateKey, Uniform};
use arx_event_notifications::{ReconfigNotification, ReconfigNotificationListener};
use arx_infallible::{Mutex, MutexGuard, RwLock};
...
pub struct NodeNetworkInterface {
    ...
    pub(crate) network_reqs_rx: arx_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
...
fn setup_node_network_interface(
    ...
    let (connection_reqs_tx, _) = arx_channel::new(QueueStyle::FIFO, MAX_QUEUE_SIZE, None);
