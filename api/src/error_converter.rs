...
use arx_api_types::{mime_types::JSON, ArxError, ArxErrorCode};
use poem::{
...
/// be a JSON representation of ArxError. For our own code, this is exactly
...
fn build_error_response(error_string: String) -> Response {
    Json(ArxError::new_with_error_code(
        error_string,
        ArxErrorCode::WebFrameworkError,
    ))
    .into_response()
}
