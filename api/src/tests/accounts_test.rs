...
use arx_api_test_context::{current_function_name, find_value};
use arx_api_types::{MoveModuleBytecode, MoveResource, StateKeyWrapper};
use serde_json::json;
use std::str::FromStr;
...
#[ignore]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_get_module_arx_config() {
    let mut context = new_test_context(current_function_name!());
    let address = "0x1";
...
async fn test_get_account_resources_with_pagination() {
    ...
    assert!(!resp.headers().contains_key("X-Arx-Cursor"));
