...
use arx_api_test_context::{current_function_name, TestContext};
use arx_cached_packages::arx_stdlib;
use arx_framework::BuiltPackage;
use arx_sdk::types::LocalAccount;
...
async fn test_read_resoure_group() {
    ...
    let path = PathBuf::from(std::env!("CARGO_MANIFEST_DIR"))
        .join("../arx-move/move-examples/resource_groups/primary");
    publish_package(&mut context, &mut admin0, path, named_addresses.clone()).await;

    let path = PathBuf::from(std::env!("CARGO_MANIFEST_DIR"))
        .join("../arx-move/move-examples/resource_groups/secondary");
...
async fn publish_package(
    ...
    let mut build_options = arx_framework::BuildOptions::default();
    ...
    let payload = arx_stdlib::code_publish_package_txn(bcs::to_bytes(&metadata).unwrap(), code);
    let txn =
