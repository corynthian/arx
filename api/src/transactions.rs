...
use arx_api_types::{
    verify_function_identifier, verify_module_identifier, Address, ArxError, ArxErrorCode,
...
use arx_crypto::{hash::CryptoHash, signing_message};
use arx_types::{
...
pub enum SubmitTransactionPost {
    ...
    // TODO: https://github.com/arx-labs/arx-core/issues/2275
    #[oai(content_type = "application/x.arx.signed_transaction+bcs")]
    Bcs(Bcs),
}
...
impl TransactionsApi {
    ...
    ///   1. Hash message bytes: "RawTransaction" bytes + BCS bytes of [Transaction](https://arx-labs.github.io/arx-core/arx_types/transaction/enum.Transaction.html).
    ...
    /// Make sure to use the `application/x.arx.signed_transaction+bcs` Content-Type.
    ...
    async fn encode_submission(
        ...
        data.0
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code_no_info(err, ArxErrorCode::InvalidInput)
            })?;
...
impl TransactionsApi {
    ...
    async fn create_internal(&self, txn: SignedTransaction) -> Result<(), ArxError> {
        ...
        match mempool_status.code {
            ...
            MempoolStatusCode::InvalidSeqNumber => Err(ArxError::new_with_error_code(
                mempool_status.message,
                ArxErrorCode::SequenceNumberTooOld,
            )),
            MempoolStatusCode::InvalidUpdate => Err(ArxError::new_with_error_code(
                mempool_status.message,
                ArxErrorCode::InvalidTransactionUpdate,
            )),
            MempoolStatusCode::UnknownStatus => Err(ArxError::new_with_error_code(
    ...
    async fn create(
        ...
        match self.create_internal(txn.clone()).await {
            ...
            Err(error) => match error.error_code {
                ArxErrorCode::InternalError => Err(
                    SubmitTransactionError::internal_from_arx_error(error, ledger_info),
                ),
                ArxErrorCode::VmError
                | ArxErrorCode::SequenceNumberTooOld
                | ArxErrorCode::InvalidTransactionUpdate => Err(
                    SubmitTransactionError::bad_request_from_arx_error(error, ledger_info),
                ),
                ArxErrorCode::MempoolIsFull => Err(
                    SubmitTransactionError::insufficient_storage_from_arx_error(
                        error,
                        ledger_info,
                    ),
                ),
                _ => Err(SubmitTransactionError::internal_from_arx_error(
                    error,
                    ledger_info,
                )),
            },
    ...
    // TODO: This function leverages a lot of types from arx_types, use the
    ...
    pub async fn simulate(
        ...
        let (_, output_ext) = ArxVM::simulate_signed_transaction(&txn, &move_resolver);
        ...
        let txn = arx_types::transaction::Transaction::UserTransaction(txn);
        let zero_hash = arx_crypto::HashValue::zero();
        let info = arx_types::transaction::TransactionInfo::new(
            txn.hash(),
            zero_hash,
            zero_hash,
            None,
    ...
    pub fn get_signing_message(
        ...
        let raw_txn: RawTransaction = resolver
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code(err, ArxErrorCode::InvalidInput, &ledger_info)
            })?;
