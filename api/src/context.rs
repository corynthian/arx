...
use arx_api_types::{
    ArxErrorCode, AsConverter, BcsBlock, GasEstimation, LedgerInfo, TransactionOnChainData,
};
use arx_config::config::{NodeConfig, RoleType};
use arx_crypto::HashValue;
use arx_gas::{ArxGasParameters, FromOnChainGasSchedule};
use arx_logger::error;
use arx_mempool::{MempoolClientRequest, MempoolClientSender, SubmissionStatus};
...
impl Context {
    ...
    pub fn move_resolver_poem<E: InternalError>(
        ...
        self.move_resolver()
            ...
            .map_err(|e| E::internal_with_code(e, ArxErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn get_latest_ledger_info<E: ServiceUnavailableError>(&self) -> Result<LedgerInfo, E> {
        let maybe_oldest_version = self
            ...
            .map_err(|e| {
                E::service_unavailable_with_code_no_info(e, ArxErrorCode::InternalError)
            })?;
    ...
    pub fn get_account_state<E: InternalError>(
        ...
        AccountState::from_access_paths_and_values(
            ...
            &self.get_state_values(address, version).map_err(|err| {
                E::internal_with_code(err, ArxErrorCode::InternalError, latest_ledger_info)
            })?,
        )
    ...
    pub fn get_block_timestamp<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|err| E::internal_with_code(err, ArxErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn render_transactions_sequential<E: InternalError>(
        ...
        mut timestamp: u64,
    ) -> Result<Vec<arx_api_types::Transaction>, E> {
        if data.is_empty() {
            return Ok(vec![]);
        }
        ...
        let txns: Vec<arx_api_types::Transaction> = data
            .into_iter()
            .map(|t| {
            ...
            .map_err(|err| {
                E::internal_with_code(err, ArxErrorCode::InternalError, ledger_info)
            })?;
    ...
    pub fn get_gas_schedule<E: InternalError>(
        ...
        // Otherwise refresh the cache
        {
            ...
            let state_view = self
                ...
                .map_err(|e| {
                    E::internal_with_code(e, ArxErrorCode::InternalError, ledger_info)
                })?;
            ...
            let gas_schedule_params =
                match GasScheduleV2::fetch_config(&storage_adapter).and_then(|gas_schedule| {
                    ...
                    ArxGasParameters::from_on_chain_gas_schedule(&gas_schedule, feature_version)
                }) {
                    Some(gas_schedule) => Ok(gas_schedule),
                    None => GasSchedule::fetch_config(&storage_adapter)
                        .and_then(|gas_schedule| {
                            let gas_schedule = gas_schedule.to_btree_map();
                            ArxGasParameters::from_on_chain_gas_schedule(&gas_schedule, 0)
                        })
                        .ok_or_else(|| {
                            E::internal_with_code(
...
pub struct GasScheduleCache {
    last_updated_epoch: Option<u64>,
    gas_schedule_params: Option<ArxGasParameters>,
}
