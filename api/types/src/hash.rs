...
pub struct HashValue(pub arx_crypto::hash::HashValue);

impl From<arx_crypto::hash::HashValue> for HashValue {
    fn from(val: arx_crypto::hash::HashValue) -> Self {
        Self(val)
    }
}

impl From<HashValue> for arx_crypto::hash::HashValue {
...
impl FromStr for HashValue {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
        if let Some(hex) = s.strip_prefix("0x") {
            Ok(hex.parse::<arx_crypto::hash::HashValue>()?.into())
        } else {
            Ok(s.parse::<arx_crypto::hash::HashValue>()?.into())
        }
    }
}
...
impl HashValue {
    pub fn zero() -> Self {
        Self(arx_crypto::hash::HashValue::zero())
    }
}
