...
use arx_vm::move_vm_ext::MoveResolverExt;
...
impl<'a, R: MoveResolverExt + ?Sized> MoveConverter<'a, R> {
    ...
    pub fn into_transaction_info(
        &self,
        version: u64,
        info: &arx_types::transaction::TransactionInfo,
        accumulator_root_hash: HashValue,
        write_set: arx_types::write_set::WriteSet,
    ) -> TransactionInfo {
        TransactionInfo {
    ...
    pub fn try_into_transaction_payload(
        &self,
        payload: arx_types::transaction::TransactionPayload,
    ) -> Result<TransactionPayload> {
        use arx_types::transaction::TransactionPayload::*;
    ...
    pub fn try_into_write_set_payload(
        &self,
        payload: arx_types::transaction::WriteSetPayload,
    ) -> Result<WriteSetPayload> {
        use arx_types::transaction::WriteSetPayload::*;
        let ret = match payload {
    ...
    pub fn try_into_raw_transaction(
        ...
        Ok(RawTransaction::new(
            ...
            self.try_into_arx_core_transaction_payload(payload)?,
            max_gas_amount.into(),
            gas_unit_price.into(),
    ...
    pub fn try_into_arx_core_transaction_payload(
        &self,
        payload: TransactionPayload,
    ) -> Result<arx_types::transaction::TransactionPayload> {
        use arx_types::transaction::TransactionPayload as Target;
