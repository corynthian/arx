...
use arx_types::vm_status::StatusCode;
...
fn test_serialize_deserialize() {
    let with_code = ArxError::new_with_vm_status(
        "Invalid transaction",
        ArxErrorCode::VmError,
        arx_types::vm_status::StatusCode::UNKNOWN_MODULE,
    );
    let _: ArxError = bcs::from_bytes(&bcs::to_bytes(&with_code).unwrap()).unwrap();
    let _: ArxError = serde_json::from_str(&serde_json::to_string(&with_code).unwrap()).unwrap();

    let without_code =
        ArxError::new_with_error_code("some message", ArxErrorCode::MempoolIsFull);
    let _: ArxError = bcs::from_bytes(&bcs::to_bytes(&without_code).unwrap()).unwrap();
    let _: ArxError =
