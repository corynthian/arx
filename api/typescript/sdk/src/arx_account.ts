...
import { AccountAddress, AuthenticationKey, Ed25519PublicKey } from "./arx_types";
import { bcsToBytes } from "./bcs";

export interface ArxAccountObject {
...
/**
 * Class for creating and managing Arx account
 */
export class ArxAccount {
  /**
  ...
  static fromArxAccountObject(obj: ArxAccountObject): ArxAccount {
    return new ArxAccount(HexString.ensure(obj.privateKeyHex).toUint8Array(), obj.address);
  }
   ...
   * @returns ArxAccount
   */
  static fromDerivePath(path: string, mnemonics: string): ArxAccount {
    if (!ArxAccount.isValidPath(path)) {
  ...
  /**
   * This is the key by which Arx account is referenced.
   ...
   * See here for more info: {@link https://arx.dev/concepts/accounts#single-signer-authentication}
   ...
   * @returns ArxAccountObject instance.
   * @example An example of the returned ArxAccountObject object
   * ```
   * {
   ...
   * ```
   */
  toPrivateKeyObject(): ArxAccountObject {
    return {
...
// Returns an account address as a HexString given either an ArxAccount or a MaybeHexString.
export function getAddressFromAccountOrAddress(accountOrAddress: ArxAccount | MaybeHexString): HexString {
  return accountOrAddress instanceof ArxAccount ? accountOrAddress.address() : HexString.ensure(accountOrAddress);
}
