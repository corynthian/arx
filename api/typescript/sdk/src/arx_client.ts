...
import { ArxAccount } from "./arx_account";
...
import { Ed25519PublicKey, MultiEd25519PublicKey } from "./arx_types";
...
/**
 * Provides methods for retrieving data from Arx node.
 * For more detailed API specification see {@link https://fullnode.devnet.arxlabs.com/v1/spec}
 */
export class ArxClient {
  client: Gen.ArxGeneratedClient;

  readonly nodeUrl: string;

  /**
   * Build a client configured to connect to an Arx node at the given URL.
   *
   ...
   *
   * @param nodeUrl URL of the Arx Node API endpoint.
  ...
  constructor(nodeUrl: string, config?: Partial<Gen.OpenAPIConfig>, doNotFixNodeUrl: boolean = false) {
    ...
    this.client = new Gen.ArxGeneratedClient(conf);
  }

  /**
   * Queries an Arx account by address
   * @param accountAddress Hex-encoded 32 byte Arx account address
  ...
  static generateBCSTransaction(accountFrom: ArxAccount, rawTxn: TxnBuilderTypes.RawTransaction): Uint8Array {
   ...
   */
  static generateBCSSimulation(accountFrom: ArxAccount, rawTxn: TxnBuilderTypes.RawTransaction): Uint8Array {
   ...
   * @param sender Hex-encoded 32 byte Arx account address of transaction sender
   ...
   * @param accountFrom ArxAccount of transaction sender
  ...
  async signTransaction(
    accountFrom: ArxAccount,
    rawTransaction: TxnBuilderTypes.RawTransaction,
  ): Promise<Uint8Array> {
    return Promise.resolve(ArxClient.generateBCSTransaction(accountFrom, rawTransaction));
  }
   ...
   * that event type.
   * @param address Hex-encoded 32 byte Arx account, with or without a `0x` prefix,
   ...
   * (e.g. `0x1::coin::CoinStore<0x1::arx_coin::ArxCoin>`)
   ...
   *
   * @param accountOrPubkey The sender or sender's public key. When private key is available, `ArxAccount` instance
  ...
  async simulateTransaction(
    accountOrPubkey: ArxAccount | Ed25519PublicKey | MultiEd25519PublicKey,
    rawTransaction: TxnBuilderTypes.RawTransaction,
    query?: {
    ...
    if (accountOrPubkey instanceof ArxAccount) {
      signedTxn = ArxClient.generateBCSSimulation(accountOrPubkey, rawTransaction);
  ...
  async submitSignedBCSTransaction(signedTxn: Uint8Array): Promise<Gen.PendingTransaction> {
    ...
    return this.client.request.request<Gen.PendingTransaction>({
      ...
      method: "POST",
      body: signedTxn,
      mediaType: "application/x.arx.signed_transaction+bcs",
    });
  }
   ...
   *
   * 1. Create hash message bytes: "Arx::Transaction" bytes + BCS bytes of Transaction.
   ...
   * const bcsTxn = ArxClient.generateBCSTransaction(sender, rawTransaction);
   * const pendingTransaction = await this.submitSignedBCSTransaction(bcsTxn);
   * const transasction = await this.arxClient.waitForTransactionWithResult(pendingTransaction.hash);
   * ```
   */
   ...
   *
   * @param sender ArxAccount of transaction sender.
   ...
   * `arx move compile --save-metadata [ --included-artifacts=<...> ]`.
   * @param sender
  ...
  async estimateMaxGasAmount(forAccount: MaybeHexString): Promise<Uint64> {
    // Only Arx utility coin is accepted as gas
    const typeTag = `0x1::coin::CoinStore<${ARX_COIN}>`;
   ...
   * the account.
   * WARNING: You must create a new instance of ArxAccount after using this function.
  ...
  async rotateAuthKeyEd25519(
    ...
    const helperAccount = new ArxAccount(toPrivateKeyBytes);
    ...
    const bcsTxn = ArxClient.generateBCSTransaction(forAccount, rawTransaction);
