...
test("gets account modules", async () => {
  ...
  const module = modules.find((r) => r.abi!.name === "arx_coin");
  expect(module!.abi!.address).toBe("0x1");
});

test("gets the ArxCoin module", async () => {
  const client = new ArxClient(NODE_URL);
  const module = await client.getAccountModule("0x1", "arx_coin");
  expect(module!.abi!.address).toBe("0x1");
});
...
test(
  ...
  async () => {
    ...
    const account1 = new ArxAccount();
    await faucetClient.fundAccount(account1.address(), 100_000_000);
    let resources = await client.getAccountResources(account1.address());
    let accountResource = resources.find((r) => r.type === arxCoin);
    expect((accountResource!.data as any).coin.value).toBe("100000000");

    const account2 = new ArxAccount();
    await faucetClient.fundAccount(account2.address(), 0);
    resources = await client.getAccountResources(account2.address());
    accountResource = resources.find((r) => r.type === arxCoin);
    expect((accountResource!.data as any).coin.value).toBe("0");

    const token = new TxnBuilderTypes.TypeTagStruct(TxnBuilderTypes.StructTag.fromString("0x1::arx_coin::ArxCoin"));
    ...
    const bcsTxn = ArxClient.generateBCSTransaction(account1, rawTxn);
...
test(
  ...
  async () => {
    ...
    const checkArxCoin = async () => {
      ...
      const account1Resource = resources1.find((r) => r.type === arxCoin);
      const account2Resource = resources2.find((r) => r.type === arxCoin);
    ...
    const payload: Gen.TransactionPayload = {
      ...
      type_arguments: ["0x1::arx_coin::ArxCoin"],
    ...
    [account1, new Ed25519PublicKey(account1.pubKey().toUint8Array())].forEach(async (accountOrAddress) => {
      ...
      const account2ArxCoin = transactionRes.changes.filter((change) => {
        ...
        return (
          ...
          write.data.type === arxCoin &&
          (write.data.data as { coin: { value: string } }).coin.value === "1100000"
        );
      });
      expect(account2ArxCoin).toHaveLength(1);
    });
    await checkArxCoin();
  },
  longTestTimeout,
...
test(
  ...
  async () => {
    ...
    const bcsTxn = ArxClient.generateBCSSimulation(account1, rawTxn);
...
test(
  ...
  async () => {
    ...
    const alice = new ArxAccount();
    await faucetClient.fundAccount(alice.address(), 100_000_000);

    const helperAccount = new ArxAccount();
