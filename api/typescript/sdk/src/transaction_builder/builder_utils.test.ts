...
describe("BuilderUtils", () => {
  ...
  it("parses a sturct TypeTag", async () => {
    ...
    const arxCoin = new TypeTagParser(
      "0x1::coin::CoinStore < 0x1::test_coin::ArxCoin1 ,  0x1::test_coin::ArxCoin2 > ",
    ).parseTypeTag();
    expect(arxCoin instanceof TypeTagStruct).toBeTruthy();
    assertStruct(arxCoin as TypeTagStruct, "0x1", "coin", "CoinStore");

    const arxCoinTrailingComma = new TypeTagParser(
      "0x1::coin::CoinStore < 0x1::test_coin::ArxCoin1 ,  0x1::test_coin::ArxCoin2, > ",
    ).parseTypeTag();
    expect(arxCoinTrailingComma instanceof TypeTagStruct).toBeTruthy();
    assertStruct(arxCoinTrailingComma as TypeTagStruct, "0x1", "coin", "CoinStore");

    const structTypeTags = (arxCoin as TypeTagStruct).value.type_args;
    expect(structTypeTags.length).toBe(2);

    const structTypeTag1 = structTypeTags[0];
    assertStruct(structTypeTag1 as TypeTagStruct, "0x1", "test_coin", "ArxCoin1");

    const structTypeTag2 = structTypeTags[1];
    assertStruct(structTypeTag2 as TypeTagStruct, "0x1", "test_coin", "ArxCoin2");

    const coinComplex = new TypeTagParser(
      // eslint-disable-next-line max-len
      "0x1::coin::CoinStore < 0x2::coin::LPCoin < 0x1::test_coin::ArxCoin1 <u8>, vector<0x1::test_coin::ArxCoin2 > > >",
    ).parseTypeTag();
    ...
    expect(() => {
      new TypeTagParser("0x1::test_coin::CoinStore<0x1::test_coin::ArxCoin").parseTypeTag();
    }).toThrow("Invalid type tag.");
    ...
    expect(() => {
      new TypeTagParser("0x1:test_coin::ArxCoin").parseTypeTag();
    }).toThrow("Unrecognized token.");

    expect(() => {
      new TypeTagParser("0x!::test_coin::ArxCoin").parseTypeTag();
    }).toThrow("Unrecognized token.");
    ...
    expect(() => {
      new TypeTagParser("0x1::<::CoinStore<0x1::test_coin::ArxCoin,").parseTypeTag();
    }).toThrow("Invalid type tag.");

    expect(() => {
      new TypeTagParser("0x1::test_coin::><0x1::test_coin::ArxCoin,").parseTypeTag();
    }).toThrow("Invalid type tag.");
