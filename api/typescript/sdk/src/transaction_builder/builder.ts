...
import { ArgumentABI, EntryFunctionABI, ScriptABI, TransactionScriptABI, TypeArgumentABI } from "../arx_types/abi";
...
const RAW_TRANSACTION_SALT = "ARX::RawTransaction";
const RAW_TRANSACTION_WITH_DATA_SALT = "ARX::RawTransactionWithData";
...
export class TransactionBuilderRemoteABI {
  // We don't want the builder to depend on the actual ArxClient. There might be circular dependencies.
  constructor(
    private readonly arxClient: ArxClientInterface,
  ...
  async fetchABI(addr: string) {
    const modules = await this.arxClient.getAccountModules(addr);
    const abis = modules
  ...
  async build(func: Gen.EntryFunctionId, ty_tags: Gen.MoveType[], args: any[]): Promise<RawTransaction> {
    ...
    const [{ sequence_number: sequenceNumber }, chainId, { gas_estimate: gasUnitPrice }] = await Promise.all([
      rest?.sequenceNumber
        ...
        : this.arxClient.getAccount(senderAddress),
      rest?.chainId ? Promise.resolve(rest?.chainId) : this.arxClient.getChainId(),
      rest?.gasUnitPrice ? Promise.resolve({ gas_estimate: rest?.gasUnitPrice }) : this.arxClient.estimateGasPrice(),
    ]);
