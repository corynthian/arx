...
pub struct NetworkReceivers {
    ...
    pub consensus_messages: channel::Receiver<
        (AccountAddress, Discriminant<ConsensusMsg>),
        (AccountAddress, ConsensusMsg),
    >,
    pub quorum_store_messages: channel::Receiver<
        (AccountAddress, Discriminant<ConsensusMsg>),
        (AccountAddress, ConsensusMsg),
    >,
    pub block_retrieval:
        channel::Receiver<AccountAddress, (AccountAddress, IncomingBlockRetrievalRequest)>,
}
...
pub struct NetworkTask {
    consensus_messages_tx: channel::Sender<
        (AccountAddress, Discriminant<ConsensusMsg>),
        (AccountAddress, ConsensusMsg),
    >,
    quorum_store_messages_tx: channel::Sender<
        (AccountAddress, Discriminant<ConsensusMsg>),
        (AccountAddress, ConsensusMsg),
    >,
    block_retrieval_tx:
        channel::Sender<AccountAddress, (AccountAddress, IncomingBlockRetrievalRequest)>,
...
impl NetworkTask {
    ...
    pub fn new(
        ...
        self_receiver: arx_channel::Receiver<Event<ConsensusMsg>>,
    ) -> (NetworkTask, NetworkReceivers) {
        let (consensus_messages_tx, consensus_messages) =
            channel::new(QueueStyle::LIFO, 1, Some(&counters::CONSENSUS_CHANNEL_MSGS));
        let (quorum_store_messages_tx, quorum_store_messages) = channel::new(
            QueueStyle::FIFO,
            // TODO: tune this value based on quorum store messages with backpressure
            50,
            Some(&counters::QUORUM_STORE_CHANNEL_MSGS),
        );
        let (block_retrieval_tx, block_retrieval) = channel::new(
            QueueStyle::LIFO,
            1,
            ...
            match message {
                ...
                Event::RpcRequest(peer_id, msg, protocol, callback) => match msg {
                    ConsensusMsg::BlockRetrievalRequest(request) => {
                        ...
                        {
                            warn!(error = ?e, "arx channel closed");
                        }
                    },
                    _ => {
