...
use arx_crypto::{hash::ACCUMULATOR_PLACEHOLDER_HASH, HashValue};
...
use arx_safety_rules::{PersistentSafetyStorage, SafetyRulesManager};
use arx_secure_storage::Storage;
use arx_types::{
...
pub fn prepare_buffer_manager() -> (
    ...
    Sender<ResetRequest>,
    arx_channel::Sender<AccountAddress, VerifiedEvent>,
    arx_channels::Receiver<Event<ConsensusMsg>>,
    ...
    let safety_storage = PersistentSafetyStorage::initialize(
        Storage::from(arx_secure_storage::InMemoryStorage::new()),
        signer.author(),
    ...
    let (self_loop_tx, self_loop_rx) = arx_channels::new_test(1000);
    ...
    let (msg_tx, msg_rx) =
        arx_channel::new::<AccountAddress, VerifiedEvent>(QueueStyle::FIFO, channel_size, None);
...
async fn loopback_commit_vote(
    self_loop_rx: &mut arx_channels::Receiver<Event<ConsensusMsg>>,
    msg_tx: &arx_channel::Sender<AccountAddress, VerifiedEvent>,
