...
use super::leader_reputation::{
    extract_epoch_to_proposers_impl, ArxDBBackend, ProposerAndVoterHeuristic,
};
use crate::liveness::{
    leader_reputation::{
...
use arx_bitvec::BitVec;
use arx_consensus_types::common::{Author, Round};
use arx_crypto::{bls12381, HashValue};
use arx_infallible::Mutex;
use arx_keygen::KeyGen;
use arx_storage_interface::{DbReader, Order};
use arx_types::{
...
struct Example1 {
    ...
    arx_db: Arc<MockDbReader>,
    backend: ArxDBBackend,
}
...
impl Example1 {
    fn new(window_size: usize) -> Self {
        ...
        let arx_db = Arc::new(MockDbReader::new());
        let backend = ArxDBBackend::new(window_size, 0, arx_db.clone());
...
fn test_api(use_root_hash: bool) {
    ...
    for epoch in 1..1000 {
        ...
        assert_eq!(
            (epoch, 1),
            arx_db.add_event_with_data(proposers[0], vec![1, 2], vec![])
        );
        assert_eq!(
            (epoch, 2),
            arx_db.add_event_with_data(proposers[0], vec![3], vec![])
        );
        let backend = Box::new(ArxDBBackend::new(1, 4, arx_db.clone()));
        ...
        let state = if use_root_hash {
            [
                arx_db.get_accumulator_root_hash(0).unwrap().to_vec(),
...
fn backend_wrapper_test() {
    let arx_db = Arc::new(MockDbReader::new());
    let backend = ArxDBBackend::new(3, 3, arx_db.clone());

    arx_db.add_event(0, 1);
    arx_db.new_epoch();
    arx_db.skip_rounds(1);
    for i in 2..6 {
        arx_db.add_event(1, i);
    }
    ...
    let mut assert_history = |round, expected_history: Vec<Round>, to_fetch| {
        ...
        assert_eq!(fetch_count, arx_db.fetched(), "At round {}", round);
    };
    ...
    arx_db.add_another_transaction();
    assert_history(12, vec![12, 11, 10], false);
    ...
    arx_db.add_another_transaction();
    arx_db.add_event_after_call(1, 13);
...
fn backend_test_cross_epoch() {
    ...
    arx_db.add_event(0, 1);
    arx_db.new_epoch();
    arx_db.add_event(1, 1);
    arx_db.add_event(1, 2);
    arx_db.add_event(1, 3);
    arx_db.new_epoch();
    arx_db.add_event(2, 1);
    arx_db.add_event(2, 2);
