...
use arx_infallible::{Mutex, MutexGuard};
...
impl ArxDBBackend {
    pub fn new(window_size: usize, seek_len: usize, arx_db: Arc<dyn DbReader>) -> Self {
        Self {
            window_size,
            seek_len,
            arx_db,
    ...
    fn refresh_db_result(
        ...
        let events = self.arx_db.get_events(
            &new_block_event_key(),
            u64::max_value(),
            Order::Descending,
            limit as u64,
...
impl MetadataBackend for ArxDBBackend {
    ...
    fn get_block_metadata(
        ...
        let latest_db_version = self.arx_db.get_latest_version().unwrap_or(0);
