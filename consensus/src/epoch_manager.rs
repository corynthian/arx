...
use crate::{
    ...
    liveness::{
        ...
        leader_reputation::{
            extract_epoch_to_proposers, ArxDBBackend, LeaderReputation,
...
use arx_channel::{channel, message_queues::QueueStyle};
use arx_config::config::{ConsensusConfig, NodeConfig};
use arx_consensus_types::{
    common::{Author, Round},
    epoch_retrieval::EpochRetrievalRequest,
    request_response::PayloadRequest,
};
use arx_event_notifications::ReconfigNotificationListener;
use arx_infallible::{duration_since_epoch, Mutex};
use arx_logger::prelude::*;
use arx_mempool::QuorumStoreRequest;
use arx_network::{application::interface::NetworkClient, protocols::network::Event};
use arx_safety_rules::SafetyRulesManager;
use arx_types::{
...
pub struct EpochManager {
    ...
    self_sender: arx_channel::Sender<Event<ConsensusMsg>>,
    network_sender: ConsensusNetworkClient<NetworkClient<ConsensusMsg>>,
    timeout_sender: arx_channel::Sender<Round>,
    ...
    buffer_manager_msg_tx: Option<channel::Sender<AccountAddress, VerifiedEvent>>,
    buffer_manager_reset_tx: Option<UnboundedSender<ResetRequest>>,
    // channels to round manager
    round_manager_tx: Option<
        channel::Sender<(Author, Discriminant<VerifiedEvent>), (Author, VerifiedEvent)>,
    >,
    round_manager_close_tx: Option<oneshot::Sender<oneshot::Sender<()>>>,
    epoch_state: Option<EpochState>,
    block_retrieval_tx:
        Option<channel::Sender<AccountAddress, IncomingBlockRetrievalRequest>>,
}
...
impl EpochManager {
    ...
    fn create_proposer_election(
        ...
        match &onchain_config.proposer_election_type() {
            ...
            ProposerElectionType::LeaderReputation(leader_reputation_type) => {
                ...
                let backend = Box::new(ArxDBBackend::new(
                    window_size,
                    seek_len,
                    self.storage.arx_db(),
                ));
    ...
    fn spawn_block_retrieval_task(&mut self, epoch: u64, block_store: Arc<BlockStore>) {
        let (request_tx, mut request_rx) = channel::new(
            QueueStyle::LIFO,
            1,
    ...
    /// it sets `self.commit_msg_tx` to a new channel::Sender and returns an OrderingStateComputer
    fn spawn_decoupled_execution(
        ...
        let (commit_msg_tx, commit_msg_rx) = channel::new::<AccountAddress, VerifiedEvent>(
            QueueStyle::FIFO,
    ...
    async fn start_recovery_manager(
        ...
        let (recovery_manager_tx, recovery_manager_rx) = channel::new(
            QueueStyle::LIFO,
            1,
    ...
    async fn start_round_manager(
        ...
        let (round_manager_tx, round_manager_rx) = channel::new(
            QueueStyle::LIFO,
            1,
            Some(&counters::ROUND_MANAGER_CHANNEL_MSGS),
        );
    ...
    pub async fn start(
        mut self,
        mut round_timeout_sender_rx: arx_channel::Receiver<Round>,
