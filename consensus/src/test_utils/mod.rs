...
use arx_types::{ledger_info::LedgerInfo, validator_signer::ValidatorSigner};
...
use arx_consensus_types::{block::block_test_utils::gen_test_certificate, common::Payload};
use arx_types::block_info::BlockInfo;
...
pub fn consensus_runtime() -> runtime::Runtime {
    if nocapture() {
        ::arx_logger::Logger::new().level(Level::Debug).init();
    }

    arx_runtimes::spawn_named_runtime("consensus".into(), None)
}
