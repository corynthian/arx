...
pub trait PersistentLivenessStorage: Send + Sync {
    ...
    /// Returns a handle of the arxdb.
    fn arx_db(&self) -> Arc<dyn DbReader>;
}
...
impl RootMetadata {
    ...
    pub fn new_empty() -> Self {
        Self {
            accu_hash: *arx_crypto::hash::ACCUMULATOR_PLACEHOLDER_HASH,
...
impl StorageWriteProxy {
    pub fn new(config: &NodeConfig, arx_db: Arc<dyn DbReader>) -> Self {
        let db = Arc::new(ConsensusDB::new(config.storage.dir()));
        StorageWriteProxy { db, arx_db }
    }
}
