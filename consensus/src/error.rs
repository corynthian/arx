...
impl From<arx_executor_types::Error> for StateSyncError {
    fn from(e: arx_executor_types::Error) -> Self {
        StateSyncError { inner: e.into() }
    }
}
...
pub fn error_kind(e: &anyhow::Error) -> &'static str {
    if e.downcast_ref::<arx_executor_types::Error>().is_some() {
        return "Execution";
    }
    if let Some(e) = e.downcast_ref::<StateSyncError>() {
        if e.inner
            .downcast_ref::<arx_executor_types::Error>()
            .is_some()
        {
            return "Execution";
        }
    ...
    if e.downcast_ref::<arx_safety_rules::Error>().is_some() {
        return "SafetyRules";
    }
...
mod tests {
    ...
    fn conversion_and_downcast() {
        let error = arx_executor_types::Error::InternalError {
