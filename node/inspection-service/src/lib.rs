...
use arx_metrics_core::{register_int_counter_vec, IntCounterVec};
use once_cell::sync::Lazy;

pub static NUM_METRICS: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!("arx_metrics", "Number of metrics in certain states", &[
        "type"
    ])
    .unwrap()
});
...
pub fn gather_metrics() -> Vec<prometheus::proto::MetricFamily> {
    let metric_families = arx_metrics_core::gather();
    let mut total: u64 = 0;
