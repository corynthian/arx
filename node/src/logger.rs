...
use arx_logger::{
    arx_logger::FileWriter, info, telemetry_log_writer::TelemetryLog, LoggerFilterUpdater,
};
...
pub fn create_logger(
    ...
    let mut logger_builder = arx_logger::Logger::builder();
...
fn log_config_and_build_information(node_config: &NodeConfig) {
    ...
    // Log the node config
    info!(config = node_config, "Loaded ArxNode config");
}
