...
use arx_build_info::build_information;
use arx_config::config::NodeConfig;
use arx_consensus::network_interface::ConsensusMsg;
use arx_consensus_notifications::ConsensusNotifier;
use arx_event_notifications::ReconfigNotificationListener;
use arx_logger::{debug, telemetry_log_writer::TelemetryLog, LoggerFilterUpdater};
use arx_mempool::{network::MempoolSyncMsg, MempoolClientRequest, QuorumStoreRequest};
...
pub fn bootstrap_api_and_indexer(
    ...
    let indexer_runtime =
        indexer::bootstrap_indexer(node_config, chain_id, arx_db, mempool_client_sender)?;
...
pub fn start_consensus_runtime(
    ...
    let consensus_runtime = arx_consensus::consensus_provider::start_consensus(
        node_config,
...
pub fn start_mempool_runtime_and_get_consensus_sender(
    ...
    let instant = Instant::now();
    let mempool = arx_mempool::bootstrap(
        node_config,
...
pub fn start_node_inspection_service(node_config: &NodeConfig) {
    ...
    thread::spawn(move || {
        arx_inspection_service::inspection_service::start_inspection_service(node_config)
    });
}
...
pub fn start_telemetry_service(
    ...
    arx_telemetry::service::start_telemetry_service(
        node_config.clone(),
        chain_id,
        build_info,
        remote_log_rx,
