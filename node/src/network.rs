...
use arx_consensus::network_interface::{ConsensusMsg, DIRECT_SEND, RPC};
use arx_event_notifications::EventSubscriptionService;
use arx_logger::debug;
use arx_mempool::network::MempoolSyncMsg;
use arx_network::{
    application::{
...
use arx_network_builder::builder::NetworkBuilder;
...
pub fn consensus_network_configuration() -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        rpc_protocols,
        channel::Config::new(CONSENSUS_NETWORK_CHANNEL_BUFFER_SIZE)
            .queue_style(QueueStyle::FIFO)
            .counters(&arx_consensus::counters::PENDING_CONSENSUS_NETWORK_EVENTS),
    );
...
pub fn mempool_network_configuration() -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        channel::Config::new(MEMPOOL_NETWORK_CHANNEL_BUFFER_SIZE)
            .queue_style(QueueStyle::KLAST) // TODO: why is this not FIFO?
            .counters(&arx_mempool::counters::PENDING_MEMPOOL_NETWORK_EVENTS),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn storage_service_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(
                &arx_storage_service_server::metrics::PENDING_STORAGE_SERVER_NETWORK_EVENTS,
            ),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
fn create_network_runtime(network_config: &NetworkConfig) -> Runtime {
    ...
    arx_tokio_runtime::spawn_named_runtime(thread_name, network_config.runtime_threads)
}
