...
use anyhow::anyhow;
use arx_api::bootstrap as bootstrap_api;
use arx_config::config::{NodeConfig, PersistableConfig};
use arx_framework::ReleaseBundle;
use arx_logger::{prelude::*, telemetry_log_writer::TelemetryLog, Level, LoggerFilterUpdater};
use arx_state_sync_driver::driver_factory::StateSyncRuntimes;
...
/// Runs an Arx validator or fullnode
#[derive(Clone, Debug, Parser)]
#[clap(name = "Arx Node", author, version)]
pub struct ArxNodeArgs {
    ...
    /// Paths to the Arx framework release package to be used for genesis.
    #[clap(long, requires("test"))]
...
impl ArxNodeArgs {
    /// Runs an Arx node based on the given command line arguments and config flags
    pub fn run(self) {
        if self.test {
            ...
            let genesis_framework = if let Some(path) = self.genesis_framework {
                ...
                arx_cached_packages::head_release_bundle().clone()
            };
...
pub fn start(
    ...
    // Setup panic handler
    arx_crash_handler::setup_panic_handler();
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let test_dir =
        test_dir.unwrap_or_else(|| arx_temppath::TempPath::new().as_ref().to_path_buf());
    ...
    let arx_root_key_path = test_dir.join("mint.key");
    ...
    let config = if validator_config_path.exists() {
        ...
        let builder = arx_genesis::builder::Builder::new(&test_dir, framework.clone())?
        ...
        let mut key_file = fs::File::create(&arx_root_key_path)?;
    ...
    println!("\tArx root key path: {:?}", arx_root_key_path);
...
pub fn setup_environment_and_start_node(
    ...
    let (mempool_client_receiver, api_runtime, index_runtime) =
        services::bootstrap_api_and_indexer(&node_config, arx_db, chain_id)?;
