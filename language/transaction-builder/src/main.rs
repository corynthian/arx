...
#[derive(Debug, StructOpt)]
#[structopt(name = "Arx SDK Builder", about = "Generate boilerplate Arx SDKs")]
struct Options {
    ...
    /// and `arx_types` is assumed to be in "go_module/path/arx_types".
    #[structopt(long)]
    ...
    /// Optional version number for the `arx_types` module (useful in Rust).
    /// If `--with-ol-types` is passed, this will be the version of the generated `arx_types` module.
    #[structopt(long, default_value = "0.1.0")]
    arx_version_number: String,

    /// Optional package name (Python) or module path (Go) of the `arx_types` dependency.
    #[structopt(long)]
...
fn main() {
    ...
    let abis = arx_transaction_builder::read_abis(&options.abi_directories)
    ...
    let install_dir = match options.target_source_dir {
        None => {
            ...
            match options.language {
                Language::Rust => {
                    arx_transaction_builder::rust::output(&mut out, &abis, /* local types */ true)
                        .unwrap()
                },
                Language::Go => {
                    arx_transaction_builder::golang::output(
                        &mut out,
    ...
    // Arx types
    if let Some(registry_file) = options.with_arx_types {
        ...
        if let Language::Rust = options.language {
            arx_transaction_builder::rust::replace_keywords(&mut registry)
        }

        let (package_name, _package_path) = match options.language {
            Language::Rust => (
                if options.arx_version_number == "0.1.0" {
                    "ol-types".to_string()
                } else {
                    format!("ol-types:{}", options.arx_version_number)
                },
                vec!["ol-types"],
            ),
    ...
    let installer: Box<dyn arx_transaction_builder::SourceInstaller<Error = Box<dyn std::error::Error>>> =
        match options.language {
            Language::Rust => Box::new(arx_transaction_builder::rust::Installer::new(
                install_dir,
                options.arx_version_number,
            )),
            Language::Go => Box::new(arx_transaction_builder::golang::Installer::new(
                install_dir,
