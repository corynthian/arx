...
/// If `local_types` is true, we generate a file suitable for the Arx codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targetting the Arx repository itself (as opposed to generated Arx types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("arx_types::transaction", vec![
                    "TransactionPayload",
                    "EntryFunction",
                ]),
                ("arx_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![("arx_types", vec![
                "AccountAddress",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an Arx `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    arx_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, arx_types_version: String) -> Self {
        Installer {
            install_dir,
            arx_types_version,
        }
...
"#,
            name, version, self.arx_types_version,
        )?;
