...
use crate::{
    ...
    arx_vm_impl::{get_transaction_output, ArxVMImpl, ArxVMInternals},
    block_executor::BlockArxVM,
    counters::*,
...
use arx_framework::natives::code::PublishRequest;
use arx_gas::{ArxGasMeter, ChangeSetConfigs};
use arx_logger::prelude::*;
use arx_state_view::StateView;
use arx_types::{
    account_config,
...
#[derive(Clone)]
pub struct ArxVM(pub(crate) ArxVMImpl);

struct ArxSimulationVM(ArxVM);

impl ArxVM {
    pub fn new<S: StateView>(state: &S) -> Self {
        Self(ArxVMImpl::new(state))
    }
    ...
    pub fn internals(&self) -> ArxVMInternals {
        ArxVMInternals::new(&self.0)
    }
    ...
    fn validate_publish_request<S: MoveResolverExt>(
        ...
        for m in modules {
            ...
            arx_framework::verify_module_metadata(m, self.0.get_features())
    ...
    pub(crate) fn execute_user_transaction<S: MoveResolverExt + StateView>(
        ...
        let mut gas_meter = ArxGasMeter::new(
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[arx_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    pub(crate) fn process_waypoint_change_set<S: MoveResolverExt + StateView>(
        ...
        let change_set_ext = match self.execute_writeset(
            storage,
            &writeset_payload,
            Some(arx_types::account_config::reserved_vm_address()),
    ...
    pub fn simulate_signed_transaction(
        ...
        let vm = ArxVM::new(state_view);
        let simulation_vm = ArxSimulationVM(vm);
...
impl VMExecutor for ArxVM {
    ...
    fn execute_block(
        ...
        let ret =
            BlockArxVM::execute_block(transactions, state_view, Self::get_concurrency_level());
        if ret.is_ok() {
...
impl VMAdapter for ArxVM {
    ...
    fn should_restart_execution(vm_output: &TransactionOutput) -> bool {
        let new_epoch_event_key = arx_types::on_chain_config::new_epoch_event_key();
        vm_output
            .events()
            .iter()
    ...
    fn execute_single_transaction<S: MoveResolverExt + StateView>(
        ...
        Ok(match txn {
            PreprocessedTransaction::BlockMetadata(block_metadata) => {
                fail_point!("arx_vm::execution::block_metadata");
            ...
            PreprocessedTransaction::UserTransaction(txn) => {
                fail_point!("arx_vm::execution::user_transaction");
                    ...
                    vm_status.clone().keep_or_discard()
                {
                    error!(
                        *log_context,
                        "[arx_vm] Transaction breaking invariant violation. txn: {:?}",
...
impl AsRef<ArxVMImpl> for ArxVM {
    fn as_ref(&self) -> &ArxVMImpl {
        &self.0
    }
}

impl AsMut<ArxVMImpl> for ArxVM {
    fn as_mut(&mut self) -> &mut ArxVMImpl {
        &mut self.0
    }
}

impl ArxSimulationVM {
