...
use crate::{
    ...
    arx_vm::ArxVM,
    block_executor::ArxTransactionOutput,
    data_cache::{AsMoveResolver, StorageAdapter},
    logging::AdapterLogSchema,
};
use arx_aggregator::{delta_change_set::DeltaChangeSet, transaction::TransactionOutputExt};
use arx_block_executor::task::{ExecutionStatus, ExecutorTask};
...
pub(crate) struct ArxExecutorTask<'a, S> {
    vm: ArxVM,
    base_view: &'a S,
}

impl<'a, S: 'a + StateView + Sync> ExecutorTask for ArxExecutorTask<'a, S> {
    type Argument = &'a S;
    type Error = VMStatus;
    type Output = ArxTransactionOutput;
    type Txn = PreprocessedTransaction;

    fn init(argument: &'a S) -> Self {
        let vm = ArxVM::new(argument);
    ...
    fn execute_transaction(
    ...
    ) -> ExecutionStatus<ArxTransactionOutput, VMStatus> {
            ...
            .execute_single_transaction(txn, &view.as_move_resolver(), &log_context)
        {
            Ok((vm_status, mut output_ext, sender)) => {
                ...
                if ArxVM::should_restart_execution(output_ext.txn_output()) {
                    info!(log_context, "Reconfiguration occurred: restart required",);
                    ExecutionStatus::SkipRest(ArxTransactionOutput::new(output_ext))
                } else {
                    ExecutionStatus::Success(ArxTransactionOutput::new(output_ext))
                }
            },
