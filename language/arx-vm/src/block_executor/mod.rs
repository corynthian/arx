...
use crate::{
    ...
    block_executor::vm_wrapper::ArxExecutorTask,
    counters::{
...
use arx_aggregator::{delta_change_set::DeltaOp, transaction::TransactionOutputExt};
use arx_block_executor::{
    errors::Error,
...
pub(crate) struct ArxTransactionOutput(TransactionOutputExt);

impl ArxTransactionOutput {
...
impl BlockExecutorTransactionOutput for ArxTransactionOutput {
        ...
        let signature_verified_block: Vec<PreprocessedTransaction> =
            RAYON_EXEC_POOL.install(|| {
                transactions
                    .into_par_iter()
                    .with_min_len(25)
                    .map(preprocess_transaction::<ArxVM>)
                    .collect()
            });
        drop(signature_verification_timer);

        BLOCK_EXECUTOR_CONCURRENCY.set(concurrency_level as i64);
        let executor = BlockExecutor::<PreprocessedTransaction, ArxExecutorTask<S>, S>::new(
            concurrency_level,
        );
