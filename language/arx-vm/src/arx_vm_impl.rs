...
use arx_aggregator::transaction::TransactionOutputExt;
use arx_framework::RuntimeModuleMetadataV1;
use arx_gas::{
    AbstractValueSizeGasParameters, ArxGasParameters, ChangeSetConfigs, FromOnChainGasSchedule,
    Gas, NativeGasParameters, StorageGasParameters,
};
use arx_logger::prelude::*;
use arx_state_view::StateView;
use arx_types::{
    account_config::{TransactionValidation, ARX_TRANSACTION_VALIDATION, CORE_CODE_ADDRESS},
    chain_id::ChainId,
    on_chain_config::{
...
pub struct ArxVMImpl {
    move_vm: Arc<MoveVmExt>,
    gas_feature_version: u64,
    gas_params: Option<ArxGasParameters>,
...
impl ArxVMImpl {
    ...
    pub fn new<S: StateView>(state: &S) -> Self {
        ...
        let (mut gas_params, gas_feature_version): (Option<ArxGasParameters>, u64) =
            match GasScheduleV2::fetch_config(&storage) {
                Some(gas_schedule) => {
                    let feature_version = gas_schedule.feature_version;
                    let map = gas_schedule.to_btree_map();
                    (
                        ArxGasParameters::from_on_chain_gas_schedule(&map, feature_version),
                        feature_version,
                    )
                },
                None => match GasSchedule::fetch_config(&storage) {
                    Some(gas_schedule) => {
                        let map = gas_schedule.to_btree_map();
                        (ArxGasParameters::from_on_chain_gas_schedule(&map, 0), 0)
                    },
                    None => (None, 0),
                },
            };
    ...
    pub(crate) fn transaction_validation(&self) -> &TransactionValidation {
        self.transaction_validation
            .as_ref()
            .unwrap_or(&ARX_TRANSACTION_VALIDATION)
    }
    ...
    ) -> Result<&ArxGasParameters, VMStatus> {
    ...
    pub(crate) fn extract_module_metadata(
        ...
        if self.features.is_enabled(FeatureFlag::VM_BINARY_FORMAT_V6) {
            arx_framework::get_vm_metadata(&self.move_vm, module.clone())
        } else {
            arx_framework::get_vm_metadata_v0(&self.move_vm, module.clone())
        }
    }
...
#[derive(Clone, Copy)]
pub struct ArxVMInternals<'a>(&'a ArxVMImpl);

impl<'a> ArxVMInternals<'a> {
    pub fn new(internal: &'a ArxVMImpl) -> Self {
        Self(internal)
    }
    ...
    pub fn gas_params(
        self,
        log_context: &AdapterLogSchema,
    ) -> Result<&'a ArxGasParameters, VMStatus> {
