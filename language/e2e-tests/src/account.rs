...
//! Test infrastructure for modeling Arx accounts.

use crate::gas_costs;
use arx_crypto::ed25519::*;
...
/// Details about a Arx account.
///
/// Tests will typically create a set of `Account` instances to run transactions on. This type
/// encodes the logic to operate on and verify operations on any Arx account.
...
impl Account {
    ...
    /// not automatically get added to the Arx store. To add an account to the store, use
    ...
    pub fn with_keypair(privkey: Ed25519PrivateKey, pubkey: Ed25519PublicKey) -> Self {
        let addr = arx_types::account_address::from_public_key(&pubkey);
        Account {
            addr,
            privkey,
            pubkey,
    ...
    ///
    /// The address will be [`arx_test_root_address`][account_config::arx_test_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][struct@GENESIS_KEYPAIR] as its keypair.
    pub fn new_arx_root() -> Self {
        Self::new_genesis_account(account_config::arx_test_root_address())
    }
