...
use arx_types::transaction::{Script, SignedTransaction};
...
";
    let modules = arx_cached_packages::head_release_bundle().compiled_modules();
...
pub fn create_account_txn(
    ...
    sender
        .transaction()
        .payload(arx::arx_account_create_account(
            *new_account.address(),
        ))
        .sequence_number(seq_num)
        .sign()
