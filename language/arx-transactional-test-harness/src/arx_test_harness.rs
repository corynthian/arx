...
use arx_api_types::AsConverter;
use arx_crypto::{
    ed25519::{Ed25519PrivateKey, Ed25519PublicKey},
    hash::HashValue,
    ValidCryptoMaterialStringExt,
};
use arx_gas::{InitialGasSchedule, TransactionGasParameters};
use arx_language_e2e_tests::data_store::{FakeDataStore, GENESIS_CHANGE_SET_HEAD};
use arx_state_view::TStateView;
use arx_types::{
    access_path::AccessPath,
    account_config::{arx_test_root_address, AccountResource, CoinStoreResource},
...
use arx_vm_genesis::GENESIS_KEYPAIR;
use clap::StructOpt;
...
/// The Arx transaction test adapter.
///
...
///   - It executes transactions through ArxVM, instead of MoveVM directly
struct ArxTestAdapter<'a> {
...
/// Arx-specific arguments for the publish command.
#[derive(StructOpt, Debug)]
struct ArxPublishArgs {
...
/// Arx-specifc arguments for the run command.
#[derive(StructOpt, Debug)]
struct ArxRunArgs {
    #[structopt(long = "private-key", parse(try_from_str = RawPrivateKey::parse))]
    private_key: Option<RawPrivateKey>,
...
/// Arx-specifc arguments for the init command.
#[derive(StructOpt, Debug)]
struct ArxInitArgs {
...
impl arx_storage_interface::DbReader for FakeDbReader {
...
/// Default private key mappings for special Arx accounts.
fn arx_framework_private_key_mapping() -> Vec<(String, Ed25519PrivateKey)> {
...
static PRECOMPILED_ARX_FRAMEWORK: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    let deps = vec![PackagePaths {
        name: None,
        paths: arx_cached_packages::head_release_bundle()
            .files()
            .unwrap(),
        named_address_map: arx_framework::named_addresses().clone(),
    }];
    ...
    match program_res {
        Ok(af) => af,
        Err((files, errors)) => {
            eprintln!("!!!Arx Framework failed to compile!!!");
...
impl<'a> ArxTestAdapter<'a> {
    ...
    /// Obtain the ArxCoin amount under address `signer_addr`
    fn fetch_account_balance(&self, signer_addr: &AccountAddress) -> Result<u64> {
        let arx_coin_tag = CoinStoreResource::struct_tag();

        let coin_access_path =
            AccessPath::resource_access_path(*signer_addr, arx_coin_tag.clone());
        ...
        let annotated =
            MoveValueAnnotator::new(&self.storage).view_resource(&arx_coin_tag, &balance_blob)?;
    ...
    fn run_transaction(&mut self, txn: Transaction) -> Result<TransactionOutput> {
        let mut outputs = ArxVM::execute_block(vec![txn], &self.storage)?;
    ...
    fn create_and_fund_account(&mut self, account_addr: AccountAddress, amount: u64) {
        let parameters = self
            .fetch_transaction_parameters(&arx_test_root_address(), None, None, None, None)
            .unwrap();

        let txn = RawTransaction::new(
            arx_test_root_address(),
            parameters.sequence_number,
            arx_cached_packages::arx::arx_account_create_account(account_addr),
        ...
        let txn = RawTransaction::new(
            ...
            arx_cached_packages::arx::arx_coin_mint(account_addr, amount),
            parameters.max_gas_amount,
            parameters.gas_unit_price,
            parameters.expiration_timestamp_secs,
            ChainId::test(),
        ...
        self.run_transaction(Transaction::UserTransaction(txn))
            .expect("Failed to mint arx coin. This should not happen.");
    }
}

impl<'a> MoveTestAdapter<'a> for ArxTestAdapter<'a> {
    type ExtraInitArgs = ArxInitArgs;
    type ExtraPublishArgs = ArxPublishArgs;
    type ExtraRunArgs = ArxRunArgs;
    type ExtraValueArgs = ();
    type Subcommand = ArxSubCommand;
    ...
    fn init(
        ...
        let mut named_address_mapping = arx_framework::named_addresses().clone();
        ...
        for (name, private_key) in arx_framework_private_key_mapping() {
    ...
    fn handle_subcommand(&mut self, input: TaskInput<Self::Subcommand>) -> Result<Option<String>> {
        match input.command {
            ArxSubCommand::BlockCommand(block_cmd) => {
            ...
            ArxSubCommand::ViewTableCommand(view_table_cmd) => {
...
pub fn run_arx_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    // TODO: remove once bundles removed
    arx_vm::arx_vm::allow_module_bundle_for_test();
    run_test_impl::<ArxTestAdapter>(path, Some(&*PRECOMPILED_ARX_FRAMEWORK))
}
