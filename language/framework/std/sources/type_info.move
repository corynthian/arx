module std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `arx_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `arx_framework::chain_id::get()` will
    /// return whichever ID was passed to `arx_framework::chain_id::initialize_for_test()`.
    ...
    fun test_size_of_val(
        ...
        let two_bools = TwoBools{boarx_1: false, boarx_2: false};
        ...
        let boarx_vector = vector::singleton(false);
        // Push back another bool.
        vector::push_back(&mut boarx_vector, false);
        // Assert size is 3 bytes (1 per element, 1 for base vector).
        assert!(size_of_val(&boarx_vector) == 3, 0);
