...
module std::multi_ed25519 {
    ...
    const E_INVALID_THRESHArxD_OR_NUMBER_OF_SIGNERS: u64 = 3;
    ...
    const THRESHArxD_SIZE_BYTES: u64 = 1;
    ...
    public fun generate_keys(threshold: u8, n: u8): (SecretKey, ValidatedPublicKey) {
        assert!(1 <= threshold && threshold <= n, error::invalid_argument(E_INVALID_THRESHArxD_OR_NUMBER_OF_SIGNERS));
    ...
    public fun new_unvalidated_public_key_from_bytes(bytes: vector<u8>): UnvalidatedPublicKey {
        ...
        assert!(len % INDIVIDUAL_PUBLIC_KEY_NUM_BYTES == THRESHArxD_SIZE_BYTES, error::invalid_argument(E_WRONG_PUBKEY_SIZE));
    ...
    public fun new_validated_public_key_from_bytes(bytes: vector<u8>): Option<ValidatedPublicKey> {
        ...
        if (vector::length(&bytes) % INDIVIDUAL_PUBLIC_KEY_NUM_BYTES == THRESHArxD_SIZE_BYTES &&
