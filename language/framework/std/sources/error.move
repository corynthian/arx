...
///
/// Canonical error codes use the 3 lowest bytes of the u64 abort code range (the upper 5 bytes are free
/// for other use).
...
/// ```
///
/// This code is both valid in the worlds with and without canonical errors. It can be used as a plain
/// module local
/// error reason understand by the existing error map tooling, or as a canonical code.
///
/// The actual canonical categories have been adopted from Google's canonical error codes, which in turn
/// are derived
/// from Unix error codes [see here](https://cloud.google.com/apis/design/errors#handling_errors). Each
/// code has an
/// associated HTTP error code which can be used in REST apis. The mapping from error code to http code
/// is not 1:1;
