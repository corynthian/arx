/**
 * Simple module for tracking and calculating shares of a pool of coins. The shares are worth more as the
 * total coins in the pool increases. New shareholder can buy more shares or redeem their existing
 * shares.
 *
 * Example flow:
 * 1. Pool start outs empty.
 * 2. Shareholder A buys in with 1000 coins. A will receive 1000 shares in the pool. Pool now has 1000
 * total coins and 1000 total shares.
 * 3. Pool appreciates in value from rewards and now has 2000 coins. A's 1000 shares are now worth 2000
 * coins.
 * 4. Shareholder B now buys in with 1000 coins. Since before the buy in, each existing share is worth
 * 2 coins, B will receive 500 shares in exchange for 1000 coins. Pool now has 1500 shares and 3000
 * coins.
 * 5. Pool appreciates in value from rewards and now has 6000 coins.
 * 6. A redeems 500 shares. Each share is worth 6000 / 1500 = 4. A receives 2000 coins. Pool has 4000
 * coins and 1000 shares left.
 */
module std::pool_u64 {
    ...
    struct Pool has store {
        ...
        // Default to 1. This can be used to minimize rounding errors when computing shares and
	// coins amount. However, users need to make sure the coins amount don't overflow when
