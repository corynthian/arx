...
module arx::resource_account {
    ...
    public entry fun create_resource_account_and_fund(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        coin::register<ArxCoin>(&resource);
        coin::transfer<ArxCoin>(origin, signer::address_of(&resource), fund_amount);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun create_resource_account_and_publish_package(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        arx::code::publish_package_txn(&resource, metadata_serialized, code);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun test_create_account_and_retrieve_cap(user: signer) acquires Container {
        ...
        let resource_addr = arx::account::create_resource_address(&user_addr, seed);
    ...
    public entry fun with_coin(framework: signer, user: signer) acquires Container {
        let user_addr = signer::address_of(&user);
        arx::arx_account::create_account(copy user_addr);

        let (burn, mint) = arx::arx_coin::initialize_for_test(&framework);
        let coin = coin::mint<ArxCoin>(100, &mint);
        ...
        let resource_addr = arx::account::create_resource_address(&user_addr, seed);
        coin::transfer<ArxCoin>(&user, resource_addr, 10);
    ...
    #[expected_failure(abort_code = 0x60005, location = arx::coin)]
    public entry fun without_coin(framework: signer, user: signer) acquires Container {
        let user_addr = signer::address_of(&user);
        arx::arx_account::create_account(user_addr);
