module arx::arx_account {
    use arx::account::{Self, new_event_handle};
    use arx::arx_coin::ArxCoin;
    use arx::coin::{Self, Coin};
    use arx::create_signer::create_signer;
    use arx::event::{EventHandle, emit_event};
    use std::error;
    use std::signer;
    use std::vector;

    friend arx::genesis;
    friend arx::resource_account;

    /// Account does not exist.
    const EACCOUNT_NOT_FOUND: u64 = 1;
    /// Account is not registered to receive Arx.
    const EACCOUNT_NOT_REGISTERED_FOR_Arx: u64 = 2;
    ...
    public entry fun transfer(source: &signer, to: address, amount: u64) {
        ...
        // Resource accounts can be created without registering them to receive Arx.
        // This conveniently does the registration if necessary.
        if (!coin::is_account_registered<ArxCoin>(to)) {
            coin::register<ArxCoin>(&create_signer(to));
        };
        coin::transfer<ArxCoin>(source, to, amount)
    }
    ...
    public fun assert_account_is_registered_for_apt(addr: address) {
        ...
        assert!(coin::is_account_registered<ArxCoin>(addr), error::not_found(EACCOUNT_NOT_REGISTERED_FOR_Arx));
    }
    ...
    use std::string::utf8;
    #[test_only]
    use arx::account::create_account_for_test;
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = arx::arx_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, bob, 500);
        assert!(coin::balance<ArxCoin>(bob) == 500, 0);
        transfer(alice, carol, 500);
        assert!(coin::balance<ArxCoin>(carol) == 500, 1);
        transfer(alice, carol, 1500);
        assert!(coin::balance<ArxCoin>(carol) == 2000, 2);
    ...
    public fun test_transfer_to_resource_account(alice: &signer, core: &signer) {
        ...
        assert!(!coin::is_account_registered<ArxCoin>(resource_acc_addr), 0);

        let (burn_cap, mint_cap) = arx::arx_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, resource_acc_addr, 500);
        assert!(coin::balance<ArxCoin>(resource_acc_addr) == 500, 1);
    ...
    public fun test_batch_transfer(from: &signer, core: &signer, recipient_1: &signer, recipient_2: &signer) {
        ...
        assert!(coin::balance<ArxCoin>(recipient_1_addr) == 100, 0);
        assert!(coin::balance<ArxCoin>(recipient_2_addr) == 500, 1);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }
