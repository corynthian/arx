...
module arx::block {
    ...
    public(friend) fun initialize(arx: &signer, epoch_interval_microsecs: u64) {
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                new_block_events: account::new_event_handle<NewBlockEvent>(arx),
                update_epoch_interval_events: account::new_event_handle<UpdateEpochIntervalEvent>(arx),
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        ...
        let block_resource = borrow_global_mut<BlockResource>(@arx);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@arx).epoch_interval / 1000000
    }
    ...
    fun block_prologue(
        ...
        let block_metadata_ref = borrow_global_mut<BlockResource>(@arx);
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@arx).height
    }
    ...
    #[test(arx = @arx)]
    public entry fun test_update_epoch_interval(arx: signer) acquires BlockResource {
        account::create_account_for_test(@arx);
        initialize(&arx, 1);
        assert!(borrow_global<BlockResource>(@arx).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&arx, 2);
        assert!(borrow_global<BlockResource>(@arx).epoch_interval == 2, 1);
    }

    #[test(arx = @arx, account = @0x123)]
