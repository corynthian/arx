spec arx::coin {
    ...
    spec initialize_supply_config(arx: &signer) {
        let arx_addr = signer::address_of(arx);
        aborts_if !system_addresses::is_arx_address(arx_addr);
        aborts_if exists<SupplyConfig>(arx_addr);
        ensures !global<SupplyConfig>(arx_addr).allow_upgrades;
        ensures exists<SupplyConfig>(arx_addr);
    }

    /// Can only be updated by `@arx`.
    spec allow_supply_upgrades(arx: &signer, allowed: bool) {
        modifies global<SupplyConfig>(@arx);
        let arx_addr = signer::address_of(arx);
        aborts_if !system_addresses::is_arx_address(arx_addr);
        aborts_if !exists<SupplyConfig>(arx_addr);
        let post allow_upgrades_post = global<SupplyConfig>(@arx);
    ...
    /// The creator of `CoinType` must be `@arx`.
    ...
    spec upgrade_supply<CoinType>(account: &signer) {
        ...
        aborts_if !exists<SupplyConfig>(@arx);
        aborts_if !exists<CoinInfo<CoinType>>(account_addr);
        let supply_config = global<SupplyConfig>(@arx);
    ...
    spec initialize_aggregatable_coin<CoinType>(arx: &signer): AggregatableCoin<CoinType> {
        // TODO: specify this.
    }
