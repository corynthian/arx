module arx::account {
    ...
    #[test_only]
    /// Create signer for testing, independently of an Arx-style `Account`.
    ...
    public(friend) fun initialize(arx: &signer) {
        ...
        move_to(arx, OriginatingAddress {
    ...
    fun update_auth_key_and_originating_address_table(
        ...
        let address_map = &mut borrow_global_mut<OriginatingAddress>(@arx).address_map;
    ...
    public(friend) fun create_reserved_account(addr: address): (signer, SignerCapability) {
        assert!(
            addr == @0x1 ||
                addr == @0x2 ||
                addr == @0x3 ||
    ...
    fun test_create_signer_for_test() {
        assert!(signer::address_of(&create_signer_for_test(@arx)) == @0x1, 0);
    ...
    public entry fun test_cannot_contrarx_resource_account_via_auth_key() acquires Account {
    ...
    #[test(alice = @0xa11ce)]
    #[expected_failure(abort_code = 65537, location = arx::ed25519)]
    ...
    #[test(bob = @0x345, charlie = @0x567, framework = @arx)]
