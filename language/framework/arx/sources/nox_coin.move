...
module arx::nox_coin {
    ...
    struct NoxCoin has key {}

    struct MintCapStore has key {
        mint_cap: MintCapability<NoxCoin>,
    }
    ...
    public(friend) fun initialize(arx: &signer): (BurnCapability<NoxCoin>, MintCapability<NoxCoin>) {
        system_addresses::assert_arx(arx);

        let (burn_cap, freeze_cap, mint_cap) = coin::initialize_with_parallelizable_supply<NoxCoin>(
            arx,
            string::utf8(b"NoxCoin"),
            string::utf8(b"NOX"),
            8, /* decimals */
            true, /* monitor_supply */
        );
    ...
    public(friend) fun configure_accounts_for_test(
        ...
        // Mint the core resource account NoxCoin for gas so it can execute system transactions.
        coin::register<NoxCoin>(core_resources);
        let coins = coin::mint<NoxCoin>(
            18446744073709551615,
            &mint_cap,
        );
        coin::deposit<NoxCoin>(signer::address_of(core_resources), coins);
    ...
    public entry fun mint(
        ...
        let coins_minted = coin::mint<NoxCoin>(amount, mint_cap);
        coin::deposit<NoxCoin>(dst_addr, coins_minted);
    }
    ...
    #[test_only]
    public fun initialize_for_test(arx: &signer): (BurnCapability<NoxCoin>, MintCapability<NoxCoin>) {
    ...
    #[test_only]
    public fun initialize_for_test_without_aggregator_factory(arx: &signer): (BurnCapability<NoxCoin>, MintCapability<NoxCoin>) {
        initialize(arx)
    }
}
