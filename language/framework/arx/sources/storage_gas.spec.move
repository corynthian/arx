spec arx::storage_gas {
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@arx);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@arx);
    }
    ...
    spec set_config(arx: &signer, config: StorageGasConfig) {
    ...
    spec initialize(arx: &signer) {
        include system_addresses::AbortsIfNotOpenLibra{ account: arx };
        aborts_if exists<StorageGasConfig>(@arx);
        aborts_if exists<StorageGas>(@arx);

        ensures exists<StorageGasConfig>(@arx);
        ensures exists<StorageGas>(@arx);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@arx);
        aborts_if !exists<StorageGas>(@arx);
        aborts_if !exists<state_storage::StateStorageUsage>(@arx);
    }
