...
module arx::voting {
    ...
    public fun create_proposal_with_empty_execution_hash_should_fail_generic(governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@arx);
        let governance_address = signer::address_of(governance);
    ...
    #[test_only]
    public entry fun test_voting_passed_generic(arx: &signer, governance: &signer, use_create_multi_step: bool, use_resolve_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@arx);
    ...
    #[test(arx = @arx, governance = @0x123)]
    public entry fun test_voting_passed(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_generic(arx, governance, false, false);
    }

    #[test(arx = @arx, governance = @0x123)]
    public entry fun test_voting_passed_multi_step(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_generic(arx, governance, true, true);
    }

    #[test(arx = @arx, governance = @0x123)]
    #[expected_failure(abort_code=0x5000a, location = Self)]
    public entry fun test_voting_passed_multi_step_cannot_use_single_step_resolve_function(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_generic(arx, governance, true, false);
    }

    #[test(arx = @arx, governance = @0x123)]
    public entry fun test_voting_passed_single_step_can_use_generic_function(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_generic(arx, governance, false, true);
    }

    #[test_only]
    public entry fun test_cannot_resolve_twice_generic(arx: &signer, governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@arx);
        timestamp::set_time_has_started_for_testing(arx);
    ...
    public entry fun test_cannot_resolve_twice(arx: &signer, governance: &signer) acquires VotingForum {
        test_cannot_resolve_twice_generic(arx, governance, false);
    }

    #[test(arx = @arx, governance = @0x123)]
    #[expected_failure(abort_code = 0x30003, location = Self)]
    public entry fun test_cannot_resolve_twice_multi_step(arx: &signer, governance: &signer) acquires VotingForum {
        test_cannot_resolve_twice_generic(arx, governance, true);
    }

    #[test_only]
    public entry fun test_voting_passed_early_generic(arx: &signer, governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@arx);
        timestamp::set_time_has_started_for_testing(arx);
    ...
    #[test(arx = @arx, governance = @0x123)]
    public entry fun test_voting_passed_early(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_early_generic(arx, governance, false);
    }

    #[test(arx = @arx, governance = @0x123)]
    public entry fun test_voting_passed_early_multi_step(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_passed_early_generic(arx, governance, true);
    }
    ...
    public entry fun test_voting_passed_early_in_same_tx_should_fail(
        ...
        test_voting_passed_early_in_same_tx_should_fail_generic(arx, governance, false);
    }
    ...
    public entry fun test_voting_passed_early_in_same_tx_should_fail_multi_step(
        ...
        test_voting_passed_early_in_same_tx_should_fail_generic(arx, governance, true);
    }

    #[test_only]
    public entry fun test_voting_failed_generic(arx: &signer, governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@arx);
        timestamp::set_time_has_started_for_testing(arx);
    ...
    public entry fun test_voting_failed(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_failed_generic(arx, governance, false);
    }

    #[test(arx = @arx, governance = @0x123)]
    #[expected_failure(abort_code = 0x30002, location = Self)]
    public entry fun test_voting_failed_multi_step(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_failed_generic(arx, governance, true);
    }
    ...
    #[test_only]
    public entry fun test_voting_failed_early_generic(arx: &signer, governance: &signer, is_multi_step: bool) acquires VotingForum {
        account::create_account_for_test(@arx);
        timestamp::set_time_has_started_for_testing(arx);
    ...
    #[test(arx = @arx, governance = @0x123)]
    #[expected_failure(abort_code = 0x30002, location = Self)]
    public entry fun test_voting_failed_early(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_failed_early_generic(arx, governance, true);
    }

    #[test(arx = @arx, governance = @0x123)]
    #[expected_failure(abort_code = 0x30002, location = Self)]
    public entry fun test_voting_failed_early_multi_step(arx: &signer, governance: &signer) acquires VotingForum {
        test_voting_failed_early_generic(arx, governance, false);
    }
    ...
    public entry fun test_cannot_set_min_threshold_higher_than_early_resolution(
        ...
        test_cannot_set_min_threshold_higher_than_early_resolution_generic(arx, governance, false);
    }
    ...
    public entry fun test_cannot_set_min_threshold_higher_than_early_resolution_multi_step(
        ...
        test_cannot_set_min_threshold_higher_than_early_resolution_generic(arx, governance, true);
    }

    #[test(arx = @arx, governance = @0x123)]
    public entry fun test_replace_execution_hash(arx: &signer, governance: &signer) acquires VotingForum {
        account::create_account_for_test(@arx);
        timestamp::set_time_has_started_for_testing(arx);
