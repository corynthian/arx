...
module arx::storage_gas {
    ...
    public(friend) fun set_config(arx: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_arx(arx);
        *borrow_global_mut<StorageGasConfig>(@arx) = config;
    }
    ...
    /// target utilization.
    public fun initialize(arx: &signer) {
        system_addresses::assert_arx(arx);
        assert!(
            !exists<StorageGasConfig>(@arx),
        ...
        move_to(arx, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@arx),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(arx, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@arx);
        let gas = borrow_global_mut<StorageGas>(@arx);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@arx);
