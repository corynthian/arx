...
module arx::coin {
    ...
    use arx::aggregator::{Self, Aggregator};
    use arx::event::{Self, EventHandle};
    use arx::optional_aggregator::{Self, OptionalAggregator};
    ...
    public(friend) fun initialize_supply_config(arx: &signer) {
        system_addresses::assert_arx(arx);
        move_to(arx, SupplyConfig { allow_upgrades: false });
    }

    /// This should be called by on-chain governance to update the config and allow
    /// or disallow upgradability of total supply.
    public fun allow_supply_upgrades(arx: &signer, allowed: bool) acquires SupplyConfig {
        system_addresses::assert_arx(arx);
        let allow_upgrades = &mut borrow_global_mut<SupplyConfig>(@arx).allow_upgrades;
    ...
    public entry fun upgrade_supply<CoinType>(account: &signer) acquires CoinInfo, SupplyConfig {
        ...
        assert!(
            borrow_global_mut<SupplyConfig>(@arx).allow_upgrades,
    ...
    #[test(framework = @arx, other = @0x123)]
    ...
    #[test(framework = @arx)]
    #[expected_failure(abort_code = 0x20001, location = arx::aggregator)]
    ...
    #[test(framework = @arx)]
    #[expected_failure(abort_code = 0x5000B, location = arx::coin)]
