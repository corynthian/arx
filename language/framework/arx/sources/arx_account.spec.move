spec arx::arx_account {
    ...
    spec create_account(auth_key: address) {
        ...
        ensures exists<coin::CoinStore<ArxCoin>>(auth_key);
    }
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @arx;
    }
    ...
    spec assert_account_is_registered_for_apt(addr: address) {
        aborts_if !account::exists_at(addr);
        aborts_if !coin::is_account_registered<ArxCoin>(addr);
    }
