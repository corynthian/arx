spec arx::account {
    ...
    /// Only the address `@arx` can call.
    /// OriginatingAddress does not exist under `@arx` before the call.
    spec initialize(arx: &signer) {
        let arx_addr = signer::address_of(arx);
        aborts_if !system_addresses::is_arx_address(arx_addr);
        aborts_if exists<OriginatingAddress>(arx_addr);
        ensures exists<OriginatingAddress>(arx_addr);
    }
    ...
    spec create_account(new_address: address): signer {
        ...
        aborts_if new_address == @vm_reserved || new_address == @arx;
    ...
    spec rotate_authentication_key(
        ...
        modifies global<Account>(addr);
        modifies global<OriginatingAddress>(@arx);
    }
    ...
    spec create_reserved_account(addr: address): (signer, SignerCapability) {
