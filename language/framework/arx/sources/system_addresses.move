module arx::system_addresses {
    ...
    public fun assert_arx(account: &signer) {
        assert!(
            is_arx_address(signer::address_of(account)),
    ...
    public fun is_arx_address(addr: address): bool {
        addr == @arx
    }
    ...
    public fun is_reserved_address(addr: address): bool {
        is_arx_address(addr) || is_vm_address(addr)
    }
}
