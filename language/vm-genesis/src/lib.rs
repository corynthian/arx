...
use arx_framework::{ReleaseBundle, ReleasePackage};
use arx_gas::{
    AbstractValueSizeGasParameters, ArxGasParameters, ChangeSetConfigs, InitialGasSchedule,
    NativeGasParameters, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use arx_types::{
    account_config::{self, arx_test_root_address, events::NewEpochEvent, CORE_CODE_ADDRESS},
    chain_id::ChainId,
    contract_event::ContractEvent,
    on_chain_config::{
        FeatureFlag, Features, GasScheduleV2, OnChainConsensusConfig, ARX_MAX_KNOWN_VERSION,
    },
...
const ARX_COINS_BASE_WITH_DECIMALS: u64 = u64::pow(10, 8);
...
pub fn default_gas_schedule() -> GasScheduleV2 {
    GasScheduleV2 {
        feature_version: arx_gas::LATEST_GAS_FEATURE_VERSION,
        entries: ArxGasParameters::initial().to_on_chain_gas_schedule(LATEST_GAS_FEATURE_VERSION),
    }
}

pub fn encode_arx_mainnet_genesis_transaction(
...
pub fn encode_genesis_change_set(
    ...
    if genesis_config.is_test {
        initialize_core_resources_and_arx_coin(&mut session, core_resources_key);
    } else {
        initialize_arx_coin(&mut session);
    }
    initialize_on_chain_governance(&mut session, genesis_config);
...
fn initialize(
    ...
    exec_function(
        ...
        serialize_values(&vec![
            ...
            MoveValue::U64(ARX_MAX_KNOWN_VERSION.major),
...
fn initialize_arx_coin(session: &mut SessionExt<impl MoveResolver>) {
    exec_function(
        session,
        GENESIS_MODULE_NAME,
        "initialize_arx_coin",
...
fn initialize_core_resources_and_arx_coin(
    session: &mut SessionExt<impl MoveResolver>,
    core_resources_key: &Ed25519PublicKey,
) {
    ...
    exec_function(
        session,
        GENESIS_MODULE_NAME,
        "initialize_core_resources_and_arx_coin",
        vec![],
        serialize_values(&vec![
...
fn allow_core_resources_to_set_version(session: &mut SessionExt<impl MoveResolver>) {
    exec_function(
        ...
        "initialize_for_test",
        vec![],
        serialize_values(&vec![MoveValue::Signer(arx_test_root_address())]),
    );
}
...
pub fn generate_genesis_change_set_for_testing_with_count(
    ...
    let framework = match genesis_options {
        GenesisOptions::Head => arx_cached_packages::head_release_bundle(),
        GenesisOptions::Testnet => arx_framework::testnet_release_bundle(),
        GenesisOptions::Mainnet => {
            // We don't yet have mainnet, so returning testnet here
            arx_framework::testnet_release_bundle()
        },
    };
...
pub fn generate_genesis_change_set_for_mainnet(genesis_options: GenesisOptions) -> ChangeSet {
    let framework = match genesis_options {
        ...
        GenesisOptions::Testnet => arx_framework::testnet_release_bundle(),
        // We don't yet have mainnet, so returning testnet here
        GenesisOptions::Mainnet => arx_framework::testnet_release_bundle(),
    };
...
pub struct Validator {
    /// The Arx account address of the validator or the admin in the case of a commissioned or
    /// vesting managed validator.
    pub owner_address: AccountAddress,
    /// The Arx account address of the validator's operator (same as `address` if the validator is
    /// its own operator).
...
fn mainnet_genesis_config() -> GenesisConfiguration {
    ...
    GenesisConfiguration {
        ...
        is_test: false,
        min_stake: 1_000_000 * ARX_COINS_BASE_WITH_DECIMALS, // 1M ARX
        // 400M ARX
        min_voting_threshold: (400_000_000 * ARX_COINS_BASE_WITH_DECIMALS as u128),
        max_stake: 50_000_000 * ARX_COINS_BASE_WITH_DECIMALS, // 50M ARX.
        recurring_lockup_duration_secs: 30 * 24 * 3600,         // 1 month
        required_proposer_stake: 1_000_000 * ARX_COINS_BASE_WITH_DECIMALS, // 1M ARX
        rewards_apy_percentage: 10,
...
pub fn test_genesis_module_publishing() {
    ...
    for (module_bytes, module) in
        arx_cached_packages::head_release_bundle().code_and_compiled_modules()
    {
        state_view.add_module(&module.self_id(), module_bytes);
    }
    let data_cache = state_view.as_move_resolver();
    ...
    let id1 = HashValue::zero();
    let mut session = move_vm.new_session(&data_cache, SessionId::genesis(id1));
    publish_framework(&mut session, arx_cached_packages::head_release_bundle());
}
...
pub fn test_mainnet_end_to_end() {
    ...
    let balance = 10_000_000 * ARX_COINS_BASE_WITH_DECIMALS;
    let non_validator_balance = 10 * ARX_COINS_BASE_WITH_DECIMALS;
    ...
    zero_commission_validator.operator_address = account44;
    zero_commission_validator.vault_address = account44;
    ...
    same_owner_validator_1.vault_address = account44;
    let mut same_owner_validator_2 = test_validators[4].data.clone();
    same_owner_validator_2.owner_address = account46;
    same_owner_validator_2.operator_address = operator4;
    same_owner_validator_2.vault_address = account45;
    let mut same_owner_validator_3 = test_validators[5].data.clone();
    same_owner_validator_3.owner_address = account47;
    same_owner_validator_3.operator_address = operator5;
    same_owner_validator_3.vault_address = account47;
    ...
    let transaction = encode_arx_mainnet_genesis_transaction(
        &accounts,
        &validators,
    ...
    // join_during_genesis = false.
    assert!(validator_set_addresses.contains(&account45));
    assert!(validator_set_addresses.contains(&account46));
    // This validator should not be in the genesis validator set as they specified
    // join_during_genesis = false.
    assert!(!validator_set_addresses.contains(&account47));
}
