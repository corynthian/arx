...
use crate::gas_meter::{
    ArxGasParameters, InitialGasSchedule, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use anyhow::Result;
use arx_package_builder::PackageBuilder;
use arx_types::on_chain_config::GasScheduleV2;
use clap::Parser;
...
fn generate_script(gas_schedule: &GasScheduleV2) -> Result<String> {
    ...
    emitln!(writer, "use arx::governance;");
    emitln!(writer, "use arx::gas_schedule;");
    emitln!(writer);
...
pub fn generate_update_proposal(args: &GenArgs) -> Result<()> {
    ...
    pack.add_local_dep("Arx", &arx_path().to_string_lossy());
