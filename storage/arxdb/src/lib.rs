...
//! This crate provides [`ArxDB`] which represents physical storage of the core Arx data
//! structures.
//!
//! It relays read/write operations on the physical storage via [`schemadb`] to the underlying
//! Key-Value storage system, and implements arx data structures on top of it.
...
#[cfg(any(test, feature = "fuzzing"))]
use arx_config::config::DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD;
use arx_config::config::{
    PrunerConfig, RocksdbConfig, RocksdbConfigs, BUFFERED_STATE_TARGET_ITEMS,
    NO_OP_STORAGE_PRUNER_CONFIG,
};
use arx_crypto::hash::HashValue;
use arx_db_indexer::Indexer;
use arx_infallible::Mutex;
use arx_logger::prelude::*;
use arx_rocksdb_options::gen_rocksdb_options;
use arx_schemadb::{SchemaBatch, DB};
...
use arx_vm::data_cache::AsMoveResolver;
...
static ROCKSDB_PROPERTY_MAP: Lazy<HashMap<&str, String>> = Lazy::new(|| {
    ...
    .iter()
    .map(|x| (*x, format!("arx_{}", x.replace('.', "_"))))
    .collect()
});
...
fn update_rocksdb_properties(ledger_rocksdb: &DB, state_merkle_rocksdb: &DB) -> Result<()> {
    ...
    for cf_name in ledger_db_column_families() {
        for (rockdb_property_name, arx_rocksdb_property_name) in &*ROCKSDB_PROPERTY_MAP {
            ROCKSDB_PROPERTIES
                .with_label_values(&[cf_name, arx_rocksdb_property_name])
...
/// access to the core Arx data structures.
#[derive(Debug)]
pub struct ArxDB {
    ledger_db: Arc<DB>,
...
impl ArxDB {
    ...
    pub fn open<P: AsRef<Path> + Clone>(
        ...
        info!(
            ...
            "Opened ArxDB (LedgerDB + StateMerkleDB).",
        );
        Ok(myself)
    }
    ...
    fn open_indexer(
        ...
        if indexer.next_version() < ledger_next_version {
            ...
            while next_version < ledger_next_version {
                info!(next_version = next_version, "ArxDB Indexer catching up. ",);
    ...
    pub fn create_checkpoint(db_path: impl AsRef<Path>, cp_path: impl AsRef<Path>) -> Result<()> {
        ...
        {
            let ledger_db = arx_schemadb::DB::open(
                ledger_db_path,
                LEDGER_DB_NAME,
                ledger_db_column_families(),
                &arx_schemadb::Options::default(),
            )?;
            ledger_db.create_checkpoint(ledger_cp_path)?;
        }
        {
            let state_merkle_db = arx_schemadb::DB::open(
...
impl DbReader for ArxDB {
    ...
    /// Returns the transaction by version, delegates to `ArxDB::get_transaction_with_proof`.
    ...
    fn get_table_info(&self, handle: TableHandle) -> Result<TableInfo> {
        gauged_api("get_table_info", || {
            self.get_table_info_option(handle)?
                .ok_or_else(|| ArxDbError::NotFound(format!("TableInfo for {:?}", handle)).into())
        })
    }
...
impl GetRestoreHandler for Arc<ArxDB> {
    fn get_restore_handler(&self) -> RestoreHandler {
        RestoreHandler::new(
