...
/// Alternate implementation of [ArxDB] for use with consensus-only-perf-test feature.
/// It stores and serves data from in-memory data structures as opposed to [ArxDB],
/// which uses RocksDB. Note that FakeArxDB re-implements only a subset of the
/// features of [ArxDB] while passing through remaining features to the wrapped inner
/// [ArxDB].
pub struct FakeArxDB {
    inner: ArxDB,
...
impl FakeArxDB {
    ...
    fn save_and_compute_root_hash(
        ...
        let (root_hash, writes) =
            MerkleAccumulator::<FakeArxDB, TransactionAccumulatorHasher>::append(
                self,
    ...
    fn get_frozen_subtree_hashes(&self, num_transactions: u64) -> Result<Vec<HashValue>> {
        MerkleAccumulator::<FakeArxDB, TransactionAccumulatorHasher>::get_frozen_subtree_hashes(
            self,
            num_transactions,
        )
    }
...
impl DbWriter for FakeArxDB {
    fn get_state_snapshot_receiver(
        &self,
        version: Version,
        expected_root_hash: HashValue,
    ) -> Result<Box<dyn arx_storage_interface::StateSnapshotReceiver<StateKey, StateValue>>> {
        self.inner
...
impl DbReader for FakeArxDB {
    fn get_epoch_ending_ledger_infos(
        &self,
        start_epoch: u64,
        end_epoch: u64,
    ) -> Result<arx_types::epoch_change::EpochChangeProof> {
    ...
    fn get_events(
        &self,
        event_key: &arx_types::event::EventKey,
        start: u64,
        order: arx_storage_interface::Order,
        limit: u64,
        ledger_version: Version,
    ...
    fn get_account_transactions(
        ...
        include_events: bool,
        ledger_version: Version,
    ) -> Result<arx_types::transaction::AccountTransactionsWithProof> {
        self.inner
    ...
    fn get_state_value_by_version(
        ...
        // Since the genesis write set is persisted with ArxDB, we call
        // it to serve state values targetting the framework account
        // (to access ArxCoin, for example).
    ...
    fn get_latest_executed_trees(&self) -> Result<ExecutedTrees> {
        // If the genesis is not executed yet, we need to get the executed trees from the inner ArxDB
        // This is because when we call save_transactions for the genesis block, we call [ArxDB::save_transactions]
        // where there is an expectation that the root of the SMTs are the same pointers. Here,
        // we get from the inner ArxDB which ensures that the pointers match when save_transactions is called.
    ...
    fn get_latest_transaction_info_option(
        &self,
    ) -> Result<Option<(Version, arx_types::transaction::TransactionInfo)>> {
        Ok(self
            .latest_txn_info
            .load_full()
...
fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(ArxDbError::TooManyRequested(num_requested, max_allowed).into())
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    proptest! {
        ...
        fn test_save_transactions(input in arb_blocks_to_commit()) {
            ...
            let db = FakeArxDB::new(ArxDB::new_for_test(&tmp_dir));
