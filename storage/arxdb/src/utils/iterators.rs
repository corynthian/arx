...
use arx_schemadb::{iterator::SchemaIterator, ReadOptions, DB};
use arx_types::{
...
impl<'a> PrefixedStateValueIterator<'a> {
    pub fn new(
        ...
        // arx/abc|0
        // arx/abd|1
        // if we seek('arx/'), and call next, we may not reach `arx/abd/1` because the prefix extractor we adopted
        // here will stick with prefix `arx/abc` and return `None` or any arbitrary result after visited all the
        // keys starting with `arx/abc`.
