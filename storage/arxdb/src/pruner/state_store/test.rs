...
use crate::{
    ...
    ArxDB, LedgerPrunerManager, PrunerManager, StatePrunerManager,
};
use arx_config::config::{LedgerPrunerConfig, StateMerklePrunerConfig};
use arx_crypto::HashValue;
use arx_schemadb::{ReadOptions, SchemaBatch, DB};
use arx_storage_interface::{jmt_update_refs, jmt_updates, DbReader};
...
fn test_state_store_pruner() {
    ...
    let arx_db = ArxDB::new_for_test_no_cache(&tmp_dir);
    let state_store = &arx_db.state_store;
    ...
    {
        let pruner = create_state_pruner_manager(&arx_db.state_merkle_db, prune_batch_size);
...
fn test_state_store_pruner_disabled() {
    ...
    let num_versions = 25;
    let tmp_dir = TempPath::new();
    let arx_db = ArxDB::new_for_test(&tmp_dir);
    let state_store = &arx_db.state_store;
...
fn test_worker_quit_eagerly() {
    ...
    let tmp_dir = TempPath::new();
    let arx_db = ArxDB::new_for_test(&tmp_dir);
    let db = Arc::clone(&arx_db.ledger_db);
    let state_store = &arx_db.state_store;
...
fn verify_state_value_pruner(inputs: Vec<Vec<(StateKey, Option<StateValue>)>>) {
    let tmp_dir = TempPath::new();
    let db = ArxDB::new_for_test(&tmp_dir);
