...
use crate::{ArxDB, LedgerPrunerManager, LedgerStore, PrunerManager, TransactionStore};
use arx_accumulator::HashReader;
use arx_config::config::LedgerPrunerConfig;
use arx_schemadb::SchemaBatch;
...
fn verify_write_set_pruner(write_sets: Vec<WriteSet>) {
    ...
    let transaction_store = &arx_db.transaction_store;
    let num_write_sets = write_sets.len();

    let pruner = LedgerPrunerManager::new(
        Arc::clone(&arx_db.ledger_db),
        Arc::clone(&arx_db.state_store),
        LedgerPrunerConfig {
            enable: true,
            prune_window: 0,
            batch_size: 1,
    ...
    arx_db.ledger_db.write_schemas(batch).unwrap();
...
fn verify_txn_store_pruner(
    ...
    let arx_db = ArxDB::new_for_test(&tmp_dir);
    let transaction_store = &arx_db.transaction_store;
    let ledger_store = LedgerStore::new(Arc::clone(&arx_db.ledger_db));
    ...
    for i in (0..=num_transaction).step_by(step_size) {
        ...
        for j in i..num_transaction {
            ...
            arx_db.get_accumulator_summary(j as Version).unwrap();
        }
