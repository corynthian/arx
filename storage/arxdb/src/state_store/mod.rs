...
use crate::{
    ...
    ArxDbError, LedgerStore, StaleNodeIndexCrossEpochSchema, StaleNodeIndexSchema,
...
use arx_executor_types::in_memory_state_calculator::InMemoryStateCalculator;
use arx_infallible::Mutex;
use arx_jellyfish_merkle::iterator::JellyfishMerkleIterator;
use arx_logger::info;
use arx_schemadb::{ReadOptions, SchemaBatch, DB};
use arx_state_view::StateViewId;
    ...
    fn get_state_storage_usage(&self, version: Option<Version>) -> Result<StateStorageUsage> {
        version.map_or(Ok(StateStorageUsage::zero()), |version| {
            Ok(self
                ...
                .ok_or_else(|| ArxDbError::NotFound(format!("VersionData at {}", version)))?
...
impl StateStore {
    ...
    pub fn merklize_value_set(
        ...
        node_hashes: Option<&HashMap<arx_types::nibble::nibble_path::NibblePath, HashValue>>,
        version: Version,
    ...
    pub fn get_value_chunk_with_proof(
        ...
        let state_key_values: Vec<(StateKey, StateValue)> = result_iter
            ...
            ArxDbError::NotFound(format!("State chunk starting at {}", first_index)),
        );
    ...
    // TODO: change to async comment once it does like https://github.com/arx-labs/arx-core/blob/159b00f3d53e4327523052c1b99dd9889bf13b03/storage/backup/backup-cli/src/backup_types/state_snapshot/restore.rs#L147 or overlap at least two chunks.
    ...
    pub fn get_all_jmt_nodes_referenced(
        &self,
        version: Version,
    ) -> Result<Vec<arx_jellyfish_merkle::node_type::NodeKey>> {
        arx_jellyfish_merkle::JellyfishMerkleTree::new(self.state_merkle_db.as_ref())
            .get_all_nodes_referenced(version)
    }

    #[cfg(test)]
    pub fn get_all_jmt_nodes(&self) -> Result<Vec<arx_jellyfish_merkle::node_type::NodeKey>> {
        let mut iter = self
            .state_db
            .state_merkle_db
            .db
