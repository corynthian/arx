...
use crate::{
    metrics::{
        ARX_SCHEMADB_BATCH_COMMIT_BYTES, ARX_SCHEMADB_BATCH_COMMIT_LATENCY_SECONDS,
        ARX_SCHEMADB_BATCH_PUT_LATENCY_SECONDS, ARX_SCHEMADB_DELETES, ARX_SCHEMADB_GET_BYTES,
        ARX_SCHEMADB_GET_LATENCY_SECONDS, ARX_SCHEMADB_ITER_BYTES,
        ARX_SCHEMADB_ITER_LATENCY_SECONDS, ARX_SCHEMADB_PUT_BYTES,
    },
...
impl SchemaBatch {
    ...
    pub fn put<S: Schema>(&self, key: &S::Key, value: &S::Value) -> Result<()> {
        let _timer = ARX_SCHEMADB_BATCH_PUT_LATENCY_SECONDS
...
impl DB {
    ...
    pub fn get<S: Schema>(&self, schema_key: &S::Key) -> Result<Option<S::Value>> {
        let _timer = ARX_SCHEMADB_GET_LATENCY_SECONDS
    ...
    pub fn write_schemas(&self, batch: SchemaBatch) -> Result<()> {
        let _timer = ARX_SCHEMADB_BATCH_COMMIT_LATENCY_SECONDS
            ...
            for write_op in rows {
                match write_op {
                    ...
                    WriteOp::Deletion { key: _ } => {
                        ARX_SCHEMADB_DELETES.with_label_values(&[cf_name]).inc();
                    },
                }
            }
        }
