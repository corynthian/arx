...
use crate::{
    metrics::{ARX_JELLYFISH_INTERNAL_ENCODED_BYTES, ARX_JELLYFISH_LEAF_ENCODED_BYTES},
    TreeReader,
};
    ...
    pub fn encode(&self) -> Result<Vec<u8>> {
        ...
        match self {
            Node::Internal(internal_node) => {
                ...
                ARX_JELLYFISH_INTERNAL_ENCODED_BYTES.inc_by(out.len() as u64);
            },
            Node::Leaf(leaf_node) => {
                out.push(NodeTag::Leaf as u8);
                out.extend(bcs::to_bytes(&leaf_node)?);
                ARX_JELLYFISH_LEAF_ENCODED_BYTES.inc_by(out.len() as u64);
            },
            Node::Null => {
