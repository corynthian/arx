...
use arx_db::backup::restore_handler::RestoreHandler;
use arx_executor::chunk_executor::ChunkExecutor;
use arx_executor_types::TransactionReplayer;
...
impl TransactionRestoreBatchController {
    ...
    async fn run_impl(self) -> Result<()> {
        ...
        if let RestoreRunMode::Restore { restore_handler } = self.global_opt.run_mode.as_ref() {
            ArxVM::set_concurrency_level_once(self.global_opt.replay_concurrency_level);
    ...
    async fn replay_transactions(
        ...
        let db = DbReaderWriter::from_arc(Arc::clone(&restore_handler.arxdb));
        let chunk_replayer = Arc::new(ChunkExecutor::<ArxVM>::new(db));
