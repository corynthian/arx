...
use crate::{
    ...
    metrics::{
        ARX_EXECUTOR_COMMIT_BLOCKS_SECONDS, ARX_EXECUTOR_EXECUTE_BLOCK_SECONDS,
        ARX_EXECUTOR_OTHER_TIMERS_SECONDS, ARX_EXECUTOR_SAVE_TRANSACTIONS_SECONDS,
        ARX_EXECUTOR_TRANSACTIONS_SAVED, ARX_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS,
    },
};
use anyhow::Result;
use arx_crypto::HashValue;
use arx_executor_types::{BlockExecutorTrait, Error, StateComputeResult};
use arx_infallible::RwLock;
use arx_logger::prelude::*;
use arx_scratchpad::SparseMerkleTree;
...
impl TransactionBlockExecutor<Transaction> for ArxVM {
    fn execute_transaction_block(
        transactions: Vec<Transaction>,
        state_view: CachedStateView,
    ) -> Result<ChunkOutput> {
        ChunkOutput::by_transaction_execution::<ArxVM>(transactions, state_view)
    }
}
    ...
    T: Send + Sync,
{
    ...
    fn execute_block(
        ...
        let output = if parent_block_id != committed_block.id && parent_output.has_reconfiguration()
        {
            ...
            let _timer = ARX_EXECUTOR_EXECUTE_BLOCK_SECONDS.start_timer();
            let state_view = {
                let _timer = ARX_EXECUTOR_OTHER_TIMERS_SECONDS
            ...
            let chunk_output = {
                let _timer = ARX_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.start_timer();
    ...
    fn commit_blocks_ext(
        ...
        let _timer = ARX_EXECUTOR_COMMIT_BLOCKS_SECONDS.start_timer();
        ...
        let _timer = ARX_EXECUTOR_SAVE_TRANSACTIONS_SECONDS.start_timer();
        ARX_EXECUTOR_TRANSACTIONS_SAVED.observe(to_commit as f64);
