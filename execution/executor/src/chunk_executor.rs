...
use crate::{
    ...
    metrics::{
        ARX_EXECUTOR_APPLY_CHUNK_SECONDS, ARX_EXECUTOR_COMMIT_CHUNK_SECONDS,
        ARX_EXECUTOR_EXECUTE_CHUNK_SECONDS, ARX_EXECUTOR_VM_EXECUTE_CHUNK_SECONDS,
    },
};
use anyhow::Result;
use arx_executor_types::{
...
impl<V: VMExecutor> ChunkExecutorInner<V> {
    ...
    fn execute_chunk(
        ...
        let _timer = ARX_EXECUTOR_EXECUTE_CHUNK_SECONDS.start_timer();
        ...
        let chunk_output = {
            let _timer = ARX_EXECUTOR_VM_EXECUTE_CHUNK_SECONDS.start_timer();
    ...
    fn apply_chunk(
        ...
        let _timer = ARX_EXECUTOR_APPLY_CHUNK_SECONDS.start_timer();
    ...
    fn commit_chunk(&self) -> Result<ChunkCommitNotification> {
        let _timer = ARX_EXECUTOR_COMMIT_CHUNK_SECONDS.start_timer();
...
fn verify_chunk(
    ...
    (
        arx_types::proof::TransactionInfoListWithProof,
        usize,
        Vec<Transaction>,
    ),
    anyhow::Error,
...
/// This is because the [FakeArxDB] from where these transactions come from
