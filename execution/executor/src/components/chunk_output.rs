...
use anyhow::Result;
use arx_executor_types::ExecutedChunk;
use arx_logger::{sample, sample::SampleRate, trace, warn};
...
impl ChunkOutput {
    ...
    fn execute_block<V: VMExecutor>(
        ...
        use arx_state_view::{StateViewId, TStateView};
        use arx_types::write_set::WriteSet;
...
pub fn update_counters_for_processed_chunk(
    ...
    let detailed_counters = ArxVM::get_processed_transactions_detailed_counters();
    ...
    for (txn, output) in transactions.iter().zip(transaction_outputs.iter()) {
        ...
        metrics::ARX_PROCESSED_TXNS_COUNT
            .with_label_values(&[process_type, kind, state])
            .inc();

        if !error_code.is_empty() {
            metrics::ARX_PROCESSED_FAILED_TXNS_REASON_COUNT
                .with_label_values(&[
        ...
        if let Transaction::UserTransaction(user_txn) = txn {
            match user_txn.payload() {
                arx_types::transaction::TransactionPayload::Script(_script) => {
                    metrics::ARX_PROCESSED_USER_TRANSACTIONS_PAYLOAD_TYPE
                        .with_label_values(&[process_type, "script", state])
                        .inc();
                },
                arx_types::transaction::TransactionPayload::ModuleBundle(_module) => {
                    metrics::ARX_PROCESSED_USER_TRANSACTIONS_PAYLOAD_TYPE
                        .with_label_values(&[process_type, "module", state])
                        .inc();
                },
                arx_types::transaction::TransactionPayload::EntryFunction(function) => {
                    metrics::ARX_PROCESSED_USER_TRANSACTIONS_PAYLOAD_TYPE
                        .with_label_values(&[process_type, "function", state])
                        .inc();

                    let is_core = function.module().address() == &CORE_CODE_ADDRESS;
                    metrics::ARX_PROCESSED_USER_TRANSACTIONS_ENTRY_FUNCTION_MODULE
                        .with_label_values(&[
                            detailed_counters_label,
                            process_type,
                    ...
                    if is_core && detailed_counters {
                        metrics::ARX_PROCESSED_USER_TRANSACTIONS_ENTRY_FUNCTION_CORE_METHOD
                            .with_label_values(&[
                                process_type,
                                function.module().name().as_str(),
        ...
        for event in output.events() {
            ...
            metrics::ARX_PROCESSED_USER_TRANSACTIONS_CORE_EVENTS
                .with_label_values(&[
                    detailed_counters_label,
                    process_type,
                    if is_core { "core" } else { "user" },
