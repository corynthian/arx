...
use arx_crypto::{ed25519::Ed25519PrivateKey, HashValue, PrivateKey, Uniform};
use arx_db::ArxDB;
use arx_executor::{
    block_executor::BlockExecutor,
    db_bootstrapper::{generate_waypoint, maybe_bootstrap},
};
use arx_executor_test_helpers::{
    bootstrap_genesis, gen_ledger_info_with_sigs, get_test_signed_transaction,
};
use arx_executor_types::BlockExecutorTrait;
use arx_gas::{ChangeSetConfigs, LATEST_GAS_FEATURE_VERSION};
...
use arx_types::{
    ...
    account_config::{
        arx_test_root_address, new_block_event_key, CoinStoreResource, NewBlockEvent,
        CORE_CODE_ADDRESS,
    },
...
#[test]
fn test_empty_db() {
    let genesis = arx_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis.0));
    let tmp_dir = TempPath::new();
    let db_rw = DbReaderWriter::new(ArxDB::new_for_test(&tmp_dir));
    ...
    // Bootstrap empty DB.
    let waypoint = generate_waypoint::<ArxVM>(&db_rw, &genesis_txn).expect("Should not fail.");
    maybe_bootstrap::<ArxVM>(&db_rw, &genesis_txn, waypoint).unwrap();
    ...
    assert!(!maybe_bootstrap::<ArxVM>(&db_rw, &genesis_txn, waypoint).unwrap());
}
...
fn execute_and_commit(txns: Vec<Transaction>, db: &DbReaderWriter, signer: &ValidatorSigner) {
    ...
    let executor = BlockExecutor::<ArxVM, Transaction>::new(db.clone());
    let output = executor
...
fn get_arx_coin_mint_transaction(
    arx_root_key: &Ed25519PrivateKey,
    arx_root_seq_num: u64,
    account: &AccountAddress,
    amount: u64,
) -> Transaction {
    get_test_signed_transaction(
        arx_test_root_address(),
        /* sequence_number = */ arx_root_seq_num,
        arx_root_key.clone(),
        arx_root_key.public_key(),
        Some(arx::arx_coin_mint(*account, amount)),
    )
}
...
fn get_account_transaction(
    ...
    get_test_signed_transaction(
        ...
        arx_root_key.clone(),
        arx_root_key.public_key(),
        Some(arx::arx_account_create_account(*account)),
    )
}

fn get_arx_coin_transfer_transaction(
    ...
    get_test_signed_transaction(
        ...
        Some(arx::arx_coin_transfer(recipient, amount)),
    )
}
...
fn get_configuration(db: &DbReaderWriter) -> ConfigurationResource {
    let db_state_view = db.reader.latest_state_checkpoint_view().unwrap();
    let arx_framework_account_state_view =
        db_state_view.as_account_with_state_view(&CORE_CODE_ADDRESS);
    arx_framework_account_state_view
...
fn test_new_genesis() {
    let genesis = arx_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_key = &arx_vm_genesis::GENESIS_KEYPAIR.0;
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis.0));
    // Create bootstrapped DB.
    let tmp_dir = TempPath::new();
    let db = DbReaderWriter::new(ArxDB::new_for_test(&tmp_dir));
    let waypoint = bootstrap_genesis::<ArxVM>(&db, &genesis_txn).unwrap();
    ...
    let txn3 = get_arx_coin_mint_transaction(genesis_key, 2, &account1, 200_000_000);
    let txn4 = get_arx_coin_mint_transaction(genesis_key, 3, &account2, 200_000_000);
    ...
    let waypoint = generate_waypoint::<ArxVM>(&db, &genesis_txn).unwrap();
    assert!(maybe_bootstrap::<ArxVM>(&db, &genesis_txn, waypoint).unwrap());
    ...
    // Transfer some money.
    let txn = get_arx_coin_transfer_transaction(account1, 0, &account1_key, account2, 50_000_000);
