...
use arx_crypto::{hash::CryptoHash, PrivateKey};
...
use arx_storage_interface::state_view::DbStateViewAtVersion;
use arx_types::{
    access_path::AccessPath,
    account_config::{arx_test_root_address, AccountResource, CORE_CODE_ADDRESS},
...
fn test_genesis() {
    ...
    let genesis = arx_vm_genesis::test_genesis_transaction();
    ...
    let (arx_framework_account_resource, state_proof) = db
        .reader
    ...
    state_proof
        .verify(
            ...
            arx_framework_account_resource.as_ref(),
        )
        .unwrap();
}
...
fn test_reconfiguration() {
    ...
    let txn1 = get_test_signed_transaction(
        ...
        Some(arx::arx_coin_mint(validator_account, 1_000_000)),
    );
    ...
    // txn3 = set the arx version
    let txn3 = get_test_signed_transaction(
        arx_test_root_address(),
        /* sequence_number = */ 1,
        genesis_key.clone(),
        genesis_key.public_key(),
        Some(arx::version_set_version(42)),
    );
    ...
    let t3 = db
        .reader
        .get_account_transaction(arx_test_root_address(), 1, true, current_version)
        .unwrap();
    ...
    let arx_framework_account_state_view2 =
        db_state_view.as_account_with_state_view(&CORE_CODE_ADDRESS);

    assert_eq!(
        arx_framework_account_state_view2
            .get_version()
            .unwrap()
            .unwrap()
            .major,
