...
use arx_consensus_types::block::Block;
use arx_db::ArxDB;
...
use arx_state_view::account_with_state_view::{AccountWithStateView, AsAccountWithStateView};
use arx_storage_interface::{state_view::DbStateViewAtVersion, DbReaderWriter, Order};
use arx_types::{
    account_config::arx_test_root_address,
...
pub fn test_execution_with_storage_impl() -> Arc<ArxDB> {
    ...
    let mut core_resources_account: LocalAccount = LocalAccount::new(
        ...
        AccountKey::from_private_key(arx_vm_genesis::GENESIS_KEYPAIR.0.clone()),
        0,
    );

    let path = arx_temppath::TempPath::new();
    path.create_as_dir().unwrap();
    let (arx_db, db, executor, waypoint) = create_db_and_executor(path.path(), &genesis_txn);

    let parent_block_id = executor.committed_block_id();
    let signer = arx_types::validator_signer::ValidatorSigner::new(
    ...
    let reconfig1 = core_resources_account
        .sign_with_transaction_builder(txn_factory.payload(arx::version_set_version(100)));
    ...
    let reconfig2 = core_resources_account
        .sign_with_transaction_builder(txn_factory.payload(arx::version_set_version(200)));
    ...
    let account1_received_events = db
        ...
        .unwrap();
    // Account1 has one deposit event since ArxCoin was minted to it.
...
pub fn create_db_and_executor<P: AsRef<std::path::Path>>(
    ...
    Arc<ArxDB>,
    DbReaderWriter,
    BlockExecutor<ArxVM, Transaction>,
    Waypoint,
) {
    let (db, dbrw) = DbReaderWriter::wrap(ArxDB::new_for_test(&path));
    let waypoint = bootstrap_genesis::<ArxVM>(&dbrw, genesis).unwrap();
