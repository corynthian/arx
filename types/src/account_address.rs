...
const SALT: &[u8] = b"arx::staking";
const VESTING_LOCK_SALT: &[u8] = b"arx::vesting";
...
pub fn default_owner_stake_lock_address(owner: AccountAddress) -> AccountAddress {
    default_stake_lock_address(owner, owner)
}

pub fn default_stake_lock_address(
    owner: AccountAddress,
    operator: AccountAddress,
) -> AccountAddress {
    create_stake_lock_address(owner, operator, &[])
}

pub fn create_stake_lock_address(
    owner: AccountAddress,
    operator: AccountAddress,
    seed: &[u8],
) -> AccountAddress {
...
pub fn create_vesting_contract_address(
    ...
    full_seed.extend(VESTING_LOCK_SALT);
    full_seed.extend(seed);
    create_resource_address(admin, &full_seed)
}

pub fn create_vesting_address(
    admin: AccountAddress,
    operator: AccountAddress,
    nonce: u64,
    seed: &[u8],
    ...
    create_stake_lock_address(contract, operator, seed)
}
...
mod hasher {
    #[derive(serde::Deserialize, arx_crypto_derive::CryptoHasher)]
...
mod test {
    ...
    fn address_hash() {
        ...
        let hash_vec =
            &Vec::from_hex("10cb5fda1519dd286a10a6276f53b2e9ec0eec31cffb204db43e77b7bd349b5b")
