...
/// 4. Perform a ArxNet version negotiation handshake (version 1).
///
...
/// ## Transport
///
/// In addition, `NetworkAddress` is integrated with the ArxNet concept of a
...
///
/// use arx_types::network_address::NetworkAddress;
/// use bcs;
...
impl NetworkAddress {
    ...
    ///
    /// ```rust
    /// use arx_crypto::{traits::ValidCryptoMaterialStringExt, x25519};
    /// use arx_types::network_address::NetworkAddress;
    /// use std::str::FromStr;
    ///
    ...
    /// Check that a `NetworkAddress` looks like a typical ArxNet address with
    /// associated protocols.
    ///
    /// "typical" ArxNet addresses begin with a transport protocol:
    ///
    ...
    /// assert!(addr.is_arxnet_addr());
    /// ```
    pub fn is_arxnet_addr(&self) -> bool {
        parse_arxnet_protos(self.as_slice()).is_some()
    }
...
impl FromStr for NetworkAddress {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        while let Some(protocarx_type) = parts_iter.next() {
            // Allow for trailing or duplicate '/'
            if protocarx_type.is_empty() {
                continue;
            }
            protocols.push(Protocol::parse(protocarx_type, &mut parts_iter)?);
        }
...
#[cfg(any(test, feature = "fuzzing"))]
pub fn arb_arxnet_addr() -> impl Strategy<Value = NetworkAddress> {
    ...
    let arb_arxnet_protos = any::<(x25519::PublicKey, u8)>()
        .prop_map(|(pubkey, hs)| vec![Protocol::NoiseIK(pubkey), Protocol::Handshake(hs)]);

    (arb_transport_protos, arb_arxnet_protos).prop_map(
        |(mut transport_protos, mut arxnet_protos)| {
            transport_protos.append(&mut arxnet_protos);
...
impl Protocol {
    fn parse<'a>(
        ...
        let protocol = match protocarx_type {
...
/// parse canonical arxnet protocols
///
/// See: [`NetworkAddress::is_arxnet_addr`]
fn parse_arxnet_protos(protos: &[Protocol]) -> Option<&[Protocol]> {
...
mod test {
    ...
    proptest! {
        #[test]
        fn test_network_address_canonical_serialization(addr in arb_arxnet_addr()) {
        ...
        #[test]
        fn test_network_address_display_roundtrip(addr in arb_arxnet_addr()) {
        ...
        #[test]
        fn test_is_arxnet_addr(addr in arb_arxnet_addr()) {
            assert!(addr.is_arxnet_addr(), "addr.is_arxnet_addr() = false; addr: '{}'", addr);
        }
    }
}
