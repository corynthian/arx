...
use arx_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, Uniform};
use arx_db::ArxDB;
...
struct TestValidator {
    ...
    _db_path: arx_temppath::TempPath,
}

impl TestValidator {
    fn new() -> Self {
        let _db_path = arx_temppath::TempPath::new();
        _db_path.create_as_dir().unwrap();
        let (db, db_rw) = DbReaderWriter::wrap(ArxDB::new_for_test(_db_path.path()));
        arx_executor_test_helpers::bootstrap_genesis::<ArxVM>(
            &db_rw,
            &arx_vm_genesis::test_genesis_transaction(),
        )
...
//   language e2e tests in: arx-core/language/e2e-testsuite/src/tests/verify_txn.rs ->
...
// * SENDING_ACCOUNT_FROZEN: Tested in functional-tests/tests/arx_account/freezing.move.
...
fn test_validate_transaction() {
    ...
    let address = account_config::arx_test_root_address();
    let program = arx::arx_coin_mint(address, 100);
    let transaction = transaction_test_helpers::get_test_signed_txn(
        address,
        1,
        &arx_vm_genesis::GENESIS_KEYPAIR.0,
        arx_vm_genesis::GENESIS_KEYPAIR.1.clone(),
        Some(program),
    );
...
fn test_validate_invalid_signature() {
    ...
    let address = account_config::arx_test_root_address();
    let program = arx::arx_coin_transfer(address, 100);
...
fn test_get_account_sequence_number() {
    let vm_validator = TestValidator::new();
    let root_address = account_config::arx_test_root_address();
